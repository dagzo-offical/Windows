...
cat <<TIP
  ...
  KEYINGI QADAMLAR
   ...
   3a) CTF (Nimbus Reports): Dashboard → «CTF» → nishon
      http://${LAN:-localhost}:8085 ni brauzer + Burp bilan buzing
      (IDOR · info-disclosure · XSS · SSRF).
      Tarmoqda ulanish muammo bo'lsa, CTF nishonini mustaqil (oflayn,
      Docker'siz) ishga tushiring:  cd ../lab/ctf-web && python3 app.py   (:8085).
