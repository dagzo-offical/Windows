...
const RE_BOOK={
  ...
  chapters:[
    ...
    { uz:"Switch va sakrash jadvali", en:"Switch statements & jump tables", subUz:"Kompilyatorning yashirin optimallashtiruvi", subEn:"A hidden compiler optimization",
      blocks:[
        {t:"p",uz:"`switch` ko'plab `if/else if` zanjiriga o'xshaydi, lekin qiymatlar zich (masalan 0,1,2,3...) bo'lsa, kompilyator ancha aqlliroq yechim ishlatadi: **sakrash jadvali (jump table)** — manzillar massivi, unda to'g'ridan-to'g'ri qiymat bo'yicha **indekslanadi**.",
              en:"A `switch` looks like a chain of `if/else if`, but when values are dense (e.g. 0,1,2,3...) the compiler uses a much smarter approach: a **jump table** — an array of addresses, **indexed** directly by the value."},
        {t:"code",lang:"c",cap:{uz:"Manba kod",en:"Source"},lines:[
          ...
          "    }",
          "}"]},
        {t:"code",lang:"asm",cap:{uz:"Natija — soddalashtirilgan",en:"Output — simplified"},lines:[
          "day_type:",
          "    cmp   edi, 2",
          "    ja    .default        ; agar day > 2 (yoki manfiy), sakra",
          "    lea   rax, [jump_table]",
          "    mov   eax, [rax + rdi*4] ; jadvaldan manzilni ol",
          "    jmp   rax              ; o'sha manzilga sakra",
          ".case0: mov eax, 100 | ret",
          ".case1: mov eax, 200 | ret",
          ".case2: mov eax, 300 | ret",
          ".default: mov eax, -1 | ret",
          "jump_table: .quad .case0, .case1, .case2"]},
        {t:"note",uz:"RE'da bu naqshni tanish muhim: agar `jmp` buyrug'i **registrga** (masalan `jmp rax`) sakrasa — bu deyarli har doim switch/jump table. Disassemblerlar buni odatda avtomatik aniqlab, chiroyli ko'rsatadi. Qiymatlar siyrak bo'lsa (masalan 5, 1000, 99999), kompilyator jadval o'rniga oddiy `cmp`+`je` zanjiriga qaytadi.",
              en:"Recognizing this pattern matters in RE: if a `jmp` targets a **register** (e.g. `jmp rax`) — it's almost always a switch/jump table. Disassemblers usually detect and display this nicely. When values are sparse (e.g. 5, 1000, 99999), the compiler falls back to a plain `cmp`+`je` chain instead of a table."},
      ]},
    { uz:"Bitli amallar", en:"Bitwise operations", subUz:"AND, OR, XOR va siljitish — tezkor hiylalar", subEn:"AND, OR, XOR and shifts — fast tricks",
      blocks:[
        {t:"p",uz:"Bitli amallar (`and`, `or`, `xor`, `shl`, `shr`) — sonning har bir bitiga alohida ta'sir qiladigan buyruqlar. Ular oddiy ko'rinsa-da, kompilyatorlar ulardan **tezlik uchun** kutilmagan tarzda foydalanadi — buni bilish RE'da chalkashlikni oldini oladi.",
              en:"Bitwise operations (`and`, `or`, `xor`, `shl`, `shr`) act on each bit of a number individually. They look simple, but compilers use them for **speed** in unexpected ways — knowing this prevents confusion in RE."},
        {t:"h",n:"§",uz:"Keng tarqalgan hiylalar",en:"Common tricks"},
        {t:"code",lang:"asm",cap:{uz:"Naqshlar va ularning ma'nosi",en:"Patterns and what they mean"},lines:[
          "xor  eax, eax     ; eax = 0  (mov eax,0 dan tezroq!)",
          "shl  eax, 3       ; eax = eax * 8   (2^3 = 8)",
          "sar  eax, 1       ; eax = eax / 2   (ishorali son uchun)",
          "and  eax, 1       ; eax = eax % 2   (juft/toq tekshiruvi)",
          "and  eax, 0xF     ; eax ning faqat pastki 4 bitini qoldir",
          "test eax, eax     ; eax == 0 ekanini tekshiradi (bayroq o'rnatadi, o'zgartirmaydi)"]},
        {t:"h",n:"§",uz:"Nega bu muhim",en:"Why this matters"},
        {t:"p",uz:"`x * 8` yozgan dasturchi assemblerda `shl eax, 3` ko'radi — ko'paytirish buyrug'i **umuman yo'q**! Kompilyator 2'ning darajasiga ko'paytirish/bo'lishni siljitish bilan almashtiradi, chunki bu protsessorda ancha tezroq. Xuddi shunday, `if (x == 0)` ko'pincha `test eax, eax` + `je` ga aylanadi, `cmp eax, 0` emas — ikkalasi ham bir xil natija beradi, lekin `test` bir oz tezroq.",
              en:"A programmer who wrote `x * 8` will see `shl eax, 3` in assembly — there's **no multiply instruction at all**! The compiler replaces multiplication/division by a power of two with a shift, because it's much faster on the processor. Similarly, `if (x == 0)` often becomes `test eax, eax` + `je`, not `cmp eax, 0` — both give the same result, but `test` is slightly faster."},
        {t:"note",uz:"`xor reg, reg` — registrni nolga tenglashtirishning eng keng tarqalgan usuli. Buni ko'rganingizda \"bu o'zgaruvchini 0 ga o'rnatyapti\" deb o'qing, \"XOR nima?\" deb emas.",
              en:"`xor reg, reg` is the most common way to zero out a register. When you see it, read it as \"this sets the variable to 0\", not \"what is this XOR doing?\"."},
      ]},
    { uz:"Satrlar va buferlar", en:"Strings & buffers", subUz:"C satrlari va ularning xavfsizlik oqibatlari", subEn:"C strings and their security consequences",
      blocks:[
        {t:"p",uz:"C tilida satr — oddiy bayt massivi, oxiri **nol bayt** (`\\0`) bilan belgilanadi. Bu oddiy g'oya ko'plab real dunyo xavfsizlik zaifliklarining ildizi — va RE'da satr bilan ishlovchi kodni tanish muhim ko'nikma.",
              en:"In C, a string is just a byte array terminated by a **null byte** (`\\0`). This simple idea is the root of many real-world security vulnerabilities — and recognizing string-handling code is an important RE skill."},
        {t:"h",n:"§",uz:"strlen naqshi",en:"The strlen pattern"},
        {t:"p",uz:"`strlen(s)` funksiyasi ichida deyarli har doim **sikl** ko'rasiz (8-bobni eslang): bayt-baytlab o'qib, nolga tenglashtirib solishtiradi, toki nol bayt topilmaguncha. Zamonaviy kompilyatorlar buni tezkor `rep scasb` buyrug'i yoki SIMD (bir vaqtda ko'p bayt) bilan optimallashtirishi mumkin — bu murakkabroq ko'rinadi, lekin mantiq bir xil: **nol baytni qidirish**.",
              en:"Inside `strlen(s)` you'll almost always see a **loop** (recall chapter 8): reading byte by byte, comparing against zero, until the null byte is found. Modern compilers may optimize this with the fast `rep scasb` instruction or SIMD (processing many bytes at once) — it looks more complex, but the logic is the same: **searching for the null byte**."},
        {t:"h",n:"§",uz:"Bufer to'lib ketishi — RE nuqtai nazaridan",en:"Buffer overflow — from an RE perspective"},
        {t:"p",uz:"Agar kod sobit o'lchamli buferga (`char buf[16]`) tekshiruvsiz nusxa ko'chirsa (`strcpy`, o'lchamni tekshirmasdan), kiruvchi ma'lumot 16 baytdan uzun bo'lsa — u **stekdagi qo'shni ma'lumotni** (6-bobdagi stek freymini eslang: saqlangan `rbp`, qaytish manzili) ustiga yozib yuboradi. RE'da bunday kodni topish uchun: fiksirlangan o'lchamli stek buferi (`[rbp-16]`) + o'lcham tekshiruvisiz nusxa ko'chirish funksiyasi chaqiruvini qidiring.",
              en:"If code copies into a fixed-size buffer (`char buf[16]`) without a bounds check (`strcpy`, no length check), and the input is longer than 16 bytes, it will **overwrite adjacent data on the stack** (recall the stack frame from chapter 6: saved `rbp`, the return address). To spot this in RE: look for a fixed-size stack buffer (`[rbp-16]`) combined with a call to an unchecked copy function."},
        {t:"note",warn:true,uz:"Bu — Windows va Web-Pentest modullaridagi xavfsizlik mavzularining past darajadagi asosi: bufer to'lib ketishi hujumlari aynan shu mexanizmni ekspluatatsiya qiladi. Zamonaviy tizimlar buni **stack canary**, **ASLR** va **DEP/NX** kabi himoyalar bilan qiyinlashtiradi.",
              en:"This is the low-level foundation of security topics in the Windows and Web-Pentest modules: buffer overflow attacks exploit exactly this mechanism. Modern systems make it harder with defences like **stack canaries**, **ASLR** and **DEP/NX**."},
      ]},
    { uz:"O'nlik kasr sonlar (floating point)", en:"Floating-point numbers", subUz:"Nega kasr sonlar assemblerda boshqacha ko'rinadi", subEn:"Why floats look different in assembly",
      blocks:[
        {t:"p",uz:"Shu paytgacha ko'rgan barcha misollarimiz **butun sonlar** (integer) bilan ishladi — ular umumiy registrlarda (`rax`, `rdi`...) saqlanadi. **Kasr sonlar** (`float`, `double`) esa butunlay boshqa, alohida registrlar to'plamida — **XMM registrlari** (`xmm0`-`xmm15`) da ishlaydi.",
              en:"All our examples so far worked with **integers** — stored in general-purpose registers (`rax`, `rdi`...). **Floating-point numbers** (`float`, `double`) work in a completely different, separate set of registers — the **XMM registers** (`xmm0`-`xmm15`)."},
        {t:"code",lang:"c",cap:{uz:"Manba kod",en:"Source"},lines:[
        ...
        {t:"code",lang:"asm",cap:{uz:"Natija",en:"Output"},lines:[
          "add:",
          "    addss  xmm0, xmm1  ; xmm0 = xmm0 + xmm1 (bitta aniqlikdagi qo'shish)",
          "    ret                ; natija xmm0 da qaytariladi, eax emas!"]},
        {t:"h",n:"§",uz:"Muhim farqlar",en:"Key differences"},
        {t:"ul",uz:[
          "Kasr sonlar **argumentlari** ham boshqacha uzatiladi: `rdi`/`rsi` o'rniga **xmm0**, **xmm1**...",
          "**Qaytish qiymati** ham `rax` emas, **xmm0** da bo'ladi.",
          "Buyruqlar boshqacha: `add` o'rniga `addss` (single) yoki `addsd` (double), `mov` o'rniga `movss`/`movsd`.",
          "`cmp` o'rniga `comiss`/`ucomiss` ishlatiladi — kasr sonlarni solishtirish uchun."],
          en:[
          "Float **arguments** are also passed differently: **xmm0**, **xmm1**... instead of `rdi`/`rsi`.",
          "The **return value** is also in **xmm0**, not `rax`.",
          "Instructions differ: `addss` (single) or `addsd` (double) instead of `add`, `movss`/`movsd` instead of `mov`.",
          "`comiss`/`ucomiss` are used instead of `cmp` — to compare floating-point numbers."]},
        {t:"note",uz:"RE'da agar funksiya `xmm` registrlari bilan ishlayotganini ko'rsangiz — darhol bu kasr sonlar bilan ishlayotganini bilasiz, hatto manba kodni ko'rmasangiz ham.",
              en:"In RE, if you see a function working with `xmm` registers, you immediately know it's dealing with floating-point numbers — even without seeing the source code."},
      ]},
    { uz:"Rekursiya assemblerda", en:"Recursion in assembly", subUz:"Funksiya o'zini o'zi chaqirganda", subEn:"When a function calls itself",
      blocks:[
        {t:"p",uz:"**Rekursiya** — funksiya o'zini o'zi chaqirishi. Assembler nuqtai nazaridan bunda hech qanday sehr yo'q: bu shunchaki 5-bobdagi oddiy `call` buyrug'i, faqat maqsad — **o'sha funksiyaning o'zi**.",
              en:"**Recursion** is a function calling itself. From assembly's point of view there's no magic here: it's simply the ordinary `call` instruction from chapter 5, just targeting **the same function**."},
        {t:"code",lang:"c",cap:{uz:"Manba kod — faktorial",en:"Source — factorial"},lines:[
          "int factorial(int n)",
          "{",
          "    if (n <= 1) return 1;",
        ...
        {t:"code",lang:"asm",cap:{uz:"Natija — soddalashtirilgan",en:"Output — simplified"},lines:[
          ...
          "    ret",
          ".recurse:",
          "    push  rdi           ; joriy n ni saqlab qo'y (keyin kerak bo'ladi)",
          "    sub   edi, 1        ; n - 1",
          "    call  factorial     ; o'zini chaqiradi!",
          "    pop   rdi           ; saqlangan n ni qaytar",
          "    imul  eax, edi      ; eax = factorial(n-1) * n",
          "    ret"]},
        {t:"h",n:"§",uz:"Har chaqiruv — o'z stek freymi",en:"Every call gets its own stack frame"},
        {t:"p",uz:"6 va 9-boblarda ko'rgan **stek freymi** g'oyasi bu yerda o'z kuchini ko'rsatadi: har bir rekursiv chaqiruv o'zining **alohida** stek freymini oladi — o'z lokal o'zgaruvchilari, o'z qaytish manzili bilan. `factorial(5)` chaqirilganda, stekda ketma-ket 5 ta freym to'planadi, so'ng har biri navbat bilan `ret` bilan yig'ishtiriladi.",
              en:"The **stack frame** idea from chapters 6 and 9 shows its full power here: every recursive call gets its **own separate** stack frame — its own locals, its own return address. When `factorial(5)` is called, five frames pile up on the stack in sequence, then each is torn down in turn via `ret`."},
        {t:"note",warn:true,uz:"Agar rekursiya to'xtash sharti (bu yerda `n <= 1`) noto'g'ri bo'lsa yoki umuman bo'lmasa — freymlar cheksiz to'planib, **stek to'lib ketishi (stack overflow)** yuz beradi va dastur qulaydi. Bu — \"stack overflow\" atamasining haqiqiy, past darajadagi ma'nosi.",
              en:"If the recursion's stopping condition (here, `n <= 1`) is wrong or missing, frames pile up endlessly, causing a **stack overflow**, and the program crashes. This is the real, low-level meaning of the term \"stack overflow\"."},
      ]},
    { uz:"Kompilyator optimallashtirishi", en:"Compiler optimization", subUz:"Nega -O0 va -O2 mutlaqo boshqacha ko'rinadi", subEn:"Why -O0 and -O2 look completely different",
      blocks:[
        {t:"p",uz:"Shu paytgacha barcha misollarimiz **optimallashtirilmagan** (-O0) kodga o'xshab tuzilgan — har bir manba qatori aniq bir nechta buyruqqa mos keladi. Lekin real dunyoda dasturlar deyarli har doim **optimallashtirilgan** holda (-O2, -O3) tarqatiladi — va ular butunlay boshqacha ko'rinadi.",
              en:"All our examples so far have resembled **unoptimized** (-O0) code — each source line maps to a clear handful of instructions. But in the real world, programs are almost always shipped **optimized** (-O2, -O3) — and they look completely different."},
        {t:"h",n:"§",uz:"Optimallashtirish nima qiladi",en:"What optimization does"},
        {t:"ul",uz:[
          "**Inlining** — kichik funksiya chaqiruvi butunlay yo'qoladi, uning kodi to'g'ridan-to'g'ri chaqiruvchi ichiga \"joylashtiriladi\". Natijada RE'da kutilgan `call` buyrug'ini topa olmaysiz.",
          "**O'lik kodni olib tashlash** — hech qachon ishlatilmaydigan yoki natijaga ta'sir qilmaydigan kod butunlay yo'q qilinadi.",
          "**Registr taqsimoti** — lokal o'zgaruvchilar stekda emas, to'g'ridan-to'g'ri registrlarda \"yashaydi\" — `[rbp-N]` yozuvlari deyarli yo'qoladi.",
          "**Sikl birlashtirish va yozib tashlash** — bir necha sikl birga qo'shiladi yoki kichik sikllar to'liq \"yozib\" ochiladi (8-bobda eslatilgan)."],
          en:[
          "**Inlining** — a small function call disappears entirely; its code is pasted directly into the caller. As a result, in RE you won't find the `call` instruction you'd expect.",
          "**Dead code elimination** — code that's never used or doesn't affect the result is removed entirely.",
          "**Register allocation** — local variables \"live\" directly in registers instead of the stack; `[rbp-N]` references mostly disappear.",
          "**Loop fusion and unrolling** — several loops get merged, or small loops get fully \"unrolled\" (mentioned in chapter 8)."]},
        {t:"note",warn:true,uz:"**Amaliy xulosa:** real dasturlarni RE qilganda, ushbu kitobdagi \"toza\" -O0 misollarini kutmang. Kod tanish bo'lmagan tartibda, funksiyalar birlashgan, o'zgaruvchilar registrlarda \"sochilib yotgan\" holda ko'rinadi. Shuning uchun ko'p RE mutaxassislari boshida o'z kodlarini **-O0 bilan** kompilyatsiya qilib, disassemblerda ko'rib, keyin asta-sekin -O2 bilan taqqoslab o'rganishni tavsiya qiladi — aynan shu kitobda siz qilayotgan narsa.",
              en:"**Practical takeaway:** when reversing real programs, don't expect the \"clean\" -O0 examples from this book. Code appears in an unfamiliar order, functions are merged, variables are \"scattered\" across registers. That's why many RE practitioners recommend compiling your own code with **-O0** first, viewing it in a disassembler, then gradually comparing against -O2 — exactly the exercise this book has been giving you."},
      ]},
    { uz:"C++ va virtual funksiyalar", en:"C++ and virtual functions", subUz:"Ob'ektga yo'naltirilgan dasturlash assemblerda", subEn:"Object-oriented programming in assembly",
      blocks:[
        {t:"p",uz:"C++ dagi **klass** — protsessor uchun 11-bobdagi `struct`dan unchalik farq qilmaydi. Lekin **virtual funksiyalar** (polimorfizm) yangi bir mexanizm qo'shadi: **vtable (virtual jadval)**.",
              en:"A C++ **class** isn't very different from the `struct` in chapter 11, as far as the processor is concerned. But **virtual functions** (polymorphism) add a new mechanism: the **vtable (virtual table)**."},
        {t:"h",n:"§",uz:"Vtable qanday ishlaydi",en:"How the vtable works"},
        {t:"p",uz:"Virtual funksiyasi bor har bir klass ob'ekti xotirada yashirin **vtable ko'rsatkichi**ga ega (odatda birinchi 8 bayt, ofset 0 da). Vtable — bu klassning barcha virtual funksiyalari manzillarini saqlovchi massiv. Virtual funksiya chaqirilganda: avval ob'ektdan vtable ko'rsatkichi olinadi, so'ng vtable ichidan kerakli funksiya manzili **indeks orqali** olinadi, va o'sha manzilga sakraladi.",
              en:"Every object of a class with virtual functions has a hidden **vtable pointer** in memory (usually the first 8 bytes, offset 0). The vtable is an array holding the addresses of all the class's virtual functions. When a virtual function is called: first the vtable pointer is read from the object, then the target function's address is read from the vtable **by index**, and execution jumps there."},
        {t:"code",lang:"asm",cap:{uz:"Virtual chaqiruv naqshi",en:"The virtual call pattern"},lines:[
          "; obj->speak() chaqiruvi, speak vtable'da 2-o'rinda",
          "mov  rax, [rdi]        ; rax = vtable ko'rsatkichi (ob'ektdan)",
          "mov  rax, [rax + 16]   ; rax = vtable[2] (har biri 8 bayt: 2*8=16)",
          "call rax               ; hisoblangan manzilga chaqir (aniq nom emas!)"]},
        {t:"note",uz:"Bu — 9 va 10-boblardagi \"birinchi argument = `this` ko'rsatkichi\" g'oyasining aynan qo'llanilishi: C++ metodining birinchi (yashirin) argumenti har doim ob'ektning o'zi (`rdi`). RE'da **hisoblangan manzilga `call`** ko'rsangiz (`call rax` kabi, `call func_name` emas) — bu ko'pincha virtual funksiya chaqiruvi yoki funksiya ko'rsatkichi ekanidan dalolat beradi. Dekompilyatorlar (Ghidra) ko'pincha vtable'larni avtomatik aniqlab, klass tuzilishini qayta tiklashga yordam beradi.",
              en:"This is a direct application of the \"first argument = `this` pointer\" idea from chapters 9 and 10: a C++ method's first (hidden) argument is always the object itself (`rdi`). In RE, seeing a **call to a computed address** (like `call rax`, not `call func_name`) often signals a virtual function call or a function pointer. Decompilers (Ghidra) often auto-detect vtables and help reconstruct class structure."},
      ]},
    // ── 12: File formats ────────────────────────────────────────────
    ...
    { uz:"Dinamik bog'lash: PLT va GOT", en:"Dynamic linking: PLT and GOT", subUz:"Import jadvalidagi funksiya haqiqatda qanday chaqiriladi", subEn:"How an imported function is actually called",
      blocks:[
        {t:"p",uz:"O'tgan bobda **import jadvali**ni ko'rdik — dastur qaysi tashqi funksiyalarni ishlatishini. Lekin dastur ishga tushganda `puts` kabi funksiya xotirada **aniq qayerda** joylashganini oldindan bilmaydi, chunki u alohida **umumiy kutubxona** (`.so` yoki `.dll`) faylida, va uning manzili har ishga tushirishda o'zgarishi mumkin (ASLR). Buni **dinamik bog'lash** hal qiladi.",
              en:"In the previous chapter we saw the **import table** — which external functions a program uses. But when the program starts, it doesn't know in advance **exactly where** in memory a function like `puts` lives, because it's in a separate **shared library** file (`.so` or `.dll`), and its address can change on every run (ASLR). **Dynamic linking** solves this."},
        {t:"h",n:"§",uz:"PLT/GOT mexanizmi (Linux)",en:"The PLT/GOT mechanism (Linux)"},
        {t:"p",uz:"Linux'da (ELF) buning uchun ikkita jadval ishlatiladi: **GOT** (Global Offset Table) — haqiqiy manzillarni saqlaydigan joy, va **PLT** (Procedure Linkage Table) — kod undan \"sakrash trambolini\" sifatida foydalanadigan kichik funksiyalar to'plami. Birinchi chaqiruvda dinamik bog'lovchi (**ld.so**) haqiqiy manzilni topib, GOT'ga yozadi (\"**lazy binding**\" — dangasa bog'lash, faqat kerak bo'lganda). Keyingi chaqiruvlar to'g'ridan-to'g'ri GOT'dagi allaqachon ma'lum manzilga o'tadi.",
              en:"On Linux (ELF) this uses two tables: the **GOT** (Global Offset Table) — where real addresses are stored, and the **PLT** (Procedure Linkage Table) — small stub functions that code uses as a \"jump trampoline\". On the first call, the dynamic linker (**ld.so**) resolves the real address and writes it into the GOT (\"**lazy binding**\" — only resolved when needed). Subsequent calls jump straight to the now-known address in the GOT."},
        {t:"code",lang:"asm",cap:{uz:"Import chaqiruvi disassemblerda qanday ko'rinadi",en:"How an imported call looks in a disassembler"},lines:[
          "call  puts@plt        ; \"puts\" ga emas, PLT trambolinga chaqiradi",
          "; PLT stub GOT'dan haqiqiy manzilni o'qib, o'sha yerga sakraydi"]},
        {t:"note",uz:"RE'da `xxx@plt` yoki shunga o'xshash yozuvni ko'rsangiz — bu tashqi (dinamik) funksiya chaqiruvi, dastur o'z ichidagi funksiya emas. Windows'da mexanizm boshqacha nomlanadi (**Import Address Table**, IAT), lekin g'oya bir xil: haqiqiy manzil DLL yuklanganda to'ldiriladi.",
              en:"In RE, seeing `xxx@plt` or similar notation means it's a call to an external (dynamic) function, not code inside the program itself. On Windows the mechanism has a different name (**Import Address Table**, IAT), but the idea is the same: the real address is filled in when the DLL is loaded."},
      ]},
    // ── 13: First CrackMe ───────────────────────────────────────────
    ...
    { uz:"Ikkinchi CrackMe: sikl bilan tekshiruv", en:"Second CrackMe: a loop-based check", subUz:"Bir nechta bobni birlashtirib, murakkabroq mantiqni yechish", subEn:"Combining several chapters to solve harder logic",
      blocks:[
        {t:"p",uz:"Birinchi CrackMe (13-bob) to'g'ridan-to'g'ri bitta `cmp` edi. Endi biroz murakkabroq, real hayotga yaqinroq holatni ko'ramiz: seriya raqami emas, balki **matn kalit so'z** talab qiluvchi dastur — va tekshiruv sikl ichida bajariladi.",
              en:"The first CrackMe (chapter 13) was a single direct `cmp`. Now let's look at something a bit harder and closer to real life: a program that requires a **text key** instead of a serial number — and the check runs inside a loop."},
        {t:"h",n:"§",uz:"Vazifa",en:"The task"},
        {t:"p",uz:"Dastur \"Kalit so'zni kiriting\" deydi. Disassemblerda tekshiruv funksiyasini topasiz va u quyidagicha ko'rinishga ega (8-bobdagi sikl naqshini eslang):",
              en:"The program says \"Enter the key\". In the disassembler you find the check function, and it looks like this (recall the loop pattern from chapter 8):"},
        {t:"code",lang:"asm",cap:{uz:"Tekshiruv funksiyasi — soddalashtirilgan",en:"The check function — simplified"},lines:[
          "check:",
          "    xor   eax, eax    ; sum = 0",
          "    xor   ecx, ecx    ; i = 0",
          ".loop:",
          "    cmp   ecx, 8      ; 8 ta belgigacha",
          "    jge   .done",
          "    movzx edx, byte [rdi + rcx]  ; edx = kiritilgan_soz[i]",
          "    add   eax, edx    ; sum += belgi",
          "    inc   ecx",
          "    jmp   .loop",
          ".done:",
          "    cmp   eax, 850    ; sum kutilgan qiymatga teng ekanini tekshir",
          "    sete  al          ; al = 1 agar teng, 0 agar yo'q",
          "    ret"]},
        {t:"h",n:"§",uz:"Mantiqni tiklash",en:"Reconstructing the logic"},
        {t:"p",uz:"Bu yerda ko'rgan naqshlarni yig'ib tahlil qilamiz: **sikl** (8-bob) 8 ta belgini birma-bir o'qiydi, har birining **ASCII qiymatini** (`movzx` — kengaytirib yuklash) yig'indiga qo'shadi, so'ng yig'indi **850** ga teng ekanini tekshiradi. Demak, vazifa — **8 belgili, ASCII qiymatlari yig'indisi 850 ga teng** bo'lgan istalgan matnni topish. Bu ko'plab yechimga ega (masalan barcha belgilar bir xil bo'lsa, har biri ~106 — \"j\" atrofida), lekin muhimi — **algoritmni tushundik**, aniq bitta \"sehrli son\"ni emas.",
              en:"Let's piece together the patterns we've seen: a **loop** (chapter 8) reads 8 characters one by one, adds each one's **ASCII value** (`movzx` — a widening load) to a running sum, then checks whether the sum equals **850**. So the task is to find any **8-character text whose ASCII values sum to 850**. This has many valid solutions (e.g. if all characters are equal, each is roughly ~106, around \"j\") — but the important part is that we **understood the algorithm**, not just one \"magic number\"."},
        {t:"note",uz:"Bu — real CrackMe'larning odatiy xarakteri: yagona javob emas, balki **algoritmni tushunish** talab etiladi. Murakkabroq CrackMe'larda XOR, jadvallar (10-bob) yoki hatto kriptografik xesh funksiyalari (Kutubxonadagi Kriptografiya kitobini eslang) ishlatilishi mumkin.",
              en:"This is typical of real CrackMes: not a single answer, but **understanding the algorithm**. Harder CrackMes may use XOR, lookup tables (chapter 10), or even cryptographic hash functions (recall the Cryptography book in the library)."},
      ]},
    { uz:"ARM assembleriga kirish", en:"Introduction to ARM assembly", subUz:"x86'dan boshqacha, lekin tanish dunyo", subEn:"A different but familiar world from x86",
      blocks:[
        {t:"p",uz:"Shu paytgacha faqat x86-64 ko'rdik, lekin dunyodagi ko'pgina qurilma — deyarli barcha telefonlar, ko'p noutbuklar va IoT qurilmalari — **ARM** protsessorida ishlaydi. Yaxshi xabar: **tushunchalar bir xil**, faqat sintaksis boshqacha.",
              en:"So far we've only seen x86-64, but most of the world's devices — nearly all phones, many laptops and IoT devices — run on **ARM** processors. The good news: **the concepts are identical**, only the syntax differs."},
        {t:"h",n:"§",uz:"x86 bilan taqqoslash",en:"Comparison with x86"},
        {t:"code",lang:"asm",cap:{uz:"Bir xil funksiya — x86-64 va ARM64",en:"The same function — x86-64 vs ARM64"},lines:[
          "; x86-64 (5-bobdagi kabi)      ; ARM64",
          "mov  eax, 42                   mov  w0, #42",
          "ret                             ret",
          "",
          "; if (a > b) return a; else return b;  (7-bobdagi max funksiyasi)",
          "cmp edi, esi     |  cmp  w0, w1",
          "jle .use_b       |  ble  .use_b",
          "mov eax, edi     |  mov  w0, w0   ; (allaqachon w0 da)"]},
        {t:"h",n:"§",uz:"Asosiy farqlar",en:"Key differences"},
        {t:"ul",uz:[
          "**Registr nomlari**: `rax`→`x0`, `rdi`→`x0` (birinchi argument HAM birinchi natija registri — ARM'da alohida emas), 32-bitli versiyalar `w0`-`w30`.",
          "**Argumentlar**: `x0`-`x7` da uzatiladi (System V'dagi `rdi,rsi,rdx...` o'rniga) — mantiq bir xil, nomlar boshqa.",
          "**Yuklash/saqlash arxitekturasi**: ARM'da `[xotira] + [xotira]` kabi ikki xotira operandi bo'lgan buyruq yo'q — avval xotiradan registrga yuklanadi (`ldr`), keyin amal bajariladi, keyin qaytarib saqlanadi (`str`).",
          "**Shartli buyruqlar**: `jle` o'rniga `ble`, lekin mantiq — bayroqqa asoslangan shartli sakrash — bir xil (7-bobni eslang)."],
          en:[
          "**Register names**: `rax`->`x0`, `rdi`->`x0` (the first argument register doubles as the first return register — not separate like on x86), 32-bit versions are `w0`-`w30`.",
          "**Arguments**: passed in `x0`-`x7` (instead of System V's `rdi,rsi,rdx...`) — same logic, different names.",
          "**Load/store architecture**: ARM has no instruction with two memory operands like `[mem] + [mem]` — you first load from memory into a register (`ldr`), operate, then store back (`str`).",
          "**Conditional instructions**: `ble` instead of `jle`, but the logic — a flag-based conditional jump — is identical (recall chapter 7)."]},
        {t:"note",uz:"Agar siz shu kitobdagi x86-64 bo'limlarini tushungan bo'lsangiz, ARM assemblerni o'rganish — yangi tilni emas, balki **yangi lug'atni** o'rganishga o'xshaydi. Tushunchalar (stek, chaqiruv kelishuvi, sikl, shart) barchasi bir xil qoladi.",
              en:"If you've understood the x86-64 sections of this book, learning ARM assembly is less like learning a new language and more like learning **new vocabulary**. The concepts (stack, calling convention, loop, condition) all stay the same."},
      ]},
    { uz:"RE'ga qarshi texnikalar", en:"Anti-reverse-engineering techniques", subUz:"Tahlilni qiyinlashtirish usullari — xabardorlik uchun", subEn:"Ways to hinder analysis — for awareness",
      blocks:[
        {t:"p",uz:"Ba'zi dasturlar (ayniqsa zararli dasturlar va litsenziya himoyasi) teskari injeneriyani ataylab qiyinlashtiradi. Bu texnikalarni bilish — ularga duch kelganda ularni **tanib olish** uchun, ulardan foydalanish uchun emas.",
              en:"Some programs (especially malware and license protection) deliberately make reverse engineering harder. Knowing these techniques is for **recognizing** them when you encounter them, not for using them yourself."},
        {t:"h",n:"§",uz:"Keng tarqalgan usullar",en:"Common techniques"},
        {t:"ul",uz:[
          "**Packing (qadoqlash)** — asl kod siqilgan/shifrlangan holda saqlanadi va faqat ishga tushganda xotirada \"ochiladi\" (unpack). Statik disassembler bunday faylda deyarli hech narsa ko'rmaydi.",
          "**Anti-debug** — dastur debugger ostida ishlayotganini tekshiradi (masalan Windows'da `IsDebuggerPresent`) va aniqlasa, xatti-harakatini o'zgartiradi yoki to'xtaydi.",
          "**Obfuskatsiya** — kodni ataylab chalkash qiladi: keraksiz sakrashlar, o'lik kod, murakkablashtirilgan boshqaruv oqimi (14-bobdagi optimallashtirishga o'xshaydi, lekin tezlik uchun emas, chalkashlik uchun).",
          "**String shifrlash** — matnlar (5-bobdagi kabi to'g'ridan-to'g'ri emas) faqat kerak bo'lganda deshifrlanadi, shuning uchun statik tahlilda \"Wrong\"/\"Correct\" kabi qulay ipuchlari topilmaydi."],
          en:[
          "**Packing** — the original code is stored compressed/encrypted and only \"unpacked\" in memory at runtime. A static disassembler sees almost nothing useful in such a file.",
          "**Anti-debugging** — the program checks whether it's running under a debugger (e.g. `IsDebuggerPresent` on Windows) and changes behaviour or halts if detected.",
          "**Obfuscation** — code is deliberately made confusing: junk jumps, dead code, convoluted control flow (similar to the optimization in chapter 14, but for confusion, not speed).",
          "**String encryption** — text (unlike the direct strings in chapter 5) is only decrypted when needed, so static analysis won't find convenient hints like \"Wrong\"/\"Correct\"."]},
        {t:"note",uz:"Bu texnikalar tahlilni **kechiktiradi**, lekin to'xtatmaydi — dinamik tahliz (dasturni ishga tushirib kuzatish) ko'pincha statik qadoqlashni chetlab o'tadi, chunki dastur o'zi kodni deshifrlab, xotirada ochiq holga keltirishi kerak. Shu daqiqada tahlilchi xotiradan \"ochilgan\" kodni ola oladi.",
              en:"These techniques **delay** analysis, but don't stop it — dynamic analysis (running and observing the program) often bypasses static packing, because the program itself must decrypt the code and expose it in memory to run. At that moment, the analyst can dump the \"unpacked\" code straight from memory."},
      ]},
    { uz:"Zararli dastur tahliliga kirish", en:"Introduction to malware analysis", subUz:"RE ko'nikmalari amalda qanday qo'llanadi", subEn:"How RE skills apply in practice",
      blocks:[
        {t:"p",uz:"Ushbu kitobda o'rgangan barcha narsa — bitta amaliy sohaga to'g'ridan-to'g'ri olib boradi: **zararli dastur tahlili (malware analysis)**. Bu bob — xavfsizlik jamoalari (Kali va Windows modullarida ko'rgan) buni qanday qo'llashiga qisqacha kirish.",
              en:"Everything you've learned in this book leads directly to one practical field: **malware analysis**. This chapter is a brief introduction to how security teams (as seen in the Kali and Windows modules) apply it."},
        {t:"h",n:"§",uz:"Ikki bosqichli yondashuv",en:"A two-stage approach"},
        {t:"p",uz:"Professional tahlil odatda xavfsiz, ajratilgan muhitda (**sandbox** — virtual mashina, tarmoqdan uzilgan) o'tkaziladi, va ikki bosqichdan iborat:",
              en:"Professional analysis usually happens in a safe, isolated environment (a **sandbox** — a virtual machine, disconnected from the network), and consists of two stages:"},
        {t:"ul",uz:[
          "**Statik tahlil** — faylni ishga tushirmasdan: satrlarni chiqarib olish (5-bob), import jadvalini ko'rish (12-bob — qanday tizim funksiyalarini chaqirishi kutilganini bildiradi), qadoqlanganmi tekshirish (o'tgan bob).",
          "**Dinamik tahlil** — nazorat ostida ishga tushirib: qaysi fayllarni yaratadi/o'zgartiradi, qaysi tarmoq manzillariga ulanadi, qaysi registry kalitlarini yozadi (Windows modulidagi Registry darsini eslang)."],
          en:[
          "**Static analysis** — without running the file: extracting strings (chapter 5), inspecting the import table (chapter 12 — hints at what system functions it expects to call), checking whether it's packed (previous chapter).",
          "**Dynamic analysis** — running it under control: which files it creates/modifies, which network addresses it connects to, which registry keys it writes (recall the Registry lesson in the Windows module)."]},
        {t:"h",n:"§",uz:"IOC — ko'rsatkichlar",en:"IOCs — indicators"},
        {t:"p",uz:"Tahlil natijasida **IOC (Indicators of Compromise)** — buzilish ko'rsatkichlari — hosil qilinadi: fayl xeshi (Kriptografiya kitobidagi SHA-256'ni eslang), tarmoq manzillari, registry kalitlari. Bular boshqa tizimlarda xuddi shu zararli dasturni **aniqlash** uchun tarqatiladi — bu Kali modulidagi Wireshark va IDS/IPS mavzulari bilan bevosita bog'liq.",
              en:"Analysis produces **IOCs (Indicators of Compromise)**: a file hash (recall SHA-256 from the Cryptography book), network addresses, registry keys. These are shared to **detect** the same malware on other systems — directly connected to the Wireshark and IDS/IPS topics in the Kali module."},
        {t:"note",uz:"Shu bilan ushbu kitobning aylanasi yopiladi: 1-bobda \"RE nima uchun kerak\" deb boshlangan edik, va endi ko'ryapmizki, siz o'rgangan har bir ko'nikma — assemblerni o'qish, funksiya va strukturalarni tanish, fayl formatini tushunish — real dunyoda xavfsizlik jamoalarining kundalik ishi.",
              en:"This closes the loop of the book: we began chapter 1 asking \"why learn RE\", and now we see that every skill you've learned — reading assembly, recognizing functions and structures, understanding file formats — is the daily work of real-world security teams."},
      ]},
    { uz:"Tezkor ma'lumotnoma", en:"Quick reference", subUz:"Barcha kitob bo'yicha bitta sahifada", subEn:"The whole book on one page",
      blocks:[
        {t:"p",uz:"Bu bob — kelajakda qaytib kelib foydalanish uchun **tezkor ma'lumotnoma**: shu kitobda ko'rgan eng muhim buyruqlar va naqshlarning qisqacha jamlanmasi.",
              en:"This chapter is a **quick reference** to return to later: a compact summary of the most important instructions and patterns from this book."},
        {t:"h",n:"§",uz:"Asosiy x86-64 buyruqlari",en:"Core x86-64 instructions"},
        {t:"code",lang:"text",cap:{uz:"Buyruqlar",en:"Instructions"},lines:[
          "mov/lea      ko'chirish / manzil olish        cmp/test    solishtirish",
          "add/sub/imul arifmetika                       je/jne/jg/jl shartli sakrash",
          "and/or/xor   bitli amallar                    jmp         shartsiz sakrash",
          "shl/shr/sar  siljitish (*2^n, /2^n)            call/ret    funksiya chaqiruv/qaytish",
          "push/pop     stekka joylash/olish              addss/movss kasr son amallari"]},
        {t:"h",n:"§",uz:"Naqshlar jadvali",en:"Pattern table"},
        {t:"code",lang:"text",cap:{uz:"Assemblerda ko'rsangiz — bu degani",en:"When you see this in assembly — it means"},lines:[
          "push rbp / mov rbp,rsp        funksiya prologi (6-bob)",
          "leave / pop rbp / ret         funksiya epilogi (6-bob)",
          "orqaga sakraydigan jmp        sikl (8-bob)",
          "jmp reg (masalan jmp rax)     switch jadvali yoki virtual chaqiruv (12,17-bob)",
          "[reg + reg*N]                 massiv indeksi (10-bob)",
          "[reg + sobit_son]             struktura maydoni (11-bob)",
          "xor reg, reg                  reg = 0",
          "xxx@plt yoki IAT orqali call   tashqi (dinamik) funksiya (18-bob)"]},
        {t:"h",n:"§",uz:"Chaqiruv kelishuvlari (System V x86-64)",en:"Calling conventions (System V x86-64)"},
        {t:"code",lang:"text",cap:{uz:"Argumentlar tartibi",en:"Argument order"},lines:[
          "Butun sonlar:  rdi, rsi, rdx, rcx, r8, r9, so'ng stek",
          "Kasr sonlar:   xmm0, xmm1, xmm2, ...",
          "Qaytish:       rax (butun son), xmm0 (kasr son)"]},
        {t:"note",uz:"Bu jadval — butun kitobning siqilgan xulosasi. Yangi ikkilik fayl ochganingizda, avval **matnlarni** (5-bob), so'ng **importlarni** (12-bob), keyin \"asosiy\" funksiyalarni shu naqshlar orqali o'qing. Muvaffaqiyat tilaymiz!",
              en:"This table is a compressed summary of the whole book. When you open a new binary, first look at the **strings** (chapter 5), then the **imports** (chapter 12), then read the \"interesting\" functions using these patterns. Good luck!"},
      ]},
    { uz:"Windows ichki tuzilishi: PEB, TEB va istisnolar", en:"Windows internals for RE: PEB, TEB & exceptions", subUz:"Real dunyo dasturlari va zararli kod tayangan tuzilmalar", subEn:"The structures real-world programs and malware rely on",
      blocks:[
        {t:"p",uz:"O'rta darajadagi RE mutaxassisi Windows dasturini tahlil qilganda ikkita yashirin tuzilmani albatta uchratadi: **PEB** (Process Environment Block) va **TEB** (Thread Environment Block). Ular — Windows yadrosi jarayon va oqim haqidagi ma'lumotni foydalanuvchi kodiga ochib beradigan xotira bloklari.",
              en:"A mid-level RE professional analyzing a Windows program will inevitably run into two hidden structures: the **PEB** (Process Environment Block) and **TEB** (Thread Environment Block). These are memory blocks through which the Windows kernel exposes process and thread information to user-mode code."},
        {t:"h",n:"§",uz:"PEB — nega muhim",en:"Why the PEB matters"},
        {t:"p",uz:"PEB'ga har doim **`gs:[0x60]`** orqali (x86-64'da) yoki `fs:[0x30]` orqali (32-bitli x86'da) yetish mumkin — bu segment registrlari orqali maxsus kirish. PEB ichida, boshqa narsalar qatorida, **`BeingDebugged`** bayrog'i bor. Aynan shu bayroq — eng keng tarqalgan anti-debug texnikasining asosi (24-bobni eslang): dastur PEB'ni to'g'ridan-to'g'ri o'qib, `IsDebuggerPresent()` API funksiyasini chaqirmasdan ham debugger borligini bila oladi, va bu ko'plab oddiy anti-debug tekshiruvlarini chetlab o'tishni qiyinlashtiradi.",
              en:"The PEB can always be reached via **`gs:[0x60]`** (on x86-64) or `fs:[0x30]` (on 32-bit x86) — special access through segment registers. Among other things, the PEB contains the **`BeingDebugged`** flag. This flag is the basis of the most common anti-debug technique (recall chapter 24): a program can read the PEB directly, without even calling the `IsDebuggerPresent()` API function, making some naive anti-debug bypasses fail."},
        {t:"h",n:"§",uz:"Istisno boshqaruvi: SEH va VEH",en:"Exception handling: SEH and VEH"},
        {t:"p",uz:"**SEH** (Structured Exception Handling) — Windows'ning istisno (masalan nolga bo'lish, xotiraga noto'g'ri kirish) yuzaga kelganda kodni ishga tushiruvchi mexanizmi. Nega bu RE uchun muhim? Chunki ko'plab obfuskatsiya va anti-debug texnikalari **ataylab istisno yuzaga keltiradi**, so'ng maxsus istisno ishlovchisida haqiqiy dastur mantig'ini bajaradi. Oddiy statik disassembler bunday kodni to'g'ri oqim sifatida ko'rsatolmaydi — chunki haqiqiy boshqaruv o'tishi `call`/`jmp` orqali emas, balki **protsessor istisnosi orqali** amalga oshadi.",
              en:"**SEH** (Structured Exception Handling) is Windows's mechanism for running code when an exception (e.g. division by zero, invalid memory access) occurs. Why does this matter for RE? Because many obfuscation and anti-debug techniques **deliberately trigger an exception**, then run the real program logic inside a custom exception handler. A plain static disassembler can't show such code as normal flow — because the actual control transfer happens **via a processor exception**, not a `call`/`jmp`."},
        {t:"note",warn:true,uz:"Amaliy maslahat: agar dastur `int3` (nuqta uzilish buyrug'i) yoki noto'g'ri xotira manzilini **ataylab** ishlatayotganini ko'rsangiz — bu ko'pincha SEH asosidagi obfuskatsiya belgisi. Zamonaviy debuggerlar (x64dbg) istisnolarni kuzatish va ularni \"o'tkazib yuborish\" (pass to application) imkoniyatiga ega — bu bunday kodni tahlil qilishning kaliti.",
              en:"Practical tip: if you see a program **deliberately** triggering `int3` (a breakpoint instruction) or an invalid memory address — this is often a sign of SEH-based obfuscation. Modern debuggers (x64dbg) can track exceptions and \"pass them to the application\" — this is the key to analyzing such code."},
      ]},
    { uz:"ARM chuqurroq: Thumb va shartli bajarish", en:"ARM in depth: Thumb & conditional execution", subUz:"Mobil va IoT dunyosining haqiqiy assembleri", subEn:"The real assembly of the mobile and IoT world",
      blocks:[
        {t:"p",uz:"23-bobda ARM'ning asosiy tushunchalarini ko'rdik. Endi mobil dasturlar va IoT proshivkalarini tahlil qilishda tez-tez uchraydigan ikkita xususiyatni chuqurroq ko'ramiz: **Thumb rejimi** va **shartli bajarish**.",
              en:"In chapter 23 we saw ARM's basic concepts. Now let's go deeper into two features you'll frequently encounter when analyzing mobile apps and IoT firmware: **Thumb mode** and **conditional execution**."},
        {t:"h",n:"§",uz:"Thumb — ixcham kodlash",en:"Thumb — compact encoding"},
        {t:"p",uz:"Oddiy ARM buyruqlari 4 baytli, lekin **Thumb** rejimida ko'p buyruqlar **2 baytga** siqiladi — dastur hajmini kamaytiradi, bu xotirasi cheklangan qurilmalar (eski telefonlar, mikrokontrollerlar) uchun muhim edi. Zamonaviy **Thumb-2** ikkalasini aralashtiradi: ko'p buyruq 2 bayt, ba'zilari 4 bayt. RE'da muhim jihat: **bitta protsessor ikki xil rejimda ishlashi mumkin**, va disassembler qaysi rejimda ekanini bilishi kerak — aks holda baytlarni butunlay noto'g'ri talqin qiladi.",
              en:"Regular ARM instructions are 4 bytes, but in **Thumb** mode many instructions compress to **2 bytes** — reducing program size, which mattered on memory-constrained devices (older phones, microcontrollers). Modern **Thumb-2** mixes both: most instructions are 2 bytes, some are 4. What matters for RE: **the same processor can run in two different modes**, and the disassembler must know which one — otherwise it will misinterpret the bytes entirely."},
        {t:"h",n:"§",uz:"Shartli bajarish — ARM'ning noyob xususiyati",en:"Conditional execution — ARM's unique feature"},
        {t:"p",uz:"x86'da deyarli har bir buyruq shartsiz bajariladi, faqat `jcc` buyruqlari shartli. **ARM**'da esa (klassik rejimda) **har qanday buyruq** shart bilan bajarilishi mumkin — buyruq nomiga qo'shimcha qo'shiladi, masalan `moveq` (\"agar teng bo'lsa ko'chir\"). Bu 7-bobdagi if/else naqshini butunlay boshqacha ko'rinishda beradi: ba'zida alohida `cmp`+`jump` o'rniga, bir nechta shartli buyruq **ketma-ket, sakrashsiz** yoziladi.",
              en:"On x86, almost every instruction executes unconditionally; only `jcc` instructions are conditional. On **ARM** (in classic mode), **any instruction** can be made conditional — a suffix is added to the instruction name, e.g. `moveq` (\"move if equal\"). This makes the if/else pattern from chapter 7 look completely different: sometimes, instead of a separate `cmp`+jump, several conditional instructions appear **in sequence, without a jump at all**."},
        {t:"code",lang:"asm",cap:{uz:"Shartli bajarish misoli — ARM32",en:"Conditional execution example — ARM32"},lines:[
          "cmp   r0, r1        ; r0 ni r1 bilan solishtir",
          "moveq r2, #1        ; agar TENG bo'lsa: r2 = 1",
          "movne r2, #0        ; agar TENG BO'LMASA: r2 = 0",
          "; hech qanday jump yo'q — ikkala buyruq ham \"bajariladi\",",
          "; lekin faqat biri haqiqiy ta'sir qiladi"]},
        {t:"note",uz:"Bu naqsh **sakrash-siz shart** deb ataladi (branchless code) — zamonaviy x86 kompilyatorlari ham `cmov` (conditional move) orqali shunga o'xshash narsa qiladi, chunki sakrash bashorat xatosi (branch misprediction) protsessorni sekinlashtiradi. ARM'da bu — arxitekturaning tabiiy xususiyati.",
              en:"This pattern is called branchless code — modern x86 compilers do something similar via `cmov` (conditional move), because branch misprediction slows the processor down. On ARM, this is a native feature of the architecture."},
      ]},
    { uz:"Obfuskatsiyani chuqur tahlil qilish", en:"Deep-diving into obfuscation", subUz:"Kodni ataylab chalkash qiladigan texnikalar", subEn:"Techniques that deliberately confuse code",
      blocks:[
        {t:"p",uz:"24-bobda obfuskatsiyaga umumiy kirish qildik. Endi o'rta darajadagi mutaxassis duch keladigan uchta jiddiy texnikani chuqurroq ko'ramiz: **boshqaruv oqimini tekislash**, **shaffof bo'lmagan shartlar** va **virtualizatsiyaga asoslangan himoya**.",
              en:"Chapter 24 gave a general introduction to obfuscation. Now let's go deeper into three serious techniques a mid-level professional will encounter: **control-flow flattening**, **opaque predicates**, and **virtualization-based protection**."},
        {t:"h",n:"§",uz:"Boshqaruv oqimini tekislash",en:"Control-flow flattening"},
        {t:"p",uz:"Oddiy funksiyada bloklar mantiqiy ketma-ketlikda joylashadi (7-8-boblardagi kabi). **Tekislashda** esa barcha bloklar bitta katta `switch` (12-bobdagi jump table'ni eslang) atrofida qayta tuziladi: markaziy sikl har safar \"keyingi holat raqami\"ni o'qib, mos blokka sakraydi. Natijada dasturning haqiqiy mantiqiy tuzilishi (qaysi blok qaysi blokdan keyin kelishi) grafik ko'rinishda butunlay yo'qoladi — hamma narsa bir xil darajadagi \"holatlar\" bo'lib ko'rinadi.",
              en:"In a normal function, blocks follow a logical sequence (as in chapters 7-8). With **flattening**, all blocks are restructured around one giant `switch` (recall the jump table from chapter 12): a central loop reads a \"next state number\" each time and jumps to the matching block. As a result, the program's real logical structure (which block follows which) disappears entirely in a graph view — everything looks like flat, equal-level \"states\"."},
        {t:"h",n:"§",uz:"Shaffof bo'lmagan shartlar (opaque predicates)",en:"Opaque predicates"},
        {t:"p",uz:"Bu — natijasi **har doim oldindan ma'lum** bo'lgan, lekin tahlilchiga (yoki avtomatik vositaga) noma'lumdek ko'rinadigan shart. Masalan `if ((x*x) % 4 != 2)` — bu ifoda **har qanday butun x uchun har doim rost**, chunki kvadratning 4 ga qolganidan bo'linishi hech qachon 2 bo'lmaydi. Kompilyator/obfuskator buni haqiqiy shartga o'xshatib qo'yadi, lekin amalda hech qachon boshqa tarmoqqa o'tmaydi — shu \"o'lik\" tarmoqqa esa chalg'ituvchi, ma'nosiz kod joylashtiriladi.",
              en:"This is a condition whose result is **always known in advance**, but appears unknown to an analyst (or an automated tool). For example `if ((x*x) % 4 != 2)` — this expression is **always true for any integer x**, because a square's remainder mod 4 is never 2. The compiler/obfuscator disguises this as a real condition, but it never actually takes the other branch — and that \"dead\" branch is filled with confusing, meaningless code."},
        {t:"h",n:"§",uz:"Virtualizatsiyaga asoslangan himoya",en:"Virtualization-based protection"},
        {t:"p",uz:"Eng kuchli tijorat himoyalari (masalan VMProtect kabi mashhur vositalar) butunlay boshqa yondashuv qo'llaydi: asl x86 kodni **butunlay o'zgacha, maxsus \"bayt-kod\"**ga aylantiradi, va dastur ichiga shu bayt-kodni bajaradigan **kichik virtual mashina (VM)** joylashtiradi. Natijada disassembler asl mantiqni umuman ko'rmaydi — faqat VM'ning tashqi \"tarjimon\" tsiklini ko'radi. Buni yechish uchun avval **VM'ning o'zini** (uning \"buyruqlar to'plami\"ni) teskari muhandislik qilish kerak — bu o'z-o'zidan alohida, murakkab RE loyihasi.",
              en:"The strongest commercial protections (e.g. popular tools like VMProtect) take a completely different approach: they transform the original x86 code into an entirely custom \"bytecode\", and embed a small **virtual machine (VM)** inside the program that executes that bytecode. As a result, a disassembler never sees the original logic at all — only the VM's outer \"interpreter\" loop. Defeating this requires first reverse-engineering **the VM itself** (its \"instruction set\") — a separate, complex RE project in its own right."},
        {t:"note",uz:"Bu uchta texnika — RE murakkablik zinapoyasining yuqori pog'onalari. Boshlang'ich darajada ularni **tanib olish** yetarli; ularni to'liq yengish esa yillar davomida tajriba to'plashni talab qiladigan mahorat.",
              en:"These three techniques sit at the top of the RE difficulty ladder. At a beginner level, **recognizing** them is enough; fully defeating them is a skill that takes years of accumulated experience."},
      ]},
    { uz:"Qadoqlangan dasturlarni ochish metodologiyasi", en:"Unpacking methodology", subUz:"Packerdan asl kodni qanday olib chiqish", subEn:"How to extract the original code from a packer",
      blocks:[
        {t:"p",uz:"24-bobda packing tushunchasini ko'rgan edik: asl kod siqilgan/shifrlangan holda saqlanadi va faqat ishga tushganda xotirada \"ochiladi\". Bu bob — professional darajadagi **ochish (unpacking) metodologiyasi**ni bosqichma-bosqich ko'rsatadi.",
              en:"In chapter 24 we saw the concept of packing: the original code is stored compressed/encrypted and only \"unpacked\" in memory at runtime. This chapter walks through the professional-level **unpacking methodology** step by step."},
        {t:"h",n:"§",uz:"Asosiy g'oya: OEP'ni topish",en:"The core idea: finding the OEP"},
        {t:"p",uz:"Har qanday packer, qanchalik murakkab bo'lmasin, oxir-oqibat asl kodga sakrashi **shart** — aks holda dastur umuman ishlamaydi. Bu sakrash nuqtasi **OEP (Original Entry Point)** deb ataladi. Vazifa — dasturni ishga tushirib, packer o'z ishini tugatib, aynan OEP'ga sakrayotgan daqiqani **ushlab qolish**.",
              en:"No matter how complex, every packer **must** eventually jump to the original code — otherwise the program wouldn't run at all. This jump point is called the **OEP (Original Entry Point)**. The task is to run the program, let the packer finish its work, and **catch the exact moment** it jumps to the OEP."},
        {t:"pipe",steps:[
          {ic:"lock",tUz:"Qadoqlangan fayl",tEn:"Packed file",sUz:"kod shifrlangan",sEn:"code encrypted"},
          {ic:"cpu",tUz:"Ishga tushirish",tEn:"Run under debugger",sUz:"packer ochadi",sEn:"packer unpacks"},
          {ic:"eye",tUz:"OEP'ni ushlash",tEn:"Catch the OEP",sUz:"sakrash daqiqasida",sEn:"at the jump moment"},
          {ic:"database",tUz:"Xotiradan dump",tEn:"Dump from memory",sUz:"asl kod tiklandi",sEn:"original code recovered"}]},
        {t:"h",n:"§",uz:"Amaliy usullar",en:"Practical techniques"},
        {t:"ul",uz:[
          "**Yozish keyin bajarish (write-then-execute)**: ko'p packer avval xotira sahifasiga yozadi (deshifrlaydi), so'ng o'sha sahifani bajaradi. Debuggerda \"xotira bajarilishi\" bo'yicha uzilish nuqtasi qo'yib, aynan shu o'tish daqiqasini ushlash mumkin.",
          "**Stek bo'yicha kuzatish**: ko'p packer OEP'ga `jmp`/`ret` orqali o'tadi; stek tepasidan kutilmagan manzilni kuzatish (\"popad\" texnikasi) OEP'ni tez topishga yordam beradi.",
          "**Xotiradan dump qilish va import jadvalini tiklash**: OEP topilgach, jarayon xotirasi diskka \"dump\" qilinadi (masalan **Scylla** kabi vositalar bilan), so'ng import jadvali (12-bobni eslang) qayta tiklanadi — chunki packer ko'pincha uni ham xiralashtirib qo'yadi."],
          en:[
          "**Write-then-execute**: many packers first write to a memory page (decrypting it), then execute that same page. Setting a \"memory execute\" breakpoint in a debugger can catch exactly this transition.",
          "**Stack tracing**: many packers reach the OEP via `jmp`/`ret`; watching the stack top for an unexpected address (the \"popad\" technique) helps find the OEP quickly.",
          "**Memory dumping and import table reconstruction**: once the OEP is found, process memory is \"dumped\" to disk (e.g. with a tool like **Scylla**), then the import table (recall chapter 12) is rebuilt — since packers usually obscure that too."]},
        {t:"note",warn:true,uz:"Zamonaviy tijorat packer'lari (VMProtect, Themida) bu oddiy usullarga qarshi ataylab himoyalangan — ular anti-debug (24-bob), anti-dump va hatto virtualizatsiya (o'tgan bob) bilan birlashtirilgan. Bunday hollarda ochish — kunlar, hatto haftalar davom etadigan professional loyihaga aylanadi.",
              en:"Modern commercial packers (VMProtect, Themida) are deliberately hardened against these simple methods — combining anti-debug (chapter 24), anti-dump, and even virtualization (previous chapter). In such cases, unpacking becomes a professional project spanning days or even weeks."},
      ]},
    { uz:"Shellcode tahlili asoslari", en:"Shellcode analysis basics", subUz:"Fayl emas, xotiradagi xom kod", subEn:"Raw code in memory, not a file",
      blocks:[
        {t:"p",uz:"**Shellcode** — mustaqil, kichik mashina kodi bo'lagi bo'lib, u odatiy bajariluvchi fayl (12-bobdagi PE/ELF) shaklida emas, balki **xom baytlar** ko'rinishida keladi — ko'pincha ekspluatatsiya (exploit) orqali xotiraga in'ektsiya qilinadi. Zararli dastur tahlilida (25-bob) shellcode'ni tanib olish va tahlil qilish — muhim ko'nikma.",
              en:"**Shellcode** is a standalone, small piece of machine code that arrives not as a normal executable file (PE/ELF from chapter 12), but as **raw bytes** — often injected into memory via an exploit. Recognizing and analyzing shellcode is an important skill in malware analysis (chapter 25)."},
        {t:"h",n:"§",uz:"Shellcode oddiy koddan nimasi bilan farq qiladi",en:"How shellcode differs from ordinary code"},
        {t:"ul",uz:[
          "**Manzilga bog'liq emas (position-independent)**: shellcode xotiraning istalgan qismida ishlashi kerak, chunki uning aniq manzili oldindan ma'lum emas — shuning uchun sobit manzillar o'rniga nisbiy sakrashlar va **`call`/`pop`** hiylasi (o'z manzilini o'zi topish) ishlatiladi.",
          "**Nol baytsiz (null-free)**: ko'p ekspluatatsiya usullari (masalan satr sifatida uzatiladigan bufer to'lib ketishi, 14-bobni eslang) `\\0` baytida to'xtaydi, shuning uchun shellcode odatda nol baytdan qochadigan buyruqlar bilan yoziladi.",
          "**API'larni qo'lda topish**: import jadvali (12-bob) yo'q, shuning uchun shellcode kerakli Windows funksiyalarini (masalan `LoadLibrary`) PEB orqali (o'tgan boblardan bir nechtasini eslang) qo'lda qidiradi."],
          en:[
          "**Position-independent**: shellcode must work anywhere in memory, since its exact address isn't known in advance — so instead of fixed addresses, relative jumps and the **`call`/`pop`** trick (finding its own address) are used.",
          "**Null-free**: many exploitation methods (e.g. a string-based buffer overflow, recall chapter 14) stop at a `\\0` byte, so shellcode is typically written with instructions that avoid null bytes.",
          "**Manual API resolution**: there's no import table (chapter 12), so shellcode manually looks up needed Windows functions (e.g. `LoadLibrary`) via the PEB (recall a couple of chapters back)."]},
        {t:"h",n:"§",uz:"Amaliy tahlil",en:"Practical analysis"},
        {t:"p",uz:"Shellcode'ni tahlil qilishning eng oson yo'li — uni kichik \"o'ram\" (stub) dasturga joylab, debugger ostida ishga tushirish (masalan bo'sh xotira ajratib, shellcode baytlarini nusxalab, o'sha yerga sakrab). Bu — statik tahlil imkonsiz bo'lganda (position-independent kod ko'p tarmoqlanish va hisoblangan manzillarga ega bo'lgani uchun) **dinamik tahlil**ning (1-bobni eslang) eng aniq namunasi.",
              en:"The easiest way to analyze shellcode is to embed it in a small \"stub\" program and run it under a debugger (e.g. allocate empty memory, copy the shellcode bytes in, and jump there). This is the clearest example of **dynamic analysis** (recall chapter 1) when static analysis is impractical — since position-independent code has lots of branching and computed addresses."},
      ]},
    { uz:"Java baytkodi va JVM reversing", en:"Java bytecode & JVM reversing", subUz:"Yig'ma til emas, stack-mashina", subEn:"Not assembly — a stack machine",
      blocks:[
        {t:"p",uz:"Shu paytgacha ko'rgan hammasi — **registrlarga** asoslangan protsessorlar (x86, ARM) uchun edi. Java esa butunlay boshqa modelda ishlaydi: kod **JVM baytkodiga** kompilyatsiya qilinadi, va JVM (Java Virtual Machine) — registrlar o'rniga **stek**ka asoslangan virtual protsessor.",
              en:"Everything we've seen so far was for **register-based** processors (x86, ARM). Java works on a completely different model: code compiles to **JVM bytecode**, and the JVM (Java Virtual Machine) is a **stack-based** virtual processor instead of a register-based one."},
        {t:"h",n:"§",uz:"Stack-mashina qanday ishlaydi",en:"How a stack machine works"},
        {t:"code",lang:"text",cap:{uz:"a + b baytkodda (JVM)",en:"a + b in bytecode (JVM)"},lines:[
          "iload_0     ; 0-lokal o'zgaruvchini (a) stekka joyla",
          "iload_1     ; 1-lokal o'zgaruvchini (b) stekka joyla",
          "iadd        ; stekdan ikkitasini olib, qo'shib, natijani stekka qaytar",
          "istore_2    ; natijani 2-lokal o'zgaruvchiga saqla"]},
        {t:"p",uz:"5-bobdagi `mov`/`add` o'rniga bu yerda `iload`/`iadd` kabi buyruqlar bor — hech qanday `rax` yoki `rdi` yo'q, faqat **operand stek** va **lokal o'zgaruvchilar massivi**. Bu model x86'dan ko'ra tushunarliroq ko'rinadi, chunki u yuqori darajali tilga yaqinroq.",
              en:"Instead of `mov`/`add` from chapter 5, here we have instructions like `iload`/`iadd` — no `rax` or `rdi`, only an **operand stack** and a **local variable array**. This model actually looks more understandable than x86, because it's closer to the high-level language."},
        {t:"h",n:"§",uz:"Nega dekompilyatsiya osonroq",en:"Why decompilation is easier"},
        {t:"p",uz:".class fayllar ko'p **metama'lumot**ni (sinf nomlari, metod imzolari, hatto o'zgaruvchi nomlari — agar debug ma'lumoti olib tashlanmagan bo'lsa) saqlab qoladi. Shuning uchun Java dekompilyatorlari (masalan **JADX**, **JD-GUI**) ko'pincha C/C++'dan ko'ra ancha aniqroq, deyarli asl manba kodga yaqin natija beradi. Bu — Android APK fayllarini tahlil qilishning asosiy usuli, chunki Android ilovalari katta qismi Java/Kotlin'da yozilib, DEX baytkodiga (JVM'ga o'xshash, lekin Android uchun moslashtirilgan) kompilyatsiya qilinadi.",
              en:".class files retain a lot of **metadata** (class names, method signatures, even variable names if debug info wasn't stripped). This is why Java decompilers (e.g. **JADX**, **JD-GUI**) often produce far more accurate results than for C/C++, close to the original source. This is the primary method for analyzing Android APK files, since most Android apps are written in Java/Kotlin and compiled to DEX bytecode (similar to JVM, but adapted for Android)."},
        {t:"note",uz:"Amaliy maslahat: Android zararli dastur tahlilida birinchi qadam deyarli har doim — APK'ni JADX orqali ochib, `AndroidManifest.xml` (qanday ruxsatlar so'ralgani) va asosiy `Activity` sinflarini o'qish.",
              en:"Practical tip: in Android malware analysis, the first step is almost always opening the APK with JADX and reading the `AndroidManifest.xml` (what permissions are requested) and the main `Activity` classes."},
      ]},
    { uz:".NET va CLR reversing", en:".NET and CLR reversing", subUz:"Microsoftning bosh-kodli virtual mashinasi", subEn:"Microsoft's own bytecode virtual machine",
      blocks:[
        {t:"p",uz:"Microsoft'ning **.NET** platformasi Java'ga juda o'xshash g'oyaga tayanadi: C#/VB.NET kodi **CIL** (Common Intermediate Language, ba'zan MSIL deb ham ataladi) bosh-kodiga kompilyatsiya qilinadi, va uni **CLR** (Common Language Runtime) bajaradi — bu ham stek-asosli virtual mashina.",
              en:"Microsoft's **.NET** platform relies on an idea very similar to Java's: C#/VB.NET code compiles to **CIL** (Common Intermediate Language, sometimes called MSIL) bytecode, executed by the **CLR** (Common Language Runtime) — also a stack-based virtual machine."},
        {t:"h",n:"§",uz:"Nega .NET RE'da alohida ahamiyatga ega",en:"Why .NET has special significance in RE"},
        {t:"p",uz:".class fayllar kabi, .NET assembly (.exe/.dll) fayllari ham boy metama'lumotga ega — bu **dekompilyatsiyani ajoyib darajada aniq** qiladi. **dnSpy** yoki **ILSpy** kabi vositalar bilan ko'p .NET dasturini deyarli **to'liq, o'qiladigan C# kodiga** aylantirish mumkin — bu 17-bobdagi optimallashtirilgan C/C++ kodini o'qishdan tubdan osonroq.",
              en:"Like .class files, .NET assembly (.exe/.dll) files carry rich metadata — making decompilation **remarkably accurate**. With tools like **dnSpy** or **ILSpy**, much .NET software can be turned into nearly **complete, readable C# code** — vastly easier than reading optimized C/C++ (recall chapter 17)."},
        {t:"h",n:"§",uz:"Amaliy oqim",en:"Practical workflow"},
        {t:"pipe",steps:[
          {ic:"lock",tUz:".NET fayli",tEn:".NET file",sUz:".exe / .dll",sEn:".exe / .dll"},
          {ic:"tool",tUz:"dnSpy/ILSpy",tEn:"dnSpy/ILSpy",sUz:"CIL -> C#",sEn:"CIL -> C#"},
          {ic:"code",tUz:"O'qiladigan kod",tEn:"Readable code",sUz:"deyarli asl manba",sEn:"near-original source"},
          {ic:"code",tUz:"Tahrirlash",tEn:"Edit",sUz:"dnSpy'da to'g'ridan-to'g'ri",sEn:"directly in dnSpy"}]},
        {t:"p",uz:"**dnSpy**'ning noyob kuchi — u faqat o'qish uchun emas: siz dekompilyatsiya qilingan C# kodini **to'g'ridan-to'g'ri tahrirlashingiz** va fayl ichiga qayta kompilyatsiya qilishingiz mumkin, hatto to'liq manba kodsiz ham. Bu — .NET dasturlarini \"patchlash\" (masalan litsenziya tekshiruvini o'qish uchun, 13-bobdagi CrackMe mantig'ini eslang) uchun juda kuchli imkoniyat.",
              en:"dnSpy's unique power isn't just reading: you can **directly edit** the decompiled C# code and recompile it back into the file, even without the full source. This makes it a very powerful tool for \"patching\" .NET programs (e.g. for reading a license check, recall the CrackMe logic from chapter 13)."},
        {t:"note",uz:"Himoya vositalari (masalan **ConfuseEx**, **.NET Reactor**) .NET assembly'larni obfuskatsiya qiladi — nomlarni chalkashtiradi, oqimni tekislaydi (o'tgan boblarni eslang). Lekin metama'lumotga bog'liqlik tufayli, .NET obfuskatsiyasini kompilyatsiyadan oldingi C/C++ obfuskatsiyasiga qaraganda ko'pincha yengilroq yechish mumkin.",
              en:"Protection tools (e.g. **ConfuserEx**, **.NET Reactor**) obfuscate .NET assemblies — scrambling names, flattening flow (recall previous chapters). But due to the metadata dependency, .NET obfuscation is often somewhat easier to defeat than pre-compilation C/C++ obfuscation."},
      ]},
    { uz:"Dinamik instrumentatsiya (DBI)", en:"Dynamic Binary Instrumentation (DBI)", subUz:"Debuggerdan kuchliroq: dasturga kod in'ektsiya qilish", subEn:"Beyond a debugger: injecting code into a program",
      blocks:[
        {t:"p",uz:"7-bobda debuggerlarni ko'rgan edik — ular dasturni **to'xtatib**, holatini ko'rish imkonini beradi. **DBI (Dynamic Binary Instrumentation)** bir qadam oldinga o'tadi: u dasturga **har bir buyruq yoki funksiya chaqiruvi oldidan/keyin** o'zboshimcha kodni **avtomatik** ishga tushirish imkonini beradi — dastur million marta funksiya chaqirsa ham, qo'lda million marta nuqta qo'yib chiqmasdan.",
              en:"In chapter 7 we saw debuggers — they let you **pause** a program and inspect its state. **DBI (Dynamic Binary Instrumentation)** goes a step further: it lets you **automatically** run arbitrary code before/after every instruction or function call — even if the program calls a function a million times, without manually setting a million breakpoints."},
        {t:"h",n:"§",uz:"Asosiy vositalar",en:"Core tools"},
        {t:"ul",uz:[
          "**Intel Pin** va **DynamoRIO** — professional DBI freymvorklari (C/C++ da), ko'pincha akademik tadqiqot va murakkab avtomatlashtirilgan tahlilda ishlatiladi.",
          "**Frida** — zamonaviy, skript orqali boshqariladigan DBI vositasi. JavaScript orqali istalgan funksiyani \"ushlab qolish\" (hook), argumentlarini ko'rish yoki o'zgartirish, hatto natijani almashtirish mumkin — bu mobil ilova (Android/iOS) tahlilida ayniqsa mashhur."],
          en:[
          "**Intel Pin** and **DynamoRIO** — professional DBI frameworks (in C/C++), often used in academic research and complex automated analysis.",
          "**Frida** — a modern, script-driven DBI tool. Via JavaScript you can \"hook\" any function, view or modify its arguments, even replace the return value — especially popular for mobile app (Android/iOS) analysis."]},
        {t:"code",lang:"text",cap:{uz:"Frida hook misoli — g'oya",en:"Frida hook example — the idea"},lines:[
          "Interceptor.attach(Module.findExportByName(null, \"strcmp\"), {",
          "  onEnter: function(args) {",
          "    console.log(\"strcmp chaqirildi:\", args[0].readUtf8String());",
          "  }",
          "});",
          "; -- har safar dastur strcmp() ni chaqirganda, argumentlar avtomatik chiqadi"]},
        {t:"note",uz:"DBI'ning eng katta afzalligi — **avtomatlashtirish**: 21-22-boblardagi CrackMe'da qo'lda qadam-baqadam ko'rgan narsani, DBI orqali \"dastur har safar `check()` funksiyasini chaqirganda, argumentini logga yoz\" tarzida bir marta yozib, keyin avtomatik kuzatish mumkin. Bu — 27-bobdagi Python avtomatlashtirish g'oyasi bilan bevosita bog'liq.",
              en:"DBI's biggest advantage is **automation**: what we traced step by step by hand in the CrackMes of chapters 21-22 can instead be written once as \"every time the program calls `check()`, log its argument\" via DBI, then observed automatically. This connects directly to the Python automation idea in the next chapter."},
      ]},
    { uz:"Avtomatlashtirish: Python va disassembler kutubxonalari", en:"Automation: Python & disassembly libraries", subUz:"O'z RE vositalaringizni yozish", subEn:"Writing your own RE tools",
      blocks:[
        {t:"p",uz:"O'rta darajadagi RE mutaxassisini boshlang'ichdan ajratib turadigan asosiy ko'nikmalardan biri — **avtomatlashtirish**. Qo'lda yuzlab funksiyani ko'rib chiqish o'rniga, tez-tez uchraydigan naqshlarni (bu kitobda ko'rgan barcha naqshlar!) dastur bilan avtomatik topish mumkin.",
              en:"One of the key skills that separates a mid-level RE professional from a beginner is **automation**. Instead of manually reviewing hundreds of functions, you can write a program to automatically find recurring patterns — all the patterns we've seen throughout this book!"},
        {t:"h",n:"§",uz:"Asosiy Python kutubxonalari",en:"Core Python libraries"},
        {t:"ul",uz:[
          "**Capstone** — disassembly kutubxonasi: baytlarni assembler matniga (bu kitobda ko'rgan barcha misollar kabi) dasturiy tarzda aylantiradi.",
          "**Keystone** — Capstone'ning \"aksi\": assembler matnidan mashina kodi (baytlar) yaratadi.",
          "**Unicorn** — protsessor **emulyatori**: haqiqiy operatsion tizimsiz, alohida buyruqlarni yoki butun funksiyani \"soxta\" xotirada bajarib, natijasini ko'rish imkonini beradi.",
          "**angr** — ramziy bajarish (symbolic execution) freymvorki: \"qaysi kirish qiymatida bu funksiya X natijani qaytaradi?\" kabi savollarga avtomatik javob topadi — bu 21-22-boblardagi CrackMe yechish jarayonini **to'liq avtomatlashtirishi** mumkin."],
          en:[
          "**Capstone** — a disassembly library: programmatically turns bytes into assembly text (like every example we've seen in this book).",
          "**Keystone** — Capstone's \"opposite\": generates machine code (bytes) from assembly text.",
          "**Unicorn** — a processor **emulator**: lets you execute individual instructions or a whole function in \"fake\" memory, without a real OS, and observe the result.",
          "**angr** — a symbolic execution framework: automatically answers questions like \"for which input does this function return X?\" — this can **fully automate** the CrackMe-solving process from chapters 21-22."]},
        {t:"code",lang:"text",cap:{uz:"Capstone bilan avtomatik disassembly — g'oya",en:"Automatic disassembly with Capstone — the idea"},lines:[
          "from capstone import *",
          "md = Cs(CS_ARCH_X86, CS_MODE_64)",
          "for insn in md.disasm(code_bytes, 0x1000):",
          "    print(insn.mnemonic, insn.op_str)",
          "; -- 5-bobdagi 'mov eax,42 / ret' kabi baytlarni avtomatik o'qiydi"]},
        {t:"note",uz:"Bu vositalar birgalikda ishlaydi: **Capstone** bilan o'qiysiz, **angr** bilan \"qaysi javob to'g'ri?\" savolini avtomatik hal qilasiz, **Unicorn** bilan taxminni tekshirasiz. Bu — 26-bobdagi naqshlar jadvalini qo'lda emas, **kod orqali** qidirishning eng samarali yo'li.",
              en:"These tools work together: read with **Capstone**, automatically solve \"which answer is correct?\" with **angr**, verify the guess with **Unicorn**. This is the most efficient way to search for the pattern table from chapter 26 — **through code**, not by hand."},
      ]},
    { uz:"Binar taqqoslash va patch tahlili", en:"Binary diffing & patch analysis", subUz:"Ikki versiya orasidagi farqni topish", subEn:"Finding the difference between two versions",
      blocks:[
        {t:"p",uz:"Real dunyoda xavfsizlik tadqiqotchilarining muhim vazifasi — kompaniya xavfsizlik yamog'ini (patch) chiqarganda, **aynan nima o'zgarganini** topish. Bu ma'lumot ko'pincha yamalgan zaiflikning o'zini oshkor qiladi — bu **1-day** zaifliklarni topishning asosiy usuli.",
              en:"A key task for real-world security researchers is finding **exactly what changed** when a company releases a security patch. This information often reveals the patched vulnerability itself — this is the primary method for finding **1-day** vulnerabilities."},
        {t:"h",n:"§",uz:"Binar taqqoslash (bindiff) g'oyasi",en:"The binary diffing idea"},
        {t:"p",uz:"Ikki fayl (yamoqdan oldin/keyin) baytma-bayt solishtirish deyarli foydasiz — chunki kompilyator bitta funksiya o'zgarganda ham, ko'p manzillarni qayta hisoblaydi, va butun fayl \"boshqacha\" ko'rinadi. Professional vositalar (masalan **BinDiff**, **Diaphora**) buning o'rniga funksiyalarni **strukturaviy jihatdan** (funksiyalar orasidagi chaqiruv grafigi, har funksiyaning ichki boshqaruv oqimi grafigi — 8-bobdagi sikllarni eslang) solishtiradi, funksiyalar nomi yo'q bo'lsa ham ularni **moslashtiradi**.",
              en:"Comparing two files (before/after a patch) byte-by-byte is nearly useless — because even a single changed function causes the compiler to recompute many addresses, making the whole file look \"different\". Professional tools (e.g. **BinDiff**, **Diaphora**) instead compare functions **structurally** (the call graph between functions, each function's internal control-flow graph — recall the loops from chapter 8), **matching** functions even without names."},
        {t:"h",n:"§",uz:"Amaliy oqim",en:"Practical workflow"},
        {t:"ul",uz:[
          "**1)** Yamoqdan oldingi va keyingi ikkala versiyani ham olib, har birini Ghidra/IDA'da tahlil qiling.",
          "**2)** BinDiff yordamida ikkala tahlilni solishtiring — vosita \"o'zgarmagan\" (moslashgan) va \"o'zgargan\" funksiyalarni ajratib ko'rsatadi.",
          "**3)** O'zgargan funksiyalar ichida — ko'pincha yangi `cmp`/`if` (7-bob) tekshiruvi qo'shilgan bo'ladi. Aynan **shu yangi tekshiruv** — yamalgan zaiflikning \"salbiy tasviri\": u yo'q bo'lgan versiyada zaiflik bor edi."],
          en:[
          "**1)** Take both the pre-patch and post-patch versions, analyze each in Ghidra/IDA.",
          "**2)** Compare both analyses with BinDiff — the tool highlights \"unchanged\" (matched) and \"changed\" functions.",
          "**3)** Inside changed functions — a new `cmp`/`if` check (chapter 7) has often been added. That **new check** is the \"negative image\" of the patched vulnerability: without it, the vulnerability existed."]},
        {t:"note",warn:true,uz:"1-day zaifliklarni izlash — kuchli, lekin xavfli ko'nikma: topilgan zaiflikni yamalmagan tizimlarga qarshi ishlatish jinoyat. Professional tadqiqotchilar buni **mas'uliyatli oshkor qilish** (responsible disclosure) yoki tashkilotning o'z ichki xavfsizlik jarayoni doirasida qiladi.",
              en:"1-day vulnerability research is a powerful but risky skill: using a found vulnerability against unpatched systems is a crime. Professional researchers do this within **responsible disclosure** or an organization's own internal security process."},
      ]},
    { uz:"To'liq professional vositalar to'plami", en:"The full professional toolkit", subUz:"Qaysi vosita qachon kerak — batafsil xarita", subEn:"Which tool for which situation — a detailed map",
      blocks:[
        {t:"p",uz:"7-bobda asosiy vositalarni ko'rgan edik. Bu bob — o'rta darajadagi RE mutaxassisi haqiqatda ishlatadigan **to'liq vositalar to'plami**ni, vazifa turi bo'yicha tartiblab beradi.",
              en:"Chapter 7 introduced the basic tools. This chapter organizes the **full toolkit** a mid-level RE professional actually uses, by task type."},
        {t:"h",n:"§",uz:"Statik tahlil va dekompilyatsiya",en:"Static analysis & decompilation"},
        {t:"ul",uz:[
          "**Ghidra** (bepul, NSA) — kuchli dekompilyator, skriptlash (Java/Python) qo'llab-quvvatlaydi.",
          "**IDA Pro** (tijorat) — sanoat standarti, eng yaxshi disassembly sifati va ko'p protsessor arxitekturasini qo'llab-quvvatlaydi.",
          "**Binary Ninja** (tijorat, yengilroq narxda) — zamonaviy interfeys, kuchli skriptlash API'si.",
          "**radare2 / Cutter** (bepul, ochiq kod) — buyruq qatori kuchi + Cutter grafik interfeysi."],
          en:[
          "**Ghidra** (free, NSA) — a powerful decompiler, supports scripting (Java/Python).",
          "**IDA Pro** (commercial) — the industry standard, best disassembly quality and broadest CPU architecture support.",
          "**Binary Ninja** (commercial, lower-priced) — a modern interface, a strong scripting API.",
          "**radare2 / Cutter** (free, open source) — command-line power plus Cutter's graphical front end."]},
        {t:"h",n:"§",uz:"Dinamik tahlil va debugging",en:"Dynamic analysis & debugging"},
        {t:"ul",uz:[
          "**x64dbg** (Windows, bepul) — qulay, zamonaviy interfeys.",
          "**WinDbg** (Windows, Microsoft) — yadro darajasidagi debugging uchun zarur.",
          "**GDB + GEF/pwndbg** (Linux) — xavfsizlik tadqiqotiga moslashtirilgan kengaytmalar bilan.",
          "**Frida** (ko'p platformali) — DBI, ayniqsa mobil (o'tgan boblarni eslang)."],
          en:[
          "**x64dbg** (Windows, free) — a friendly, modern interface.",
          "**WinDbg** (Windows, Microsoft) — needed for kernel-level debugging.",
          "**GDB + GEF/pwndbg** (Linux) — with extensions tailored for security research.",
          "**Frida** (cross-platform) — DBI, especially for mobile (recall the previous chapters)."]},
        {t:"h",n:"§",uz:"Maxsus vazifalar",en:"Specialized tasks"},
        {t:"ul",uz:[
          "**YARA** — zararli dastur oilalarini aniqlash uchun naqsh-bazasi tili (25-bobdagi IOC tushunchasini eslang).",
          "**Wireshark** — tarmoq trafigini tahlil qilish (Kali modulida chuqur o'rganilgan).",
          "**dnSpy / ILSpy / JADX** — .NET va Java/Android reversing (o'tgan ikki bob).",
          "**BinDiff / Diaphora** — binar taqqoslash (o'tgan bob).",
          "**010 Editor / HxD** — hex muharrirlar, fayl formatini qo'lda o'rganish uchun (12-bobni eslang).",
          "**Cuckoo Sandbox** — avtomatlashtirilgan, ajratilgan zararli dastur tahlili muhiti (25-bob)."],
          en:[
          "**YARA** — a pattern-matching language for identifying malware families (recall the IOC concept from chapter 25).",
          "**Wireshark** — network traffic analysis (studied in depth in the Kali module).",
          "**dnSpy / ILSpy / JADX** — .NET and Java/Android reversing (previous two chapters).",
          "**BinDiff / Diaphora** — binary diffing (previous chapter).",
          "**010 Editor / HxD** — hex editors, for manually studying file formats (recall chapter 12).",
          "**Cuckoo Sandbox** — an automated, isolated malware analysis environment (chapter 25)."]},
        {t:"note",uz:"Vositalarni **birma-bir** o'rganish shart emas — real vazifa kelganda, o'sha vazifaga mos vositani tanlab, hujjatini o'qib boshlash yetarli. Muhimi — bu kitobda o'rgangan **tushunchalar** (registrlar, stek, naqshlar, fayl formatlari), chunki har qanday vosita, oxir-oqibat, aynan shu tushunchalarni vizual qilib ko'rsatadi, xolos.",
              en:"You don't need to master every tool one by one — when a real task arrives, picking the matching tool and reading its docs is enough. What matters is the **concepts** you've learned in this book (registers, the stack, patterns, file formats), because any tool, in the end, is just visualizing exactly those concepts."},
      ]},
    { uz:"Kasbiy yo'l: RE mutaxassisi bo'lish", en:"Career path: becoming an RE professional", subUz:"Bu kitobdan keyin qayerga borish kerak", subEn:"Where to go after this book",
      blocks:[
        {t:"p",uz:"Ushbu kitob sizni **mustahkam o'rta darajaga** olib chiqadigan barcha asosiy tushunchalar bilan qurollantirdi: assembler o'qish, boshqaruv oqimi naqshlari, fayl formatlari, obfuskatsiya, turli platformalar (x86, ARM, JVM, .NET), avtomatlashtirish va professional vositalar. Endi — amaliyot va ixtisoslashuv navbati.",
              en:"This book has equipped you with all the core concepts to reach a **solid mid level**: reading assembly, control-flow patterns, file formats, obfuscation, multiple platforms (x86, ARM, JVM, .NET), automation, and professional tools. Now it's time for practice and specialization."},
        {t:"h",n:"§",uz:"Amaliyot manbalari",en:"Practice resources"},
        {t:"ul",uz:[
          "**crackmes.one** — minglab, qiyinlik darajasi bo'yicha tartiblangan CrackMe'lar (13, 21-22-boblardagi kabi).",
          "**CTF musobaqalari** (masalan picoCTF, HackTheBox) — \"reversing\" toifasidagi masalalar, real vaqt bosimi ostida amaliyot.",
          "**GitHub'dagi ochiq kodli malware namunalari** (faqat xavfsiz, ajratilgan muhitda, ta'lim maqsadida) — MalwareBazaar kabi manbalar orqali.",
          "**beginners.re** — Yurichevning asl, to'liq kitobi — chuqurroq, keng qamrovli o'qish uchun (muqaddimada eslatilgan)."],
          en:[
          "**crackmes.one** — thousands of CrackMes sorted by difficulty (like chapters 13, 21-22).",
          "**CTF competitions** (e.g. picoCTF, HackTheBox) — \"reversing\" category challenges, practice under real time pressure.",
          "**Open-source malware samples on GitHub** (only in a safe, isolated environment, for education) — via sources like MalwareBazaar.",
          "**beginners.re** — Yurichev's original, full book — for deeper, broader reading (mentioned in the preface)."]},
        {t:"h",n:"§",uz:"Ixtisoslashuv yo'nalishlari",en:"Specialization tracks"},
        {t:"ul",uz:[
          "**Zararli dastur tahlilchisi (Malware Analyst)** — 25-bobdagi yo'nalish, IOC va sandbox'larga chuqur cho'mish.",
          "**Zaiflik tadqiqotchisi (Vulnerability Researcher)** — 29-bobdagi patch tahlili yo'nalishi, fuzzing va ekspluatatsiya ishlab chiqish bilan birga.",
          "**Mobil xavfsizlik mutaxassisi** — Java/Android va .NET/iOS reversing'ga ixtisoslashuv (30-31-boblar).",
          "**Sanoat nazorati tizimlari (ICS/SCADA) yoki proshivka xavfsizligi** — ARM va boshqa ko'mma tizim protsessorlariga chuqur e'tibor (24-25-boblar)."],
          en:[
          "**Malware Analyst** — the track from chapter 25, going deep into IOCs and sandboxes.",
          "**Vulnerability Researcher** — the patch-analysis track from chapter 29, combined with fuzzing and exploit development.",
          "**Mobile security specialist** — specializing in Java/Android and .NET/iOS reversing (chapters 30-31).",
          "**Industrial control systems (ICS/SCADA) or firmware security** — deep focus on ARM and other embedded processors (chapters 24-25)."]},
        {t:"note",uz:"Eslatma: bu kitobdagi 22-bobdagi kabi \"kalit topish\" mashqlaridan real ish beruvchi kompaniyalar himoyasi yoki tizimlariga o'tishda — har doim **1-bobdagi axloqiy qoidaga** qayting: faqat ruxsat berilgan, qonuniy maqsadlar uchun.",
              en:"Reminder: when moving from \"find the key\" exercises like chapter 22 to real employer-owned defenses or systems — always return to the **ethical rule from chapter 1**: only for authorized, lawful purposes."},
      ]},
    { uz:"Ghidra bilan amaliy ish jarayoni", en:"A practical Ghidra workflow", subUz:"Fayl ochishdan hisobotgacha — qadam-baqadam", subEn:"From opening a file to a report — step by step",
      blocks:[
        {t:"p",uz:"7-bobda Ghidra'ni vosita sifatida tanishtirdik. Bu bob — uni **haqiqiy loyihada qanday ishlatishni**, boshidan oxirigacha, qadam-baqadam ko'rsatadi.",
              en:"Chapter 7 introduced Ghidra as a tool. This chapter shows **how to actually use it on a real project**, from start to finish, step by step."},
        {t:"h",n:"§",uz:"1) Loyiha yaratish va import",en:"1) Creating a project & importing"},
        {t:"p",uz:"Ghidra'da avval **loyiha** (project) yaratiladi — bu bir nechta faylni tahlil qilish uchun konteyner. Fayl **File → Import File** orqali qo'shiladi; Ghidra fayl formatini (12-bobdagi PE/ELF) avtomatik aniqlaydi va protsessor arxitekturasini (x86-64, ARM) taxmin qiladi.",
              en:"In Ghidra, you first create a **project** — a container for analyzing one or more files. A file is added via **File → Import File**; Ghidra automatically detects the file format (PE/ELF from chapter 12) and guesses the processor architecture (x86-64, ARM)."},
        {t:"h",n:"§",uz:"2) Avtomatik tahlil",en:"2) Auto-analysis"},
        {t:"p",uz:"Faylni ikki marta bosib CodeBrowser oynasi ochilganda, Ghidra **\"Analyze?\"** deb so'raydi. Bu bosqichda u butun fayl bo'ylab funksiyalarni aniqlaydi, satrlarni (5-bob) belgilaydi, import jadvalini (12-bob) o'qiydi va — eng muhimi — har bir funksiya uchun **dekompilyatsiya qilingan psevdo-C kod**ni tayyorlaydi. Katta fayllarda bu bir necha daqiqa davom etishi mumkin.",
              en:"When you double-click the file to open the CodeBrowser window, Ghidra asks **\"Analyze?\"**. At this stage it identifies functions across the whole file, marks strings (chapter 5), reads the import table (chapter 12), and — most importantly — prepares **decompiled pseudo-C code** for every function. On large files this can take several minutes."},
        {t:"h",n:"§",uz:"3) Boshlash nuqtasi: matnlar va importlar",en:"3) Starting point: strings and imports"},
        {t:"p",uz:"21-bobdagi CrackMe metodologiyasini eslang: har doim **Window → Defined Strings** bilan boshlang — qiziq matnlarni (\"parol\", \"muvaffaqiyatsiz\", xato xabarlari) topib, ularga **qo'sh bosish** orqali to'g'ridan-to'g'ri o'sha manzilga o'tasiz. Keyin **Symbol Tree → Imports** orqali qaysi tashqi funksiyalar (masalan `CreateFileW`, `strcmp`) chaqirilishini ko'ring — bu dastur nima qila olishi haqida tezkor xarita beradi.",
              en:"Recall the CrackMe methodology from chapter 21: always start with **Window → Defined Strings** — find interesting text (\"password\", \"failed\", error messages) and **double-click** to jump straight to that location. Then check **Symbol Tree → Imports** to see which external functions (e.g. `CreateFileW`, `strcmp`) are called — this gives a quick map of what the program can do."},
        {t:"h",n:"§",uz:"4) Xochlanma havolalar orqali orqaga kuzatish",en:"4) Tracing backward via cross-references"},
        {t:"p",uz:"Qiziqarli matn yoki funksiyani topgach, uni kim **chaqirayotganini** bilish kerak — bu uchun **o'ng tugma → References → Show References to** (yoki `Ctrl+Shift+F`) ishlatiladi. Bu — teskari izlash: \"bu narsaga kim murojaat qilyapti?\" Shu yo'l bilan tekshiruv funksiyasiga (13-bobdagi kabi) yetib borasiz.",
              en:"Once you find an interesting string or function, you need to know who **calls** it — use **right-click → References → Show References to** (or `Ctrl+Shift+F`). This is backward tracing: \"who refers to this?\" This is how you reach the check function (as in chapter 13)."},
        {t:"h",n:"§",uz:"5) Nomlash va izoh qoldirish",en:"5) Renaming and annotating"},
        {t:"p",uz:"Ghidra'ning eng qimmatli xususiyati — siz ko'rgan har narsani **nomlashingiz** mumkin: `FUN_00401500` o'rniga `check_serial`, `local_28` o'rniga `input_buffer`. `L` tugmasi (rename) va `;` (comment) — eng ko'p ishlatadigan qisqa tugmalar. Bu jarayon RE'ning aslida **hujjatlashtirilgan tushunish jarayoni** ekanini ko'rsatadi — kod o'zgarmaydi, faqat sizning unga qo'ygan izohlaringiz to'planadi.",
              en:"Ghidra's most valuable feature is that you can **rename** everything you understand: `FUN_00401500` becomes `check_serial`, `local_28` becomes `input_buffer`. `L` (rename) and `;` (comment) are the most-used shortcuts. This process shows that RE is really a **documented process of understanding** — the code doesn't change, only your accumulated annotations do."},
        {t:"note",uz:"YouTube'da **OALabs** kanalida haqiqiy zararli dastur namunalarini Ghidra bilan boshidan oxirigacha tahlil qiluvchi ko'plab video bor — hatto ingliz tilini to'liq tushunmasangiz ham, ekranda qaysi tugma bosilayotganini shu bobdagi qadamlar bilan solishtirib kuzatishingiz mumkin.",
              en:"The **OALabs** channel on YouTube has many videos analyzing real malware samples with Ghidra from start to finish — even without fully understanding English, you can follow which button is clicked by matching it against the steps in this chapter."},
      ]},
    { uz:"IDA Pro va Hex-Rays amaliyoti", en:"IDA Pro & Hex-Rays in practice", subUz:"Sanoat standarti vositaning ish jarayoni", subEn:"The workflow of the industry-standard tool",
      blocks:[
        {t:"p",uz:"**IDA Pro** — Ghidra'ga o'xshash, lekin tijorat vositasi, ko'proq protsessor arxitekturasi va sifatliroq dekompilyatsiyaga ega. Uning ish jarayoni Ghidra'ga juda o'xshaydi, lekin bir nechta noyob xususiyati bor.",
              en:"**IDA Pro** is similar to Ghidra, but a commercial tool with broader CPU architecture support and higher-quality decompilation. Its workflow closely resembles Ghidra's, but it has a few unique features."},
        {t:"h",n:"§",uz:"Grafik ko'rinish va psevdo-kod oynasi",en:"Graph view and the pseudocode window"},
        {t:"p",uz:"IDA'ning **grafik ko'rinishi** (Space tugmasi bilan almashtiriladi) har bir funksiyani vizual blok-diagramma sifatida ko'rsatadi — shartli sakrashlar (7-bob) va sikllar (8-bob) yashil/qizil chiziqlar bilan ko'rinadi, bu boshqaruv oqimini tez tushunishga yordam beradi. `F5` tugmasi — **Hex-Rays dekompilyatori**ni ochadi, u assemblerni to'g'ridan-to'g'ri psevdo-C kodga aylantiradi.",
              en:"IDA's **graph view** (toggled with Space) shows every function as a visual block diagram — conditional jumps (chapter 7) and loops (chapter 8) appear as green/red lines, helping you quickly grasp control flow. `F5` opens the **Hex-Rays decompiler**, which turns assembly directly into pseudo-C code."},
        {t:"h",n:"§",uz:"FLIRT imzolari",en:"FLIRT signatures"},
        {t:"p",uz:"IDA'ning noyob kuchi — **FLIRT** (Fast Library Identification and Recognition Technology): u ma'lum kutubxona funksiyalarining \"barmoq izi\"ni (baytlar naqshi) tanib, hatto ular statik bog'langan (12-bobdagi bog'lash tushunchasi) va nomlari yo'qolgan bo'lsa ham, ularni avtomatik nomlaydi. Bu — o'zingiz yozmagan, lekin dasturga \"quyilgan\" standart kutubxona kodini (masalan C runtime) tez tanib olishga yordam beradi.",
              en:"IDA's unique power is **FLIRT** (Fast Library Identification and Recognition Technology): it recognizes known library functions' \"fingerprint\" (byte pattern) and automatically names them, even when statically linked (recall linking from chapter 12) and stripped of names. This helps you quickly recognize standard library code (e.g. the C runtime) baked into a program that you didn't write."},
        {t:"note",uz:"Amaliy maslahat: FLIRT tufayli IDA'da funksiyalar ro'yxatining katta qismi allaqachon nomlangan (`__libc_start_main`, `malloc`...) ko'rinishi mumkin — bu chalg'itmasin, aynan **nomlanmagan, dasturga xos** funksiyalarga e'tibor bering, chunki asl mantiq shu yerda.",
              en:"Practical tip: because of FLIRT, a large part of IDA's function list may already appear named (`__libc_start_main`, `malloc`...) — don't be distracted by this; focus on the **unnamed, program-specific** functions, since that's where the real logic lives."},
      ]},
    { uz:"x64dbg va WinDbg amaliy buyruqlari", en:"x64dbg & WinDbg practical commands", subUz:"Debuggerni haqiqatda boshqarish", subEn:"Actually driving a debugger",
      blocks:[
        {t:"p",uz:"7-bobda debuggerlarni tanishtirdik. Bu yerda — kundalik ishda eng ko'p ishlatiladigan **aniq buyruqlar va tugmalar**.",
              en:"Chapter 7 introduced debuggers. Here are the **exact commands and shortcuts** used most in daily work."},
        {t:"h",n:"§",uz:"x64dbg (Windows, foydalanuvchi rejimi)",en:"x64dbg (Windows, user mode)"},
        {t:"code",lang:"text",cap:{uz:"Eng ko'p ishlatiladigan tugmalar",en:"Most-used shortcuts"},lines:[
          "F9   -- ishga tushirish / davom ettirish",
          "F8   -- bitta buyruq (step over -- call'ni chaqirmasdan o'tkazadi)",
          "F7   -- bitta buyruq (step into -- call ichiga kiradi)",
          "F2   -- uzilish nuqtasi (breakpoint) qo'yish/olib tashlash",
          "Ctrl+G -- ma'lum manzilga o'tish",
          "bp funksiya_nomi -- nomlangan funksiyaga uzilish nuqtasi"]},
        {t:"h",n:"§",uz:"WinDbg (Windows, foydalanuvchi va yadro rejimi)",en:"WinDbg (Windows, user and kernel mode)"},
        {t:"p",uz:"WinDbg — buyruq qatoriga asoslangan, kuchliroq, lekin qiyinroq o'rganiladigan vosita. Uning eng qimmatli buyrug'i — **`!analyze -v`**: dastur qulagach (crash), bu buyruq avtomatik ravishda qulash sababini, stek izini va tegishli ma'lumotni tahlil qilib beradi — bu zararli dastur va barqarorlik muammolarini tez diagnostika qilish uchun juda muhim.",
              en:"WinDbg is command-line based, more powerful but harder to learn. Its most valuable command is **`!analyze -v`**: after a crash, this command automatically analyzes the crash cause, the stack trace, and related information — essential for quickly diagnosing malware and stability issues."},
        {t:"code",lang:"text",cap:{uz:"Foydali WinDbg buyruqlari",en:"Useful WinDbg commands"},lines:[
          "!analyze -v        -- qulashni avtomatik tahlil qil",
          "k                  -- stek izi (call stack, 6-9-boblarni eslang)",
          "u eip L10          -- joriy manzildan 10 buyruqni disassemble qil",
          "dd/dq manzil       -- xotirani DWORD/QWORD sifatida ko'rsat",
          "bp manzil          -- uzilish nuqtasi",
          "lm                 -- yuklangan modullar (DLL) ro'yxati"]},
        {t:"note",warn:true,uz:"**Yadro rejimi debugging** (kernel debugging) — WinDbg'ning eng kuchli, lekin xavfli qismi: u butun operatsion tizimni ikkinchi mashinadan (yoki virtual mashinadan) boshqarishga imkon beradi, drayverlar va yadro darajasidagi zararli dasturlarni (rootkit) tahlil qilish uchun zarur. Bu — keyingi bobda batafsil.",
              en:"**Kernel-mode debugging** is WinDbg's most powerful but riskiest capability: it lets you control an entire OS from a second machine (or VM), needed to analyze drivers and kernel-level malware (rootkits). More on this in the next chapter."},
      ]},
    { uz:"Frida skriptlash — amaliy retseptlar", en:"Frida scripting — practical recipes", subUz:"Real vaziyatlar uchun tayyor namunalar", subEn:"Ready-made patterns for real situations",
      blocks:[
        {t:"p",uz:"28-bobda Frida'ning g'oyasini ko'rgan edik. Bu yerda — mobil va desktop tahlilida eng ko'p uchraydigan **to'rt real vazifa** uchun skript naqshlari.",
              en:"Chapter 28 introduced Frida's core idea. Here are script patterns for the **four most common real-world tasks** in mobile and desktop analysis."},
        {t:"h",n:"§",uz:"1) Funksiya natijasini almashtirish",en:"1) Replacing a function's return value"},
        {t:"code",lang:"text",cap:{uz:"Masalan: 'root aniqlangan' tekshiruvini aylanib o'tish",en:"E.g. bypassing a \"root detected\" check"},lines:[
          "var isRootedPtr = Module.findExportByName(null, \"isDeviceRooted\");",
          "Interceptor.replace(isRootedPtr, new NativeCallback(function () {",
          "    return 0;  // -- doim \"root yo'q\" deb javob ber",
          "}, 'int', []));"]},
        {t:"h",n:"§",uz:"2) Argumentlarni ko'rish va o'zgartirish",en:"2) Viewing and modifying arguments"},
        {t:"p",uz:"`onEnter` ichida `args[0]`, `args[1]`... orqali funksiyaga uzatilgan argumentlarni (9-bobdagi kelishuvni eslang) ko'rish va hatto o'zgartirish mumkin — bu tarmoq manzili yoki fayl yo'lini \"soxtalashtirish\" uchun ishlatiladi.",
              en:"Inside `onEnter`, you can view and even modify the arguments passed to a function (recall the convention from chapter 9) via `args[0]`, `args[1]`... — used to \"spoof\" a network address or file path, for example."},
        {t:"h",n:"§",uz:"3) Java/Kotlin metodlarini ushlash (Android)",en:"3) Hooking Java/Kotlin methods (Android)"},
        {t:"code",lang:"text",cap:{uz:"Java sinfi metodini Frida orqali ushlash",en:"Hooking a Java class method via Frida"},lines:[
          "Java.perform(function () {",
          "    var LoginCheck = Java.use(\"com.example.app.LoginCheck\");",
          "    LoginCheck.verify.implementation = function (pass) {",
          "        console.log(\"Kiritilgan parol: \" + pass);",
          "        return true;  // -- 32-bobdagi Java dunyosiga bevosita bog'liq",
          "    };",
          "});"]},
        {t:"h",n:"§",uz:"4) Xotiradan qidiruv",en:"4) Scanning memory"},
        {t:"p",uz:"`Memory.scan()` orqali butun jarayon xotirasidan ma'lum bayt naqshini (masalan shifrlash kaliti yoki maxsus signatura) qidirish mumkin — bu 30-bobdagi shellcode tahlili va 29-bobdagi obfuskatsiya tahlilida qo'l keladi.",
              en:"`Memory.scan()` lets you search the entire process memory for a specific byte pattern (e.g. an encryption key or a special signature) — useful for the shellcode analysis of chapter 30 and the obfuscation analysis of chapter 29."},
        {t:"note",uz:"Frida skriptlarini o'rganishning eng tez yo'li — GitHub'dagi ochiq \"frida-scripts\" to'plamlarini o'qish va sinab ko'rish (o'zingizning qurilmangizda, ruxsat berilgan ilovada).",
              en:"The fastest way to learn Frida scripting is to read and try open \"frida-scripts\" collections on GitHub (on your own device, with an authorized app)."},
      ]},
    { uz:"radare2 buyruqlar to'plami", en:"radare2 command cookbook", subUz:"Buyruq qatorida tezkor tahlil", subEn:"Fast analysis from the command line",
      blocks:[
        {t:"p",uz:"**radare2** (yoki uning grafik interfeysi **Cutter**) — bepul, buyruq qatoriga asoslangan, juda tezkor vosita. Grafik vositalarga qaraganda o'rganish qiyinroq, lekin skript va avtomatlashtirish (27-bobni eslang) uchun ayniqsa kuchli.",
              en:"**radare2** (or its graphical front end **Cutter**) is a free, command-line-based, very fast tool. It's harder to learn than graphical tools, but especially powerful for scripting and automation (recall chapter 27)."},
        {t:"code",lang:"text",cap:{uz:"Asosiy r2 buyruqlari",en:"Core r2 commands"},lines:[
          "r2 -A dastur_nomi   -- faylni oching va avtomatik tahlil qiling",
          "aa                   -- (ichida) avtomatik tahlilni qayta ishga tushirish",
          "afl                  -- barcha topilgan funksiyalar ro'yxati",
          "pdf @ main           -- 'main' funksiyasini disassemble qilib chop et",
          "VV                   -- vizual grafik rejimga o'tish (IDA'ning grafik ko'rinishiga o'xshash)",
          "/ \"parol\"           -- fayl ichidan 'parol' matnini qidirish (5-bobni eslang)",
          "iz                   -- barcha aniqlangan satrlarni ko'rsatish",
          "ii                   -- import qilingan funksiyalar ro'yxati (12-bobni eslang)"]},
        {t:"p",uz:"Diqqat qiling: `pdf`, `iz`, `ii` kabi buyruqlar — bu kitobda ko'rgan tushunchalarning (disassembly, matnlar, importlar) shunchaki boshqa vositadagi ko'rinishi. Vositalar farq qiladi, lekin siz izlayotgan **ma'lumot bir xil** qoladi.",
              en:"Notice that commands like `pdf`, `iz`, `ii` are simply another tool's view of the same concepts you've seen throughout this book (disassembly, strings, imports). The tools differ, but the **information you're looking for stays the same**."},
        {t:"note",uz:"radare2'ning kuchli tomoni — u Python bilan (r2pipe kutubxonasi orqali) juda oson skriptlanadi, bu uni 27-bobdagi avtomatlashtirish ish oqimiga tabiiy ravishda ulaydi.",
              en:"radare2's strength is that it's very easy to script with Python (via the r2pipe library), naturally connecting it to the automation workflow from chapter 27."},
      ]},
    { uz:"Yadro rejimi va drayver reversing", en:"Kernel mode & driver reversing", subUz:"Ring 0 — eng chuqur qatlam", subEn:"Ring 0 — the deepest layer",
      blocks:[
        {t:"p",uz:"Shu paytgacha ko'rgan barcha kod **foydalanuvchi rejimida** (user mode, \"Ring 3\") ishlagan. Operatsion tizimning o'zi va drayverlar esa **yadro rejimida** (kernel mode, \"Ring 0\") ishlaydi — bu yerda cheklovlar deyarli yo'q: to'g'ridan-to'g'ri apparatga, boshqa jarayonlarning xotirasiga kirish mumkin.",
              en:"Everything we've seen so far ran in **user mode** (\"Ring 3\"). The OS itself and drivers run in **kernel mode** (\"Ring 0\") — where restrictions are almost nonexistent: direct hardware access, access to other processes' memory."},
        {t:"h",n:"§",uz:"Nega bu muhim",en:"Why this matters"},
        {t:"p",uz:"Eng ilg'or **rootkit**lar (o'z-o'zini yashiruvchi zararli dastur) va eng qattiq **anti-cheat**/DRM himoyalar aynan yadro rejimida ishlaydi, chunki bu yerdan ular hatto foydalanuvchi rejimidagi debuggerdan (7-bob) ham **yashirinishi** mumkin. Bunday kodni tahlil qilish uchun oddiy debugger yetarli emas — **ikkita mashina** kerak: biri tahlil qilinayotgan tizim, ikkinchisi esa unga tashqaridan ulanadigan debugger (masalan WinDbg kernel debugging rejimida).",
              en:"The most advanced **rootkits** (self-hiding malware) and the strictest **anti-cheat**/DRM protections run precisely in kernel mode, because from there they can **hide** even from a user-mode debugger (chapter 7). Analyzing such code requires more than a plain debugger — it needs **two machines**: one being analyzed, and a second connecting to it externally (e.g. WinDbg in kernel debugging mode)."},
        {t:"h",n:"§",uz:"Drayver strukturasi",en:"Driver structure"},
        {t:"p",uz:"Windows drayveri (`.sys` fayli) — 12-bobdagi PE formatining bir turi, lekin oddiy `main()` o'rniga **`DriverEntry`** kirish nuqtasidan boshlanadi. Drayver o'zini ro'yxatdan o'tkazib, foydalanuvchi rejimidagi dasturlardan **IOCTL** (Input/Output Control) so'rovlarini qabul qiladi — bu foydalanuvchi va yadro rejimi orasidagi asosiy aloqa kanali. RE'da IOCTL kodlarini (odatda `switch`/jump table, 12-bobni eslang) tahlil qilish — drayver \"nima qila olishi\"ni tushunishning kaliti.",
              en:"A Windows driver (`.sys` file) is a variant of the PE format from chapter 12, but starts from a **`DriverEntry`** entry point instead of a plain `main()`. A driver registers itself and accepts **IOCTL** (Input/Output Control) requests from user-mode programs — the primary communication channel between user and kernel mode. In RE, analyzing IOCTL codes (usually a `switch`/jump table, recall chapter 12) is the key to understanding what a driver can do."},
        {t:"note",warn:true,uz:"Yadro rejimidagi xatolik butun tizimni qulatadi (\"blue screen\"), shuning uchun bunday tahlil **har doim virtual mashinada** olib boriladi, hech qachon shaxsiy asosiy tizimda emas.",
              en:"A kernel-mode error crashes the entire system (\"blue screen\"), so this kind of analysis is **always done in a virtual machine**, never on a personal main system."},
      ]},
    { uz:"Proshivka va IoT qurilmalarini reversing qilish", en:"Firmware & IoT device reversing", subUz:"Fayl formatidan tashqaridagi dunyo", subEn:"The world beyond the file format",
      blocks:[
        {t:"p",uz:"19-bobda PE/ELF formatlarini ko'rdik — bular operatsion tizim ishga tushiradigan fayllar. Ko'p **IoT qurilmalar** (routerlar, kameralar, aqlli uy asboblari) esa butun **proshivka**ni (firmware) — operatsion tizim + dastur + sozlamalar — bitta katta faylda saqlaydi, u operatsion tizim tomonidan emas, **qurilmaning o'zi** tomonidan yuklanadi.",
              en:"Chapter 19 covered PE/ELF formats — files an OS launches. Many **IoT devices** (routers, cameras, smart home appliances), however, store their entire **firmware** — OS + program + settings — as one large file, loaded not by an OS but by **the device itself**."},
        {t:"h",n:"§",uz:"Proshivkani ajratish",en:"Extracting the firmware"},
        {t:"p",uz:"Birinchi qadam — proshivka faylining ichida nima borligini bilish. **`binwalk`** vositasi (Kali modulidagi vositalar ruhida) fayl ichidan tanish \"imzolarni\" (masalan siqilgan fayl tizimi, ELF header, sertifikat) avtomatik qidiradi va topilgan qismlarni **ajratib chiqaradi** — ko'pincha bitta proshivka faylida to'liq Linux fayl tizimi yashiringan bo'ladi.",
              en:"The first step is finding out what's inside the firmware file. The **`binwalk`** tool (in the spirit of the Kali module's tooling) automatically scans for known \"signatures\" inside the file (e.g. a compressed filesystem, an ELF header, a certificate) and **extracts** what it finds — often a full Linux filesystem is hidden inside a single firmware file."},
        {t:"h",n:"§",uz:"Fayl tizimidan keyin — oddiy ELF tahlili",en:"After the filesystem — ordinary ELF analysis"},
        {t:"p",uz:"Fayl tizimi ajratilgach, ichidagi ikkilik fayllar odatiy **ELF** fayllar (19-bob) — faqat protsessor odatda x86 emas, **MIPS** yoki **ARM** (23, 28-boblar) bo'ladi, chunki ko'p arzon IoT qurilmalar shu arxitekturalarni ishlatadi. Demak, IoT reversing — aslida siz allaqachon bilgan ko'nikmalarning (ELF tahlili + ARM/boshqa arxitektura) **kombinatsiyasi**, faqat boshqa \"konteynerda\" (proshivka) keladi.",
              en:"Once the filesystem is extracted, the binaries inside are ordinary **ELF** files (chapter 19) — just usually not x86, but **MIPS** or **ARM** (chapters 23, 28), since many cheap IoT devices use those architectures. So IoT reversing is really a **combination** of skills you already have (ELF analysis + ARM/another architecture), just arriving in a different \"container\" (firmware)."},
        {t:"h",n:"§",uz:"UEFI/BIOS — eng past qatlam",en:"UEFI/BIOS — the lowest layer"},
        {t:"p",uz:"Kompyuterning o'zida ham shunga o'xshash tushuncha bor: **UEFI** (zamonaviy) yoki **BIOS** (eski) — operatsion tizim ishga tushishidan **oldin** ishlaydigan proshivka (6-bobdagi \"funksiya chaqirilishidan oldingi\" g'oyaga o'xshaydi, faqat butun tizim darajasida). UEFI reversing — Secure Boot (Windows modulida ko'rilgan) kabi mexanizmlarni chuqur tushunish uchun zarur, va odatda mustaqil, ixtisoslashtirilgan soha hisoblanadi.",
              en:"The computer itself has a similar concept: **UEFI** (modern) or **BIOS** (legacy) — firmware that runs **before** the OS boots (similar to the \"before the function call\" idea in chapter 6, but at whole-system scale). UEFI reversing is needed to deeply understand mechanisms like Secure Boot (covered in the Windows module), and is generally considered its own specialized field."},
      ]},
    { uz:"Mobil ilovalarni chuqur tahlil qilish", en:"Deep mobile app analysis", subUz:"Native kutubxonalar va tamperga qarshi himoya", subEn:"Native libraries and anti-tampering",
      blocks:[
        {t:"p",uz:"30-bobda Java/Android baytkodini ko'rdik. Lekin ko'p zamonaviy Android ilovalari (ayniqsa o'yinlar, bank ilovalari) muhim mantiqni **native kutubxonalarda** (`.so` fayllar — 19-bobdagi ELF, odatda ARM'da) saqlaydi, chunki bu Java baytkodidan ko'ra tahlil qilish qiyinroq.",
              en:"Chapter 30 covered Java/Android bytecode. But many modern Android apps (especially games, banking apps) keep critical logic in **native libraries** (`.so` files — ELF from chapter 19, usually ARM), since these are harder to analyze than Java bytecode."},
        {t:"h",n:"§",uz:"JNI — ikki dunyo orasidagi ko'prik",en:"JNI — the bridge between two worlds"},
        {t:"p",uz:"**JNI** (Java Native Interface) — Java kodining native (ARM/x86 mashina kodi) funksiyalarni chaqirish mexanizmi. RE'da odatiy oqim: avval JADX (30-bob) bilan Java qismini o'qib, `System.loadLibrary()` va `native` deb belgilangan metodlarni topasiz — bu sizga **qaysi `.so` faylni va qaysi funksiyani** Ghidra/IDA'da (bu bobdan oldingi ikki bob) tekshirish kerakligini ko'rsatadi.",
              en:"**JNI** (Java Native Interface) is the mechanism by which Java code calls native (ARM/x86 machine code) functions. The typical RE flow: first read the Java side with JADX (chapter 30) and find `System.loadLibrary()` calls and methods marked `native` — this tells you **which `.so` file and which function** to inspect in Ghidra/IDA (the previous two chapters)."},
        {t:"h",n:"§",uz:"Tamperga qarshi (anti-tampering) himoya",en:"Anti-tampering protection"},
        {t:"p",uz:"Ko'p mobil ilova o'zining **o'zgartirilmaganini** tekshiradi: imzo tekshiruvi (fayl xeshi — Kriptografiya kitobidagi SHA-256'ni eslang), root/jailbreak aniqlash, debugger aniqlash (24-bobdagi anti-debug'ga o'xshash, lekin mobil kontekstda). Bu tekshiruvlarni tahlil qilish uchun Frida (28, va shu bobdan oldingi bob) — eng samarali vosita, chunki u tekshiruv natijasini **jonli, dastur ishlab turgan paytda** almashtirish imkonini beradi.",
              en:"Many mobile apps check that they haven't been **tampered with**: signature verification (a file hash — recall SHA-256 from the Cryptography book), root/jailbreak detection, debugger detection (similar to the anti-debug of chapter 24, but in a mobile context). Frida (chapter 28, and the previous chapter) is the most effective tool for analyzing these checks, since it lets you replace the check's result **live, while the app is running**."},
        {t:"note",uz:"Bank va to'lov ilovalarida bu himoyalar ayniqsa qattiq va ko'p qatlamli bo'ladi — bir nechta mustaqil tekshiruv birlashtiriladi, shunda bittasini aylanib o'tish yetarli bo'lmaydi. Bu — 29-bobdagi \"chuqurlik himoyasi\" g'oyasining mobil qo'llanilishi.",
              en:"In banking and payment apps these protections are especially strict and layered — several independent checks are combined, so bypassing just one isn't enough. This is the mobile application of the \"defense in depth\" idea from chapter 29."},
      ]},
    { uz:"Ekspluatatsiyaga yo'naltirilgan RE", en:"Exploitation-oriented RE", subUz:"Zaiflikni topishdan ishlatishgacha", subEn:"From finding a bug to weaponizing it",
      blocks:[
        {t:"p",uz:"14-bobda bufer to'lib ketishini RE nuqtai nazaridan ko'rgan edik. Bu bob — zaiflikni **topishdan** uni **isbotlashgacha** bo'lgan yo'lni, va zamonaviy himoyalarni qisqacha ko'rsatadi. Bu — Web-Pentest va Kali modullaridagi mavzularga to'g'ridan-to'g'ri bog'lanadi, lekin **binar** darajada.",
              en:"Chapter 14 looked at buffer overflow from an RE perspective. This chapter briefly shows the path from **finding** a vulnerability to **proving** it, and modern defenses. This connects directly to topics in the Web-Pentest and Kali modules, but at the **binary** level."},
        {t:"h",n:"§",uz:"Zamonaviy himoyalar",en:"Modern mitigations"},
        {t:"ul",uz:[
          "**Stack canary** — funksiya prologida (6-bob) qaytish manzilidan oldin tasodifiy qiymat joylanadi; epilogda u tekshiriladi — o'zgargan bo'lsa, dastur darhol to'xtaydi.",
          "**ASLR** (Address Space Layout Randomization) — dastur va kutubxonalar har ishga tushirilganda **turli** xotira manzillariga yuklanadi, shuning uchun \"qattiq kodlangan\" manzilga sakrash ishlamaydi.",
          "**DEP/NX** (Data Execution Prevention) — stek va uyum (heap) sohalari **bajarilmaydigan** deb belgilanadi, shuning uchun 30-bobdagi shellcode'ni to'g'ridan-to'g'ri stekka yozib bajarib bo'lmaydi."],
          en:[
          "**Stack canary** — a random value is placed before the return address in the function prologue (chapter 6); the epilogue checks it — if changed, the program halts immediately.",
          "**ASLR** (Address Space Layout Randomization) — the program and libraries load at **different** memory addresses on every run, so jumping to a \"hardcoded\" address no longer works.",
          "**DEP/NX** (Data Execution Prevention) — the stack and heap are marked **non-executable**, so the shellcode from chapter 30 can't simply be written to the stack and run."]},
        {t:"h",n:"§",uz:"ROP — himoyalarni chetlab o'tish g'oyasi",en:"ROP — the idea behind bypassing defenses"},
        {t:"p",uz:"DEP'ni chetlab o'tish uchun ishlab chiqilgan **ROP** (Return-Oriented Programming) — dasturning **o'zidagi**, allaqachon bajariladigan (shuning uchun DEP'ga zid emas) kichik kod bo'laklarini (\"gadget\"lar, odatda `ret` bilan tugaydigan bir necha buyruq) stekka ketma-ket \"qaytish manzillari\" sifatida terib, ularni ketma-ket bajartirish g'oyasi — 6-bobdagi `ret` mexanizmining o'ziga qarshi ishlatilishi.",
              en:"**ROP** (Return-Oriented Programming), developed to bypass DEP, is the idea of chaining small pieces of code already present **in the program** (which is therefore already executable, so it doesn't conflict with DEP) — \"gadgets\", usually a few instructions ending in `ret` — laid out on the stack as consecutive \"return addresses\" so they execute in sequence. It's the `ret` mechanism from chapter 6, turned against itself."},
        {t:"note",warn:true,uz:"Bu bobning maqsadi — tushunchalarni **tanishtirish**, ekspluatatsiya yozishni o'rgatish emas. Ekspluatatsiya ishlab chiqish — RE'dan alohida, chuqur ixtisoslashuv talab qiladigan yo'nalish (38-bobdagi \"Vulnerability Researcher\" yo'nalishini eslang), va faqat qonuniy, ruxsat berilgan doirada (CTF, bug bounty, o'z laboratoriyangiz) amalga oshirilishi kerak.",
              en:"This chapter's goal is to **introduce** the concepts, not to teach exploit writing. Exploit development is its own deep specialization separate from RE (recall the \"Vulnerability Researcher\" track from chapter 38), and should only be practiced within legal, authorized bounds (CTF, bug bounty, your own lab)."},
      ]},
    { uz:"Ramziy bajarish va taint tahlili chuqurroq", en:"Symbolic execution & taint analysis in depth", subUz:"Kodni 'sinab ko'rish' emas, matematik isbotlash", subEn:"Not \"trying\" code — proving it mathematically",
      blocks:[
        {t:"p",uz:"27-bobda **angr**'ni tilga oldik. Bu bob uning orqasidagi g'oyani — **ramziy bajarish**ni (symbolic execution) — va u bilan bog'liq **taint tahlili**ni chuqurroq tushuntiradi.",
              en:"Chapter 27 mentioned **angr**. This chapter explains the idea behind it — **symbolic execution** — and the related concept of **taint analysis** — in more depth."},
        {t:"h",n:"§",uz:"Ramziy bajarish nima",en:"What symbolic execution is"},
        {t:"p",uz:"Oddiy bajarishda o'zgaruvchi **aniq son** (masalan `x = 5`) qiymatiga ega. **Ramziy bajarishda** esa o'zgaruvchi **noma'lum ramz** (`x = X`) sifatida ko'riladi, va dastur shu ramz bilan \"bajariladi\" — har bir shart (7-bob) uchrganda, ikkala tarmoq ham (X qandaydir qiymatda bo'lsa) mumkin deb hisoblanadi, va har bir yo'l uchun **matematik cheklovlar to'plami** yig'iladi. Oxirida, \"qaysi X qiymatida bu yo'lga kirish mumkin?\" degan savol **SMT-solver** (matematik tenglamalar yechuvchi) yordamida avtomatik hal qilinadi.",
              en:"In normal execution, a variable has a **concrete number** (e.g. `x = 5`). In **symbolic execution**, a variable is treated as an **unknown symbol** (`x = X`), and the program \"executes\" with that symbol — at every condition (chapter 7), both branches are considered possible (for some value of X), and a **set of mathematical constraints** is collected for each path. In the end, the question \"for which X can this path be reached?\" is solved automatically by an **SMT solver** (a mathematical equation solver)."},
        {t:"h",n:"§",uz:"22-bobga qaytish — endi avtomatik",en:"Back to chapter 22 — now automated"},
        {t:"p",uz:"Eslang: 22-bobda \"8 ta belgining ASCII yig'indisi 850 ga teng bo'lishi kerak\" degan CrackMe'ni **qo'lda** yechgan edik. Ramziy bajarish bilan bu jarayon avtomatlashadi: angr dasturni ramziy kirish bilan \"bajaradi\", `sete al` (\"muvaffaqiyat\" bayrog'i) 1 ga teng bo'lgan yo'lni belgilaydi, va SMT-solver aynan **shu yo'lga olib keladigan** kiritish qiymatini hisoblab chiqaradi — siz hech qanday algoritmni qo'lda tahlil qilmasdan ham.",
              en:"Recall: in chapter 22 we solved a CrackMe requiring \"the ASCII sum of 8 characters equals 850\" **by hand**. Symbolic execution automates this: angr \"runs\" the program with symbolic input, marks the path where `sete al` (the \"success\" flag) equals 1, and the SMT solver computes the exact input value **that leads to that path** — without you analyzing the algorithm by hand at all."},
        {t:"h",n:"§",uz:"Taint tahlili — ma'lumot manbasini kuzatish",en:"Taint analysis — tracking a data source"},
        {t:"p",uz:"**Taint tahlili** — o'xshash, lekin sodda g'oya: \"ishonchsiz\" manbadan (masalan tarmoq yoki foydalanuvchi kiritishi) kelgan ma'lumotni **belgilab** (\"tainted\"), so'ng dastur bo'ylab shu belgi qayerlarga tarqalishini kuzatish. Agar \"belgilangan\" ma'lumot xavfli funksiyaga (masalan buyruq bajaruvchi funksiya) yetib borsa — bu potentsial zaiflik. Bu — 14-bobdagi bufer to'lib ketishini va boshqa in'ektsiya turdagi zaifliklarni **avtomatik** topishning asosi.",
              en:"**Taint analysis** is a related but simpler idea: **mark** data coming from an \"untrusted\" source (e.g. the network or user input) as \"tainted\", then track where that mark spreads throughout the program. If \"tainted\" data reaches a dangerous function (e.g. one that executes commands) — that's a potential vulnerability. This is the basis for **automatically** finding buffer overflows (chapter 14) and other injection-type vulnerabilities."},
        {t:"note",uz:"Bu texnikalar — zamonaviy avtomatlashtirilgan zaiflik topish vositalarining (fuzzerlar bilan birgalikda) yuragi. Ularni **tushunish** — bu kitobdagi eng murakkab, lekin eng qimmatli senior darajadagi bilim.",
              en:"These techniques are the heart of modern automated vulnerability-finding tools (together with fuzzers). **Understanding** them is the most complex, but most valuable, senior-level knowledge in this book."},
      ]},
    { uz:"O'z tahlil skriptlaringizni yozish", en:"Writing your own analysis scripts", subUz:"Ghidra va IDA'ni dasturlash", subEn:"Programming Ghidra and IDA",
      blocks:[
        {t:"p",uz:"27-bobda tashqi Python vositalarini (Capstone va h.k.) ko'rdik. Lekin Ghidra va IDA'ning o'zi ham **ichki skriptlash** imkoniyatiga ega — bu takrorlanadigan vazifalarni avtomatlashtirishning eng to'g'ridan-to'g'ri yo'li.",
              en:"Chapter 27 covered external Python tools (Capstone, etc.). But Ghidra and IDA themselves also support **built-in scripting** — the most direct way to automate repetitive tasks."},
        {t:"h",n:"§",uz:"Ghidra skriptlari (Java yoki Python)",en:"Ghidra scripts (Java or Python)"},
        {t:"p",uz:"Ghidra'ning **Script Manager** oynasi orqali Java yoki Python (Jython/PyGhidra) skriptlarini yozish va ishga tushirish mumkin. Tipik vazifa: \"barcha funksiyalarni aylanib chiqib, faqat bitta `xor`ga ega bo'lganlarini toping\" (13-bobdagi qo'lda qidiruvni avtomatlashtirish) yoki \"barcha satrlarni chiqarib, faylga yozing\" (5-bob).",
              en:"Through Ghidra's **Script Manager** window you can write and run Java or Python (Jython/PyGhidra) scripts. A typical task: \"iterate over all functions and find those with exactly one `xor`\" (automating the manual search from chapter 13) or \"extract all strings and write them to a file\" (chapter 5)."},
        {t:"h",n:"§",uz:"IDAPython",en:"IDAPython"},
        {t:"p",uz:"IDA'da o'xshash imkoniyat — **IDAPython**: `idc`, `idautils`, `idaapi` modullari orqali IDA'ning barcha funksiyalariga (nomlash, xochlanma havolalarni ko'rish, dekompilyatsiya natijasini olish) dasturiy kirish mumkin. Ko'plab professional RE mutaxassisi o'z kundalik ishini shu kabi skriptlar bilan sezilarli tezlashtiradi — masalan, 500 funksiyali fayldan \"kripto konstantalari\" (masalan AES S-box qiymatlari — Kriptografiya kitobini eslang) bor funksiyalarni bir necha soniyada avtomatik topish.",
              en:"IDA has a similar capability — **IDAPython**: through the `idc`, `idautils`, `idaapi` modules you get programmatic access to all of IDA's functionality (renaming, viewing cross-references, retrieving decompiled output). Many professional RE practitioners significantly speed up their daily work with such scripts — e.g. automatically finding functions containing \"crypto constants\" (like AES S-box values — recall the Cryptography book) in a 500-function file in a few seconds."},
        {t:"note",uz:"Skriptlash — o'rta darajadan senior darajaga o'tishning aniq belgisi: boshlang'ich mutaxassis bir funksiyani qo'lda tahlil qiladi, senior mutaxassis esa \"bu naqshni butun faylda avtomatik toping\" deb skript yozadi.",
              en:"Scripting is a clear marker of the transition from mid-level to senior: a beginner analyzes one function by hand, while a senior writes a script saying \"find this pattern automatically across the whole file\"."},
      ]},
    { uz:"To'liq amaliy tahlil: boshidan oxirigacha", en:"A complete walkthrough: start to finish", subUz:"Barcha boblarni birlashtirib, real vazifani yechish", subEn:"Combining every chapter to solve a realistic task",
      blocks:[
        {t:"p",uz:"Bu bob — butun kitobni birlashtiruvchi **yakuniy amaliy mashq**: nomsiz, kichik Windows dasturi (\"target.exe\") berilgan, deylik, va sizdan uning **litsenziya tekshiruvi mantig'ini to'liq tiklash** so'raladi. Quyida — professional yondashuvning aniq bosqichlari.",
              en:"This chapter is the **final practical exercise** tying the whole book together: imagine you're given a small, unnamed Windows program (\"target.exe\") and asked to **fully reconstruct its license-check logic**. Below are the exact steps of a professional approach."},
        {t:"h",n:"§",uz:"1-bosqich: xavfsiz muhit",en:"Stage 1: a safe environment"},
        {t:"p",uz:"Har doim izolyatsiya qilingan virtual mashinada boshlanadi (25-bobdagi sandbox g'oyasi) — hatto \"faqat CrackMe\" bo'lsa ham, odat qilib olish muhim.",
              en:"Always start in an isolated VM (the sandbox idea from chapter 25) — even for \"just a CrackMe\", it's important to build the habit."},
        {t:"h",n:"§",uz:"2-bosqich: statik razvedka (5-10 daqiqa)",en:"Stage 2: static recon (5-10 minutes)"},
        {t:"p",uz:"`file`/PE-bear bilan fayl turini tekshirish (19-bob) → qadoqlanganmi tekshirish, entropiya baland bo'lsa ha (24, 30-boblar) → matnlarni ko'rish (5-bob) → import jadvalini ko'rish (12-bob). Bu bosqich oxirida sizda dastur haqida **umumiy taassurot** bo'ladi: qadoqlanganmi, qanday API'lardan foydalanadi, qiziq matnlari bormi.",
              en:"Check the file type with `file`/PE-bear (chapter 19) → check if it's packed, high entropy means yes (chapters 24, 30) → view strings (chapter 5) → view the import table (chapter 12). By the end of this stage you have a **general impression**: is it packed, which APIs does it use, are there interesting strings."},
        {t:"h",n:"§",uz:"3-bosqich: qadoqlanganmi? (agar ha bo'lsa)",en:"Stage 3: is it packed? (if so)"},
        {t:"p",uz:"Agar entropiya yuqori va importlar juda kam bo'lsa — 30-bobdagi OEP topish metodologiyasini qo'llang, xotiradan dump qiling, import jadvalini tiklang.",
              en:"If entropy is high and imports are very few — apply the OEP-finding methodology from chapter 30, dump from memory, reconstruct the import table."},
        {t:"h",n:"§",uz:"4-bosqich: tekshiruv funksiyasini topish",en:"Stage 4: finding the check function"},
        {t:"p",uz:"\"Noto'g'ri\"/\"To'g'ri\" matniga xochlanma havolani (Ghidra/IDA boblarini eslang) toping — bu sizni asosiy funksiyaga olib boradi.",
              en:"Find the cross-reference to the \"Wrong\"/\"Correct\" string (recall the Ghidra/IDA chapters) — this leads you to the core function."},
        {t:"h",n:"§",uz:"5-bosqich: dekompilyatsiya + qo'lda tozalash",en:"Stage 5: decompile + manual cleanup"},
        {t:"p",uz:"Hex-Rays/Ghidra dekompilyatorini oching (17-bobdagi optimallashtirish haqidagi ogohlantirishni eslang — kod \"toza\" bo'lmasligi mumkin), o'zgaruvchilarni nomlang, naqshlarni (7-18-boblar: shart, sikl, massiv, struktura) tanib chiqing.",
              en:"Open the Hex-Rays/Ghidra decompiler (recall the optimization warning from chapter 17 — the code may not be \"clean\"), rename variables, recognize patterns (chapters 7-18: conditionals, loops, arrays, structures)."},
        {t:"h",n:"§",uz:"6-bosqich: dinamik tasdiqlash",en:"Stage 6: dynamic confirmation"},
        {t:"p",uz:"x64dbg bilan (bu bobdan oldingi to'rtinchi bob) tekshiruv funksiyasiga uzilish nuqtasi qo'ying, taxminingizni **haqiqiy ijroda** tasdiqlang.",
              en:"With x64dbg (four chapters back), set a breakpoint at the check function, confirm your hypothesis **in real execution**."},
        {t:"h",n:"§",uz:"7-bosqich: hujjatlashtirish",en:"Stage 7: documenting"},
        {t:"p",uz:"25-bobdagi kabi, topilmalaringizni yozing: qaysi funksiya, qanday algoritm, qanday kirish natijaga olib keladi. Bu — professional RE ishining haqiqiy yakuniy mahsuloti — kod emas, **tushuncha va uning hujjati**.",
              en:"As in chapter 25, write up your findings: which function, what algorithm, what input leads to what outcome. This is the real final product of professional RE work — not code, but **understanding, documented**."},
        {t:"note",uz:"E'tibor bering: bu yetti bosqichning **hech biri yangi emas** — ularning barchasi shu kitobning turli boblaridan. Senior darajadagi mutaxassisni ajratib turadigan narsa yangi bilim emas, balki mavjud bilimni **tartibli, ishonchli metodologiyaga** aylantira olish qobiliyati.",
              en:"Notice that **none** of these seven stages is new — they all come from different chapters of this book. What sets a senior-level professional apart isn't new knowledge, but the ability to turn existing knowledge into a **disciplined, reliable methodology**."},
      ]},
    { uz:"Video darslar va qo'shimcha manbalar", en:"Video courses & further resources", subUz:"Tilni tushunmasangiz ham — ekrandan o'rganish", subEn:"Learning from the screen, even without the language",
      blocks:[
        {t:"p",uz:"Bu kitob nazariyani beradi; **video** esa qo'l harakatlarini, vosita interfeysini va fikrlash jarayonini jonli ko'rsatadi. Quyidagi kanallar — RE hamjamiyatida keng tanilgan, sifatli, bepul manbalar. Til to'siq bo'lsa ham: ekranda **qaysi tugma bosilishini**, **qaysi oyna ochilishini** shu kitobdagi boblar bilan solishtirib kuzating — vizual naqsh so'zsiz ham tushunarli bo'ladi.",
              en:"This book gives you theory; **video** shows hand movements, tool interfaces and the live thought process. The channels below are well-known, high-quality, free resources in the RE community. Even if the language is a barrier: watch **which button is clicked**, **which window opens**, and match it against this book's chapters — the visual pattern is understandable even without words."},
        {t:"h",n:"§",uz:"Tavsiya etilgan kanallar",en:"Recommended channels"},
        {t:"ul",uz:[
          "**LiveOverflow** — binar ekspluatatsiya, RE va CTF'ga chuqur, tushunarli kirish (32-bobdagi ROP g'oyasini video orqali ko'rish uchun ayniqsa foydali).",
          "**OALabs** — haqiqiy zararli dastur namunalarini Ghidra bilan boshidan oxirigacha tahlil qilish (33-bobdagi Ghidra ish jarayonini jonli ko'rish uchun).",
          "**John Hammond** — CTF yechimlari, zararli dastur tahlili va raqamli sud ekspertizasi, sokin va bosqichma-bosqich uslubda.",
          "**Guided Hacking** — o'yin xakerlik va RE'ga ixtisoslashgan, 400+ video darslar bilan (Frida va debugger amaliyoti uchun foydali)."],
          en:[
          "**LiveOverflow** — deep, understandable introduction to binary exploitation, RE and CTF (especially useful for seeing the ROP idea from a previous chapter in video form).",
          "**OALabs** — analyzing real malware samples with Ghidra from start to finish (great for seeing the Ghidra workflow from an earlier chapter live).",
          "**John Hammond** — CTF walkthroughs, malware analysis and digital forensics, in a calm, step-by-step style.",
          "**Guided Hacking** — specializes in game hacking and RE, with 400+ video tutorials (useful for Frida and debugger practice)."]},
        {t:"ul",uz:[
          "LiveOverflow: youtube.com/LiveOverflow",
          "OALabs: youtube.com/channel/UC--DwaiMV-jtO-6EvmKOnqg",
          "John Hammond: youtube.com/@_JohnHammond",
          "Guided Hacking: youtube.com/guidedhacking"],
          en:[
          "LiveOverflow: youtube.com/LiveOverflow",
          "OALabs: youtube.com/channel/UC--DwaiMV-jtO-6EvmKOnqg",
          "John Hammond: youtube.com/@_JohnHammond",
          "Guided Hacking: youtube.com/guidedhacking"]},
        {t:"note",uz:"Eng samarali usul: bitta real videoni tanlang (masalan OALabs'dan bitta \"malware analysis\" video), uni shu kitobdagi tegishli bobga (masalan 25 yoki 33-bob) solishtirib, **ikkalasini parallel** kuzating — nazariya va amaliyot birlashganda eng tez o'rganiladi.",
              en:"The most effective method: pick one real video (e.g. one \"malware analysis\" video from OALabs), match it against the relevant chapter in this book (e.g. chapter 25 or 33), and watch **both in parallel** — theory and practice combined is how you learn fastest."},
      ]},
    // ── 44: Tools ───────────────────────────────────────────────
...
const SE_BOOK={
  ...
  chapters:[
    ...
    { uz:"Professional pretekst yaratish (ruxsat etilgan testlar uchun)", en:"Building a professional pretext (for authorized tests)", subUz:"Ishonchli, tekshirilgan rol qanday quriladi", subEn:"How a believable, vetted role is constructed",
      blocks:[
        {t:"p",uz:"5-bobda pretexting tushunchasini ko'rgan edik: hujumchi soxta rol o'ylab topadi. Bu bob — **ruxsat etilgan xavfsizlik testlarida** (masalan tashkilot o'z buyurtmasi bilan) professional pretekst qanday tuzilishini ko'rsatadi — bu, xuddi RE kitobidagi \"CrackMe\" mashqlari kabi, faqat qonuniy, nazorat ostidagi doirada amaliy.",
              en:"In chapter 5 we saw the pretexting concept: an attacker invents a false role. This chapter shows how a professional pretext is built **in authorized security tests** (e.g. commissioned by the organization itself) — practiced only within a lawful, controlled scope, much like the \"CrackMe\" exercises in the RE book."},
        {t:"h",n:"§",uz:"Ishonchli pretekstning uch qismi",en:"Three parts of a believable pretext"},
        {t:"ul",uz:[
          "**Moslik (plausibility)** — rol tashkilot kontekstiga mos bo'lishi kerak: kichik ofisda \"auditor\" ishonarli, katta korporatsiyada \"yangi IT xodimi\" ishonarli.",
          "**Tayyorgarlik (backstory)** — savolga tayyor javob: \"kimning buyrug'i bilan keldingiz?\", \"qaysi bo'limdansiz?\" kabi so'roqlarga izchil javob bera olish.",
          "**Chiqish yo'li (exit strategy)** — shubha tug'ilganda vaziyatdan xotirjam chiqish rejasi, ziddiyatga bormaslik."],
          en:[
          "**Plausibility** — the role must fit the organization's context: an \"auditor\" is believable at a small office, a \"new IT hire\" at a large corporation.",
          "**Backstory readiness** — consistent answers to questions like \"who sent you?\", \"which department are you from?\".",
    ...
    { uz:"SE testini rejalashtirish va ruxsat doirasi", en:"Planning an SE test & rules of engagement", subUz:"Professional ijtimoiy injeneriya testining tuzilishi", subEn:"The structure of a professional social engineering engagement",
      blocks:[
        {t:"p",uz:"Xavfsizlik jamoalari (\"red team\") ba'zan tashkilotning o'zi buyurtmasi bilan nazorat ostida SE testi o'tkazadi — xodimlarning haqiqiy tayyorligini o'lchash uchun. Bu — tasodifiy \"hazil\" emas, balki qattiq tuzilgan jarayon.",
              en:"Security teams (\"red teams\") sometimes run a controlled SE test at an organization's own request — to measure staff's real preparedness. This isn't a casual \"prank\", but a strictly structured process."},
        {t:"h",n:"§",uz:"Ruxsat doirasi (Rules of Engagement)",en:"Rules of Engagement"},
        {t:"ul",uz:[
          "**Aniq maqsad** — nima o'lchanadi: fishing bosish darajasimi, jismoniy kirish imkoniyatimi, telefon orqali ma'lumot olishmi.",
          "**Chegaralar** — qaysi xodimlar, qaysi binolar, qaysi vaqt oralig'ida; kim va qachon to'xtatishi mumkin.",
          "**Xavfsizlik chegaralari** — real zarar keltirmaslik (masalan haqiqiy pul o'tkazmasini amalga oshirmaslik, faqat \"bosdim\" darajasida to'xtash).",
          "**Favqulodda aloqa** — agar xodim politsiyani chaqirsa yoki jiddiy vaziyat yuzaga kelsa, darhol tekshirish uchun kontakt shaxs."],
          en:[
          "**Clear objective** — what's being measured: phishing click rate, physical access feasibility, or phone-based information extraction.",
          "**Boundaries** — which employees, which buildings, which time window; who can halt the test and when.",
          "**Safety boundaries** — no real harm (e.g. never actually executing a money transfer, stopping at the \"click\" level).",
          "**Emergency contact** — a person to verify with immediately if an employee calls the police or a serious situation arises."]},
        {t:"h",n:"§",uz:"Nega bu muhim",en:"Why this matters"},
    ...
    { uz:"Fishing simulyatsiya dasturini qurish", en:"Building a phishing simulation program", subUz:"Himoya tomonidan: butun tashkilotni sinash", subEn:"From the defense side: testing a whole organization",
      blocks:[
        {t:"p",uz:"3-bobda fishing'ni hujumchi nuqtai nazaridan ko'rdik. Bu bob — **himoya** tomonidan: xavfsizlik jamoasi butun tashkilot uchun muntazam fishing simulyatsiyasini qanday quradi.",
              en:"Chapter 3 looked at phishing from the attacker's side. This chapter looks at it from the **defense** side: how a security team builds an ongoing phishing simulation for a whole organization."},
        {t:"h",n:"§",uz:"Dastur tuzilishi",en:"Program structure"},
        {t:"pipe",steps:[
          {ic:"users",tUz:"Segmentatsiya",tEn:"Segmentation",sUz:"bo'lim/lavozim bo'yicha",sEn:"by dept/role"},
          {ic:"mail",tUz:"Shablon yuborish",tEn:"Send templates",sUz:"turli qiyinlik darajasi",sEn:"varying difficulty"},
          {ic:"eye",tUz:"Kuzatish",tEn:"Track",sUz:"bosish, kiritish, xabar berish",sEn:"click, submit, report"},
          {ic:"check",tUz:"Ta'lim",tEn:"Train",sUz:"darhol, shaxsiylashtirilgan",sEn:"immediate, personalized"}]},
        {t:"h",n:"§",uz:"To'g'ri o'lchovlar",en:"The right metrics"},
        {t:"p",uz:"Ko'p yangi dasturlar faqat \"bosish darajasi\"ni (click rate) o'lchaydi — bu chalg'ituvchi bo'lishi mumkin. Yetuqroq dastur uch ko'rsatkichni kuzatadi: **bosish darajasi** (nazariy xavf), **ma'lumot kiritish darajasi** (haqiqiy xavf — parol kiritilganmi), va — eng muhimi — **xabar berish darajasi** (nechta xodim shubhali xatni faol ravishda IT'ga xabar qildi). Vaqt o'tishi bilan maqsad — xabar berish darajasini oshirish, faqat bosishni kamaytirish emas.",
    ...
    { uz:"Vishing testlari qanday olib boriladi", en:"How authorized vishing tests are run", subUz:"Telefon orqali sinov — professional metodologiya", subEn:"Phone-based testing — a professional methodology",
      blocks:[
        {t:"p",uz:"4-bobda vishing'ni hujumchi texnikasi sifatida ko'rdik. **Ruxsat etilgan** vishing testi — xavfsizlik jamoasi tashkilot xodimlarining telefon orqali qanday himoyalanganini tekshirishning nazorat ostidagi usuli.",
              en:"Chapter 4 covered vishing as an attacker technique. An **authorized** vishing test is a controlled method for a security team to check how well an organization's staff are protected over the phone."},
        {t:"h",n:"§",uz:"Xavfsiz test qurilishi",en:"Building a safe test"},
        {t:"ul",uz:[
          "**Skript oldindan tayyorlanadi** va yuridik/rahbariyat tomonidan ko'rib chiqiladi — improvizatsiya emas.",
          "**Haqiqiy zarar chegarasi** — masalan, OTP kod so'ralganda, xodim uni aytishga tayyor bo'lsa ham, testchi darhol to'xtatib, \"bu test edi\" deb tushuntiradi (4-bobdagi haqiqiy hujumdan farqli o'laroq).",
          "**Yozib olish faqat ruxsat bilan** — ko'p yurisdiksiyada telefon suhbatini yozish uchun barcha tomonlarning roziligi yoki qonuniy asos kerak."],
          en:[
          "**The script is prepared in advance** and reviewed by legal/management — not improvised.",
          "**A real-harm boundary** — e.g. if an OTP code is requested and the employee is willing to give it, the tester immediately stops and explains \"this was a test\" (unlike a real attack in chapter 4).",
          "**Recording only with consent** — in many jurisdictions, recording a phone call requires all parties' consent or a lawful basis."]},
    ...
    { uz:"Jismoniy kirish testi metodologiyasi", en:"Physical penetration test methodology", subUz:"8-bobdan professional darajaga", subEn:"From chapter 8 to a professional level",
      blocks:[
        {t:"p",uz:"8-bobda jismoniy ijtimoiy injeneriya texnikalarini (tailgating, badge klonlash) ko'rgan edik. Bu bob — ularni **professional, ruxsat etilgan jismoniy kirish testi** sifatida qanday tuzilishini ko'rsatadi.",
              en:"Chapter 8 covered physical social engineering techniques (tailgating, badge cloning). This chapter shows how they're structured as a **professional, authorized physical penetration test**."},
        {t:"h",n:"§",uz:"Tayyorgarlik bosqichi",en:"The preparation stage"},
        {t:"p",uz:"Jismoniy test binoning tashqi ko'rinishini (Google Street View, ijtimoiy tarmoqdagi xodim rasmlari — keyingi kitobdagi OSINT bilan bog'liq) o'rganishdan boshlanadi: kirish nuqtalari, xavfsizlik kamerasi joylashuvi, xodimlarning odatiy kiyimi (mos kiyinish uchun).",
              en:"A physical test begins by studying the building's exterior (Google Street View, employee photos on social media — connected to OSINT in the next book): entry points, camera placement, typical employee attire (to dress accordingly)."},
        {t:"h",n:"§",uz:"Test paytida — dalolatnoma",en:"During the test — proof of access"},
        {t:"p",uz:"Testchi hech qachon haqiqiy zarar keltirmaydi yoki narsa o'g'irlamaydi — buning o'rniga muvaffaqiyatli kirishni **dalolatlaydi**: masalan cheklangan xonaga qo'yilgan maxsus belgi (\"USI WAS HERE\" kartochka) yoki xavfsizlik jamoasi bilan kelishilgan boshqa zararsiz iz qoldiriladi.",
              en:"A tester never causes real damage or steals anything — instead, successful access is **documented as proof**: e.g. placing a special marker (\"I WAS HERE\" card) in a restricted room, or leaving another harmless trace agreed with the security team."},
    ...
    { uz:"Kognitiv og'ishlar chuqurroq", en:"Cognitive biases in depth", subUz:"Cialdini'dan tashqari yana olti tuzoq", subEn:"Six more traps beyond Cialdini",
      blocks:[
        {t:"p",uz:"2-bobda ta'sirning olti asosiy tamoyilini (Cialdini asosida) ko'rgan edik. Xulq-atvor psixologiyasida yana bir necha **kognitiv og'ish** bor — ular ijtimoiy injeneriyada Cialdini tamoyillari bilan birga, ba'zan mustaqil ishlatiladi.",
              en:"Chapter 2 covered the six core principles of influence (based on Cialdini). Behavioral psychology has several more **cognitive biases** — used in social engineering alongside, or sometimes independently of, Cialdini's principles."},
        {t:"h",n:"§",uz:"Qo'shimcha og'ishlar",en:"Additional biases"},
        {t:"ul",uz:[
          "**Langar effekti (anchoring)** — birinchi eshitilgan raqam yoki fakt keyingi qarorlarga asossiz ta'sir qiladi (\"hisobingizda $50,000 xavf ostida\" — raqam o'zi haqiqiy bo'lmasa ham, qo'rquvni kuchaytiradi).",
          "**Yo'qotishdan qochish (loss aversion)** — odamlar yutishdan ko'ra yo'qotishdan ko'proq qo'rqadi; \"hisobingiz 24 soatda yopiladi\" — yutuq va'dasidan ko'ra kuchliroq ta'sir qiladi.",
          "**Cho'kib ketgan xarajat (sunk cost)** — \"allaqachon 20 daqiqa vaqt sarfladingiz, endi to'xtash behuda\" — davom etishga majburlaydi.",
          "**Tasdiqlash og'ishi (confirmation bias)** — odam allaqachon kutgan narsasiga (masalan \"biz haqiqatan ham yangi yetkazib beruvchi kutayotgan edik\") mos keluvchi xabarga kamroq shubha bilan qaraydi."],
          en:[
          "**Anchoring** — the first number or fact heard unduly influences later decisions (\"$50,000 in your account is at risk\" — the number amplifies fear even if not real).",
          "**Loss aversion** — people fear losing more than they value gaining; \"your account closes in 24 hours\" hits harder than a promise of reward.",
          "**Sunk cost** — \"you've already spent 20 minutes, stopping now would be wasteful\" — pressures continuation.",
    ...
    { uz:"Ichki tahdid (Insider Threat)", en:"Insider threats", subUz:"Hujumchi allaqachon ichkarida bo'lganda", subEn:"When the attacker is already inside",
      blocks:[
        {t:"p",uz:"Shu paytgacha tashqi hujumchini ko'rdik. **Ichki tahdid** — bu tashkilot ichidagi odam (xodim, pudratchi) orqali yuzaga keladigan xavf, va u ijtimoiy injeneriya bilan ikki xil bog'liq.",
              en:"So far we've looked at an external attacker. **Insider threat** is risk coming from someone already inside the organization (employee, contractor), and it connects to social engineering in two ways."},
        {t:"h",n:"§",uz:"Ikki turi",en:"Two types"},
        {t:"ul",uz:[
          "**Ijtimoiy muhandislik qilingan insider** — halol xodim tashqi hujumchi tomonidan (masalan uzoq muddatli \"do'stlik\" yoki shantaj orqali) manipulyatsiya qilinib, bilmasdan ma'lumot oqizadi yoki kirishni beradi.",
          "**Zararli insider** — norozi yoki sotib olingan xodim ataylab tashkilotga zarar yetkazadi; ular ko'pincha o'zlarining **qonuniy** kirish huquqidan foydalanadi, shuning uchun ularni aniqlash texnik jihatdan qiyinroq."],
          en:[
          "**A socially engineered insider** — an honest employee manipulated by an external attacker (e.g. via long-term \"friendship\" or blackmail) into unknowingly leaking data or granting access.",
          "**A malicious insider** — a disgruntled or bribed employee who deliberately harms the organization; they often use their own **legitimate** access, making detection technically harder."]},
    ...
    { uz:"SE hujumidan keyingi javob choralari", en:"Incident response after a successful SE attack", subUz:"Aldov muvaffaqiyatli bo'lganda nima qilish kerak", subEn:"What to do when the deception succeeds",
      blocks:[
        {t:"p",uz:"Bu kitobning ko'p qismi oldini olishga qaratilgan. Lekin ehtiyotkorlikka qaramay, hujum **muvaffaqiyatli** bo'lishi mumkin. Bu bob — shunday holatda qanday harakat qilish kerakligini ko'rsatadi.",
              en:"Most of this book focuses on prevention. But despite caution, an attack **can** succeed. This chapter shows how to respond when it does."},
        {t:"h",n:"§",uz:"Birinchi soat — cheklash",en:"The first hour — containment"},
        {t:"ul",uz:[
          "**Darhol ma'lumot bering** — sharmandalikdan qo'rqib jim turish vaziyatni yomonlashtiradi; xavfsizlik jamoasiga zudlik bilan xabar bering.",
          "**Parolni almashtiring** — agar hisob ma'lumoti berilgan bo'lsa, darhol o'zgartiring (Parollar kitobidagi mavzu), va shu parol ishlatilgan boshqa joylarni ham tekshiring.",
          "**Qurilmani izolyatsiya qiling** — agar zararli havola bosilgan bo'lsa, qurilmani tarmoqdan uzing (Wi-Fi/kabel), IT jamoasi tekshirmaguncha ishlatmang.",
          "**Moliyaviy tranzaktsiyani to'xtatishga urinib ko'ring** — agar pul o'tkazma bo'lsa, bankka **darhol** qo'ng'iroq qiling; birinchi soatlarda tranzaktsiyani bekor qilish imkoniyati yuqoriroq."],
          en:[
          "**Report immediately** — staying silent out of embarrassment worsens the situation; notify the security team right away.",
          "**Change the password** — if credentials were given, change them immediately (a topic from the Passwords book), and check other places that password was reused.",
          "**Isolate the device** — if a malicious link was clicked, disconnect the device from the network (Wi-Fi/cable) until IT checks it.",
          "**Try to halt any financial transaction** — for a money transfer, call the bank **immediately**; the chance of reversing it is highest in the first hours."]},
    ...
    { uz:"Ta'minot zanjiri va uchinchi tomon orqali hujumlar", en:"Supply chain & third-party attacks", subUz:"Sizni emas, hamkoringizni nishonga olish", subEn:"Targeting your partner, not you",
      blocks:[
        {t:"p",uz:"Ba'zan hujumchi tashkilotning o'zini emas, uning **ishonchli hamkori**ni (yetkazib beruvchi, pudratchi, dasturiy ta'minot provayderi) nishonga oladi — chunki hamkorlar ko'pincha kamroq nazorat ostida, lekin **ichkariga ishonchli kirish** huquqiga ega.",
              en:"Sometimes an attacker targets not the organization itself, but its **trusted partner** (a supplier, contractor, software provider) — because partners are often less scrutinized, yet have **trusted access inside**."},
        {t:"h",n:"§",uz:"Tipik sxema",en:"A typical scheme"},
        {t:"p",uz:"Hujumchi kichik, kamroq himoyalangan yetkazib beruvchini buzadi, so'ng o'sha yetkazib beruvchi nomidan (haqiqiy, ishonchli email manzili yoki dasturiy yangilanish kanali orqali) asosiy nishonga hujum qiladi. Nishon xodimi \"tanish\" manbadan kelgan xabarga (7-bobdagi ijtimoiy dalil tamoyiliga o'xshab) kamroq shubha bilan qaraydi — chunki bu haqiqatan **ishonchli aloqa** ichida sodir bo'ladi, soxta emas.",
              en:"An attacker breaches a smaller, less-protected supplier, then attacks the primary target using that supplier's identity (a real, trusted email address or a software update channel). The target's employee is less suspicious of a message from a \"familiar\" source (similar to the social-proof principle from chapter 2) — because it genuinely arrives within a **trusted relationship**, not a fake one."},
        {t:"h",n:"§",uz:"Himoya",en:"Defence"},
        {t:"ul",uz:[
          "**Uchinchi tomon xavfini baholash** — muhim hamkorlarning xavfsizlik amaliyotini tekshirish, shartnomaga xavfsizlik talablarini kiritish.",
          "**Eng kam imtiyoz hamkorlarga ham** — pudratchiga faqat ish uchun zarur tizimlarga kirish berish, hamma narsaga emas.",
          "**\"Ishonchli\" manbani ham tekshirish** — hatto tanish hamkordan g'ayrioddiy so'rov kelsa, baribir 4-bobdagi qoidani qo'llang: to'xtang, boshqa kanal orqali tasdiqlang."],
          en:[
    ...
    { uz:"Sun'iy intellekt davridagi SE — kengroq tahdidlar", en:"SE in the AI era — broader threats", subUz:"Ommaviy hujumlar endi shaxsiylashtirilgan", subEn:"Mass attacks are now personalized",
      blocks:[
        {t:"p",uz:"9-bobda deepfake va ovoz klonlashni ko'rgan edik. Sun'iy intellektning ta'siri bundan ham kengroq: u **ommaviy** fishing hujumlarini ham **shaxsiylashtirilgan** darajaga olib chiqmoqda.",
              en:"Chapter 9 covered deepfakes and voice cloning. AI's impact is broader still: it's pushing **mass** phishing attacks to a **personalized** level."},
        {t:"h",n:"§",uz:"AI yordamida shaxsiylashtirilgan fishing",en:"AI-assisted personalized phishing"},
        {t:"p",uz:"Ilgari spear phishing (3-bob) qo'lda, sekin tayyorlanardi — bitta nishon uchun soatlab OSINT (keyingi kitob) kerak edi. Endi til modellari ochiq ma'lumotlar (LinkedIn profili, kompaniya sayti) asosida **soniyalar ichida**, mukammal grammatikada, nishonning yozish uslubiga o'xshash shaxsiylashtirilgan xat yaratishi mumkin — bu spear phishing narxini keskin pasaytiradi va uni **ommaviy miqyosda** qilish imkonini beradi.",
              en:"Previously, spear phishing (chapter 3) was slow and manual — hours of OSINT (next book) per target. Now language models can generate a personalized email in **seconds**, with perfect grammar, mimicking the target's writing style, based on open information (a LinkedIn profile, a company site) — sharply cutting the cost of spear phishing and enabling it **at mass scale**."},
        {t:"h",n:"§",uz:"Himoya tomonida ham AI",en:"AI on the defense side too"},
        {t:"p",uz:"Yaxshi xabar: zamonaviy email xavfsizlik tizimlari ham til modellaridan foydalanib, nomuvofiqliklarni (masalan yozish uslubi to'satdan o'zgargan xat, yoki domen bilan tarixiy yozishma naqshi mos kelmasligi) avtomatik aniqlaydi. Bu — hujum va himoyaning bir xil texnologiya atrofida davom etuvchi \"poygasi\".",
    ...
    { uz:"Professional SE hisobotini yozish", en:"Writing a professional SE report", subUz:"Topilmalardan tavsiyagacha", subEn:"From findings to recommendations",
      blocks:[
        {t:"p",uz:"Ruxsat etilgan SE testi (10-13-boblarni eslang) oxirida — mahsulot kod yoki video emas, **hisobot**. Bu bob uning tuzilishini ko'rsatadi.",
              en:"At the end of an authorized SE test (recall chapters 10-13), the deliverable isn't code or video — it's a **report**. This chapter shows its structure."},
        {t:"h",n:"§",uz:"Hisobot tarkibi",en:"Report contents"},
        {t:"ul",uz:[
          "**Ijrochi xulosa (executive summary)** — bir sahifada, texnik bo'lmagan rahbariyat uchun: nima qilindi, umumiy xavf darajasi qanday.",
          "**Metodologiya** — qaysi ruxsat doirasida, qanday sxemalar (fishing, vishing, jismoniy) qo'llanildi.",
          "**Topilmalar** — har bir muvaffaqiyatli (va muvaffaqiyatsiz) urinish, sana, xodim toifasi (ism emas — 15-bobdagi \"aybga emas, tizimga\" tamoyiliga mos), qaysi tamoyil (2-bob) ishlatilgani.",
          "**Xavf darajasi** — har bir topilma qanchalik jiddiy (masalan bitta xodim havola bosdi — past; moliya xodimi haqiqiy o'tkazma qildi — yuqori).",
          "**Tavsiyalar** — aniq, amalga oshiriladigan qadamlar: yangi protokol, qo'shimcha trening, texnik nazorat (masalan email filtri)."],
          en:[
          "**Executive summary** — one page, for non-technical leadership: what was done, the overall risk level.",
          "**Methodology** — what scope, which schemes (phishing, vishing, physical) were used.",
          "**Findings** — each successful (and unsuccessful) attempt, date, employee category (not name — matching the \"system, not blame\" principle from chapter 15), which principle (chapter 2) was used.",
          "**Risk rating** — how serious each finding is (e.g. one employee clicking a link — low; a finance employee making a real transfer — high).",
    ...
    { uz:"Kasbiy yo'l: SE va xavfsizlik xabardorligi mutaxassisi", en:"Career path: SE & security awareness professional", subUz:"Bu kitobdan keyin qayerga borish kerak", subEn:"Where to go after this book",
      blocks:[
        {t:"p",uz:"Ushbu kitob sizni ijtimoiy injeneriyani **tanib olish, tushunish va himoyalanish** bo'yicha mustahkam bilim bilan qurollantirdi — psixologiyadan tortib, professional test metodologiyasi va tashkiliy dasturlargacha. Endi — ixtisoslashuv navbati.",
              en:"This book has equipped you with solid knowledge to **recognize, understand and defend against** social engineering — from psychology to professional test methodology and organizational programs. Now it's time for specialization."},
        {t:"h",n:"§",uz:"Ixtisoslashuv yo'nalishlari",en:"Specialization tracks"},
        {t:"ul",uz:[
          "**Xavfsizlik xabardorligi menejeri** — 12-bobdagi yo'nalish: tashkilot bo'ylab trening dasturlarini loyihalash va o'lchash.",
          "**Ruxsat etilgan SE testchisi (red team)** — 10-13-boblardagi yo'nalish: professional, shartnoma asosidagi fishing/vishing/jismoniy testlar.",
          "**Insayder tahdidlarni tahlil qiluvchi** — 16-bobdagi yo'nalish, ko'proq xulq-atvor tahlili va monitoringga yo'naltirilgan.",
          "**Hodisalarga javob (incident response) mutaxassisi** — 17-bobdagi yo'nalish, SE hujumidan keyingi tezkor javob va tergovga ixtisoslashuv."],
          en:[
          "**Security awareness manager** — the track from chapter 12: designing and measuring training programs across an organization.",
          "**Authorized SE tester (red team)** — the track from chapters 10-13: professional, contract-based phishing/vishing/physical tests.",
          "**Insider threat analyst** — the track from chapter 16, focused more on behavioral analysis and monitoring.",
          "**Incident response specialist** — the track from chapter 17, specializing in rapid response and investigation after an SE attack."]},
        {t:"p",uz:"Amaliy tomonni chuqurlashtirish uchun — **OSINT kitobi** (kutubxonada) bevosita davomi hisoblanadi, chunki har qanday professional SE testi OSINT bilan boshlanadi. Texnik tomondan esa **Kali modulidagi L28 (Social Engineering/SET)** darsi amaliy vositalarni ko'rsatadi.",
    ...
    { uz:"Email sarlavhasini tekshirish: amaliy usul", en:"Inspecting email headers: a practical method", subUz:"Xat haqiqatan kimdan kelganini qanday isbotlash", subEn:"How to actually prove who sent an email",
      blocks:[
        {t:"p",uz:"3-bobda \"yuboruvchi manzilini tekshiring\" deb aytgan edik. Bu bob — buni **aniq qanday** qilishni ko'rsatadi: email dasturidagi \"ko'rsatilgan\" ism va manzil hech narsani isbotlamaydi, chunki uni hujumchi osonlik bilan soxtalashtira oladi. Haqiqiy isbot — **email sarlavhasi (header)** ichida.",
              en:"In chapter 3 we said \"check the sender address\". This chapter shows **exactly how**: the \"displayed\" name and address in your email client prove nothing, since an attacker can easily fake them. The real proof lives inside the **email header**."},
        {t:"h",n:"§",uz:"Sarlavhani qanday ochish",en:"How to open the header"},
        {t:"p",uz:"Ko'p email xizmatida (Gmail: \"⋮\" → \"Show original\"; Outlook: \"Fayl → Xususiyatlar\") xatning **to'liq texnik sarlavhasi**ni ko'rish mumkin — bu odatiy ko'rinishda yashiringan, lekin har doim mavjud bo'lgan ma'lumot.",
              en:"In most email services (Gmail: \"⋮\" → \"Show original\"; Outlook: \"File → Properties\") you can view the message's **full technical header** — information hidden in the normal view, but always present."},
        {t:"h",n:"§",uz:"Uch belgi: SPF, DKIM, DMARC",en:"Three signals: SPF, DKIM, DMARC"},
        {t:"code",lang:"text",cap:{uz:"Sarlavhada qidiriladigan qatorlar",en:"Lines to look for in the header"},lines:[
          "Authentication-Results: mx.google.com;",
          "       spf=pass (sending IP domenga ruxsat berilgan)",
          "       dkim=pass (raqamli imzo -- Kriptografiya kitobini eslang)",
          "       dmarc=pass (SPF+DKIM siyosati bilan mos)"]},
        {t:"ul",uz:[
          "**SPF** (Sender Policy Framework) — domen egasi \"faqat shu serverlar mening nomimdan xat yubora oladi\" deb e'lon qiladi; `spf=fail` — jo'natuvchi IP ro'yxatda yo'q, kuchli xavf belgisi.",
          "**DKIM** (DomainKeys Identified Mail) — xat **raqamli imzolangan** (Kriptografiya kitobidagi imzo tushunchasi); `dkim=fail` — xat yo'lda o'zgartirilgan yoki imzosiz yuborilgan.",
          "**DMARC** — SPF va DKIM natijalarini birlashtirib, domen egasining siyosatiga (\"muvofiq bo'lmasa nima qilish kerak\") asoslanadi."],
          en:[
          "**SPF** (Sender Policy Framework) — the domain owner declares \"only these servers may send mail as me\"; `spf=fail` means the sending IP isn't on that list — a strong red flag.",
          "**DKIM** (DomainKeys Identified Mail) — the message is **digitally signed** (recall the signature concept from the Cryptography book); `dkim=fail` means it was altered in transit or sent unsigned.",
    ...
    { uz:"URL va domenni tekshirish amaliyoti", en:"Practical URL & domain inspection", subUz:"Havolaning haqiqiyligini bosmasdan bilish", subEn:"Knowing a link is genuine before clicking",
      blocks:[
        {t:"p",uz:"3-bobda \"havola ustiga sichqonchani olib boring\" deb aytgan edik. Bu bob — havola manzilining o'zini **qanday o'qish va tekshirish** kerakligini ko'rsatadi.",
              en:"In chapter 3 we said \"hover over the link\". This chapter shows **how to actually read and verify** the link address itself."},
        {t:"h",n:"§",uz:"URL'ni to'g'ri o'qish",en:"Reading a URL correctly"},
        {t:"code",lang:"text",cap:{uz:"Qaysi qism 'haqiqiy domen'?",en:"Which part is the \"real domain\"?"},lines:[
          "https://accounts.google.com.secure-login.xyz/verify",
          "                            ^^^^^^^^^^^^^^^^",
          "                            HAQIQIY domen shu -- 'secure-login.xyz'",
          "                            'accounts.google.com' -- shunchaki subdomen nomi, aldov"]},
        {t:"p",uz:"Domen har doim **oxirgi ikki qismdan** (masalan `.xyz` dan oldingi so'z + `.xyz`) o'qiladi, chapdan o'ngga emas. Hujumchilar aynan shundan foydalanadi: `google.com` so'zini domenning **boshiga** (subdomen sifatida) joylab, ko'zga tanish ko'rinadi, lekin brauzer uni butunlay boshqa, hujumchiga tegishli sayt deb biladi.",
              en:"A domain is always read from the **last two parts** (e.g. the word before `.xyz` plus `.xyz`), not left to right. Attackers exploit exactly this: placing `google.com` at the **start** of the domain (as a subdomain) makes it look familiar to the eye, but the browser treats it as an entirely different site belonging to the attacker."},
        {t:"h",n:"§",uz:"Qo'shimcha tekshiruvlar",en:"Additional checks"},
        {t:"ul",uz:[
          "**Domen yoshi** — WHOIS xizmati orqali domen qachon ro'yxatdan o'tganini tekshiring; bank yoki yirik kompaniya domeni odatda yillar oldin ro'yxatdan o'tgan, bir necha kunlik domen — kuchli xavf belgisi.",
          "**Sertifikat ma'lumoti** — brauzerdagi qulf belgisini bosib, sertifikat kimga berilganini ko'ring (Kriptografiya kitobidagi CA tushunchasini eslang) — bu domen egasi haqida qo'shimcha dalil beradi.",
          "**Bitlar o'xshashligi (typosquatting)** — `rn` harflari birga `m`ga o'xshab ko'rinishi, `1` va `l` almashtirilishi kabi hiylalarga alohida diqqat qiling (3-bobdagi \"paypa1\" misolini eslang)."],
          en:[
          "**Domain age** — check via a WHOIS service when the domain was registered; a bank or large company's domain is usually years old, a days-old domain is a strong red flag.",
          "**Certificate info** — click the padlock icon in the browser to see who the certificate was issued to (recall the CA concept from the Cryptography book) — extra evidence about the domain owner.",
    ...
    { uz:"GoPhish bilan simulyatsiya qurish — amaliy qadamlar", en:"Building a simulation with GoPhish — practical steps", subUz:"Ochiq kodli fishing simulyatsiya platformasi", subEn:"An open-source phishing simulation platform",
      blocks:[
        {t:"p",uz:"11-bobda fishing simulyatsiya dasturining **g'oyasini** ko'rdik. Bu bob — buni amalga oshiruvchi haqiqiy, keng tanilgan, ochiq kodli vosita — **GoPhish** — bilan aniq qadamlarni ko'rsatadi.",
              en:"Chapter 11 covered the **idea** of a phishing simulation program. This chapter shows the concrete steps using a real, widely known, open-source tool that implements it — **GoPhish**."},
        {t:"h",n:"§",uz:"Kampaniya qurish qadamlari",en:"Steps to build a campaign"},
        {t:"pipe",steps:[
          {ic:"users",tUz:"Nishonlar",tEn:"Targets",sUz:"xodimlar ro'yxati (CSV)",sEn:"employee list (CSV)"},
          {ic:"mail",tUz:"Shablon",tEn:"Template",sUz:"soxta xat matni",sEn:"fake email text"},
          {ic:"globe",tUz:"Qo'nish sahifasi",tEn:"Landing page",sUz:"nima ko'rsatiladi",sEn:"what's shown"},
          {ic:"eye",tUz:"Natijalar",tEn:"Results",sUz:"jonli dashboard",sEn:"live dashboard"}]},
        {t:"ul",uz:[
          "**1) Nishonlar guruhi** — xodimlar ro'yxatini (ism, email, bo'lim) yuklash — 12-bobdagi segmentatsiya g'oyasini eslang.",
          "**2) Email shablon** — real hujum uslubini taqlid qiluvchi, lekin zararsiz xat matni yaratish (masalan \"parolingiz muddati tugaydi\").",
          "**3) Qo'nish sahifasi** — xodim bosgach ko'rsatiladigan sahifa: yoki darhol o'quv xabari (\"bu fishing test edi\"), yoki (murakkabroq testda) soxta login sahifasi — faqat kiritilgan ma'lumot **saqlanmaydi**, faqat \"kiritildi\" fakti qayd etiladi.",
          "**4) Jonli kuzatish** — kim xatni ochdi, kim havolani bosdi, kim ma'lumot kiritdi — barchasi 12-bobdagi uch o'lchov (bosish/kiritish/xabar berish) bo'yicha real vaqtda kuzatiladi."],
          en:[
          "**1) Target group** — upload the employee list (name, email, department) — recall the segmentation idea from chapter 12.",
          "**2) Email template** — craft text imitating a real attack style, but harmless (e.g. \"your password is expiring\").",
          "**3) Landing page** — the page shown after a click: either an immediate teaching message (\"this was a phishing test\"), or (in a harder test) a fake login page — where entered data is **never stored**, only the fact that something \"was submitted\" is logged.",
    ...
    { uz:"Ijtimoiy tarmoq maxfiyligini sozlash — amaliy qo'llanma", en:"Hardening social media privacy — a practical guide", subUz:"Har bir sozlamaning nima uchun kerakligi", subEn:"Why each setting matters",
      blocks:[
        {t:"p",uz:"Kutubxonadagi OSINT kitobida hujumchi qanday ma'lumot izlashini ko'rasiz. Bu bob — o'sha ma'lumotni **oldindan yopib qo'yish** uchun aniq, platformadan qat'iy nazar qo'llaniladigan qadamlarni beradi.",
              en:"The OSINT book in the library shows what information an attacker looks for. This chapter gives concrete steps — applicable regardless of platform — to **close that information off in advance**."},
        {t:"h",n:"§",uz:"Universal tekshiruv ro'yxati",en:"A universal checklist"},
        {t:"ul",uz:[
          "**Profil ko'rinishi** — \"Faqat do'stlar\" yoki \"Shaxsiy\" rejimiga o'ting; \"Ommaviy\" — har kim, hatto do'st bo'lmagan hujumchi ham ko'radi.",
          "**Tug'ilgan sana va joylashuv** — bular ko'pincha xavfsizlik savollarining javobi (Parollar kitobidagi \"xavfsizlik savollari zaifligi\"ni eslang) — yashiring yoki umuman ko'rsatmang.",
          "**Do'stlar/kuzatuvchilar ro'yxati** — yashiring; bu ro'yxat orqali hujumchi sizning ish joyingiz, oilangizni xaritalab, ishonchli pretekst (13-bob) quradi.",
          "**Ilova ruxsatlari** — vaqti-vaqti bilan ijtimoiy tarmoqqa ulangan uchinchi tomon ilovalarni ko'rib chiqing, keraksizlarini o'chiring — bu 22-bobdagi ta'minot zanjiri xavfiga o'xshaydi, lekin shaxsiy darajada.",
          "**Teglashni cheklash** — boshqalar sizni suratga teglashini \"tasdiqlashdan keyin\" rejimiga o'zgartiring — bu 8-bobdagi jismoniy joylashuv oshkor bo'lishini kamaytiradi."],
          en:[
          "**Profile visibility** — switch to \"Friends only\" or \"Private\"; \"Public\" means anyone, even a non-friend attacker, can see it.",
          "**Birthday and location** — these are often security-question answers (recall the \"weak security questions\" issue from the Passwords book) — hide or omit them entirely.",
          "**Friends/followers list** — hide it; through this list an attacker maps your workplace and family to build a believable pretext (chapter 13).",
          "**App permissions** — periodically review third-party apps connected to your social account, remove unneeded ones — similar to the supply-chain risk in chapter 22, but at a personal level.",
    ...
    { uz:"Apparat xavfsizlik kaliti va parol menejerini sozlash", en:"Setting up a hardware key & password manager", subUz:"9-bob va Parollar kitobini amalda birlashtirish", subEn:"Putting chapter 9 and the Passwords book into practice",
      blocks:[
        {t:"p",uz:"9-bobda MFA turlarini, jumladan apparat kalitlarini (FIDO2/YubiKey) tilga oldik. Bu bob — ularni **amalda sozlashning** aniq qadamlarini ko'rsatadi, chunki bu — deepfake va fishingga qarshi eng kuchli shaxsiy himoya.",
              en:"Chapter 9 mentioned MFA types, including hardware keys (FIDO2/YubiKey). This chapter shows the concrete steps to **actually set them up**, since this is the strongest personal defence against deepfakes and phishing."},
        {t:"h",n:"§",uz:"Nega apparat kalit eng kuchli",en:"Why a hardware key is the strongest option"},
        {t:"p",uz:"TOTP kod (masalan Google Authenticator) hali ham **qo'lda kiritiladi** — hujumchi real vaqtli fishing sahifasi orqali sizdan kodni \"o'g'irlab\", darhol haqiqiy saytga kiritishi mumkin (\"real-time phishing relay\"). Apparat kalit esa saytning **haqiqiy domeni**ni kriptografik tekshiradi (Kriptografiya kitobidagi imzo g'oyasi) — soxta, hatto aynan o'xshash domenda ham ishlamaydi, chunki domen matematik jihatdan mos kelmaydi.",
              en:"A TOTP code (e.g. Google Authenticator) is still **manually typed** — an attacker can \"steal\" it via a real-time phishing page and immediately enter it on the real site (\"real-time phishing relay\"). A hardware key, however, cryptographically verifies the site's **real domain** (the signature idea from the Cryptography book) — it simply won't work on a fake, even near-identical domain, because the domain doesn't mathematically match."},
        {t:"h",n:"§",uz:"Sozlash qadamlari (umumiy)",en:"Setup steps (general)"},
        {t:"ul",uz:[
          "**1)** Muhim hisoblarni aniqlang (email — birinchi navbatda, chunki u orqali boshqalari tiklanadi — Parollar kitobini eslang).",
          "**2)** Hisob xavfsizlik sozlamalarida \"Xavfsizlik kaliti\" yoki \"Ikkinchi faktor\" bo'limini toping.",
          "**3)** Kamida **ikkita** apparat kalit ro'yxatdan o'tkazing — bittasi kundalik, ikkinchisi zaxira (yo'qotilganda hisobdan chiqib qolmaslik uchun).",
          "**4)** Zaxira kodlarni (backup codes) xavfsiz, oflayn joyda saqlang."],
          en:[
          "**1)** Identify your important accounts (email first, since others are recovered through it — recall the Passwords book).",
          "**2)** Find \"Security key\" or \"Second factor\" in the account's security settings.",
          "**3)** Register at least **two** hardware keys — one for daily use, one as a backup (so you're not locked out if one is lost).",
    ...
    { uz:"Tashkilotda 'Xabar qilish' tugmasini joriy qilish", en:"Deploying a \"Report Phishing\" button", subUz:"Xabar berishni bir bosishga tushirish", subEn:"Turning reporting into a single click",
      blocks:[
        {t:"p",uz:"6 va 12-boblarda \"aniq xabar berish yo'li\" muhimligini takrorladik. Bu bob — buni **texnik jihatdan qanday amalga oshirish** kerakligini ko'rsatadi, chunki yaxshi niyat yetarli emas — jarayon **oson** bo'lishi kerak.",
              en:"Chapters 6 and 12 repeated how important \"a clear reporting path\" is. This chapter shows **how to technically implement it**, because good intentions aren't enough — the process must be **easy**."},
        {t:"h",n:"§",uz:"Bir tugmali xabar berish",en:"One-click reporting"},
        {t:"p",uz:"Zamonaviy email tizimlari (Outlook, Gmail Workspace) uchun mavjud **\"Report Phishing\"** plagin/tugmasi xodimga shubhali xatni faqat **bitta bosish** bilan xavfsizlik jamoasiga va (ixtiyoriy) xat provayderiga yuborish imkonini beradi — nusxa ko'chirish, forward qilish yoki qo'lda yozishga hojat yo'q.",
              en:"Modern email systems (Outlook, Gmail Workspace) have a **\"Report Phishing\"** plugin/button available, letting an employee send a suspicious email to the security team and (optionally) the mail provider with just **one click** — no copying, forwarding, or manual writing needed."},
        {t:"h",n:"§",uz:"Nega bu ishlaydi",en:"Why this works"},
        {t:"p",uz:"2-bobdagi psixologiyani eslang: agar xabar berish **qiyin** bo'lsa (bir necha qadam, kutubxonaga forward qilish, sababini yozish), xodim \"keyinroq\" deb o'tkazib yuboradi va unutadi. Ishtiyoq (friction) qanchalik kam bo'lsa, xabar berish darajasi (12-bobdagi asosiy o'lchov) shunchalik yuqori bo'ladi.",
              en:"Recall the psychology from chapter 2: if reporting is **hard** (several steps, forwarding to a mailbox, writing an explanation), an employee will think \"later\" and forget. The less friction, the higher the reporting rate (the key metric from chapter 12)."},
        {t:"h",n:"§",uz:"Tugma ortidagi jarayon",en:"The process behind the button"},
        {t:"ul",uz:[
          "Tugma bosilganda xat avtomatik xavfsizlik jamoasi navbatiga (SOAR yoki oddiy ticket tizimi) tushadi.",
          "Agar bu — 11-bobdagi simulyatsiyaning bir qismi bo'lsa, xodimga **darhol** ijobiy tasdiq ko'rsatiladi (\"To'g'ri! Siz fishingni aniqladingiz\") — bu 12-bobdagi \"jazo emas, o'rgatish\" madaniyatini mustahkamlaydi.",
          "Haqiqiy fishing bo'lsa, xavfsizlik jamoasi bir xil xat boshqa xodimlarga ham yuborilganmi tekshiradi va tarmoq bo'ylab bloklaydi — bitta xabar butun tashkilotni himoyalashi mumkin."],
          en:[
          "When clicked, the message automatically enters the security team's queue (SOAR or a simple ticketing system).",
          "If this is part of a simulation from chapter 11, the employee gets **immediate** positive confirmation (\"Correct! You spotted a phishing attempt\") — reinforcing the \"teach, don't punish\" culture from chapter 12.",
          "If it's a real phishing attempt, the security team checks whether the same email was sent to other employees and blocks it network-wide — one report can protect the whole organization."]},
      ]},
    { uz:"SE testida OSINT vositalaridan amaliy foydalanish", en:"Using OSINT tools in an SE test — practical use", subUz:"Recon bosqichini qo'lga olish", subEn:"Taking control of the recon phase",
      blocks:[
        {t:"p",uz:"1-bobda ijtimoiy injeneriyaning **birinchi bosqichi** — ma'lumot to'plash (recon) ekanini ko'rgan edik. Bu bob — professional, **ruxsat etilgan** SE testida bu bosqich amalda qanday o'tkazilishini ko'rsatadi (to'liq mexanika — kutubxonadagi OSINT kitobida).",
              en:"In chapter 1 we saw that the **first stage** of social engineering is reconnaissance. This chapter shows how that stage is actually run in a professional, **authorized** SE test (full mechanics are in the OSINT book in the library)."},
        {t:"h",n:"§",uz:"Recon-dan pretekstgacha",en:"From recon to pretext"},
        {t:"pipe",steps:[
          {ic:"search",tUz:"OSINT",tEn:"OSINT",sUz:"xodimlar, texnologiya, tuzilma",sEn:"staff, tech, structure"},
          {ic:"users",tUz:"Nishonni tanlash",tEn:"Target selection",sUz:"eng foydali xodim",sEn:"the most useful employee"},
          {ic:"code",tUz:"Pretekst",tEn:"Pretext",sUz:"13-bobdagi rol qurish",sEn:"role-building from ch. 13"},
          {ic:"check",tUz:"Ijro",tEn:"Execution",sUz:"fishing/vishing/jismoniy",sEn:"phishing/vishing/physical"}]},
        {t:"p",uz:"Professional testchi avval LinkedIn orqali tashkilot tuzilishini (kim qaysi lavozimda), keyin kompaniya saytidan texnologiyalarni (\"biz Salesforce ishlatamiz\" kabi karyera e'lonlaridan), so'ng ijtimoiy tarmoqdan xodimlarning shaxsiy uslubini (qanday yozadi, qaysi qisqartmalarni ishlatadi) o'rganadi. Bu ma'lumot **pretekstni** (13-bob) nafaqat ishonarli, balki **maqsadli** qiladi — \"yangi IT xodimi\" o'rniga \"Salesforce migratsiyasi bo'yicha yordamchi konsultant\" kabi aniqroq, tekshirib bo'lmaydigan rol.",
              en:"A professional tester first studies the organization's structure via LinkedIn (who holds which role), then technologies from the company site (\"we use Salesforce\" from career postings), then employees' personal writing style on social media (how they write, which abbreviations they use). This information makes the **pretext** (chapter 13) not just believable, but **targeted** — a specific, hard-to-verify role like \"Salesforce migration support consultant\" instead of a generic \"new IT hire\"."},
        {t:"note",warn:true,uz:"Ruxsat doirasida (10-bob) OSINT qamrovi ham belgilanadi: ba'zi mijozlar \"faqat ochiq korporativ ma'lumot, xodimlarning shaxsiy hisoblari tekshirilmasin\" kabi chegara qo'yadi — bu chegaraga qat'iy rioya qilinadi.",
              en:"The rules of engagement (chapter 10) also define OSINT scope: some clients set boundaries like \"only open corporate information, don't touch employees' personal accounts\" — this boundary is strictly followed."},
      ]},
    { uz:"Qo'ng'iroq markazi va reception uchun tekshiruv skripti", en:"A verification script for call centers & reception", subUz:"Front-line xodimlarni himoyalash", subEn:"Protecting front-line staff",
      blocks:[
        {t:"p",uz:"Qo'ng'iroq markazi va reception xodimlari — SE hujumlarining eng ko'p nishoni, chunki ularning ishi aynan **yordam berish**. Bu bob — ularga beriladigan aniq, amaliy tekshiruv skriptini ko'rsatadi.",
              en:"Call center and reception staff are the most common SE targets, because their job is literally **to help**. This chapter shows the concrete, practical verification script given to them."},
        {t:"h",n:"§",uz:"Uch bosqichli tasdiqlash skripti",en:"A three-step verification script"},
        {t:"ul",uz:[
          "**1) So'rang, bermang.** \"Ismingiz va xodim raqamingizni ayting\" — hech qachon so'ralgan ma'lumotni avval bermang (masalan mijoz hisobidagi ma'lumotni tasdiqlashdan oldin).",
          "**2) Mustaqil tasdiqlang.** Qo'ng'iroq qiluvchi bergan raqamga emas, tizimda **oldindan saqlangan** raqamga qayta qo'ng'iroq qiling — 4-bobdagi qoidaning aynan shu yerdagi qo'llanilishi.",
          "**3) Shubhalansangiz — kechiktiring.** \"Sizga qayta bog'lanaman\" deyishga **har doim** huquqingiz bor, hech kim (hatto \"direktor\" bo'lsa ham) buni rad eta olmaydi — bu skript xodimga institutsional \"yo'q deyish huquqi\"ni beradi."],
          en:[
          "**1) Ask, don't give.** \"Please tell me your name and employee ID\" — never give the requested information first (e.g. confirming account details before verifying the caller).",
          "**2) Verify independently.** Call back the number **already on file** in the system, not the number the caller provided — the rule from chapter 4, applied exactly here.",
          "**3) When in doubt — delay.** You **always** have the right to say \"I'll call you back\"; no one (even a \"director\") can override this — this script gives the employee institutional \"permission to say no\"."]},
        {t:"note",uz:"Eng muhim jihat — bu skript **yozma siyosat** sifatida rasmiylashtirilishi kerak, shunda xodim shaxsan javobgar bo'lib qolmaydi: \"kompaniya siyosati shuni talab qiladi\" deyish, xodimni shaxsiy ziddiyatdan himoya qiladi va rad etishni osonlashtiradi.",
              en:"The most important part is that this script must be formalized as **written policy**, so the employee isn't personally on the hook: saying \"company policy requires this\" protects the employee from personal conflict and makes refusal easier."},
      ]},
    { uz:"Real vaqtda signallarni aniqlash — amaliy mashq", en:"Spotting signals in real time — a practical drill", subUz:"Uchta stsenariy, uchta tahlil", subEn:"Three scenarios, three breakdowns",
      blocks:[
        {t:"p",uz:"Bu kitobning nazariyasini **amalda** sinash uchun — uchta qisqa stsenariy. Har birini o'qib, to'xtang: **qaysi signal(lar)ni** payqadingiz?",
              en:"To put this book's theory **into practice** — three short scenarios. Read each, then stop: **which signal(s)** did you notice?"},
        {t:"h",n:"§",uz:"1-stsenariy",en:"Scenario 1"},
        {t:"p",uz:"Sizga \"IT bo'limi\"dan xat keladi: \"Parolingiz bugun tunga qadar yangilanishi shart, aks holda hisobingiz bloklanadi.\" Havola — `it-support.yourcompany-portal.net`.",
              en:"You receive an email from \"IT\": \"Your password must be updated by tonight, or your account will be locked.\" The link is `it-support.yourcompany-portal.net`."},
        {t:"note",uz:"**Tahlil:** ikki signal — (1) sun'iy shoshilinchlik (2-bob), (2) domen imlosi — kompaniyangiz haqiqiy domeni emas, balki uni **taqlid qiluvchi** boshqa domen (14-bobdagi URL o'qish qoidasini eslang: haqiqiy domen — `yourcompany-portal.net`, sizning kompaniyangiz emas).",
              en:"**Breakdown:** two signals — (1) artificial urgency (chapter 2), (2) domain spelling — not your company's real domain, but a **look-alike** domain (recall the URL-reading rule from chapter 14: the real domain is `yourcompany-portal.net`, not your company)."},
        {t:"h",n:"§",uz:"2-stsenariy",en:"Scenario 2"},
        {t:"p",uz:"Telefon qo'ng'irog'i: \"Men bosh direktorning yordamchisiman, u hozir uchrashuvda. U sizdan darhol $2,000 hadya kartasi sotib olib, kodlarini menga SMS orqali yuborishingizni so'radi — bu mijoz uchun sovg'a.\"",
              en:"A phone call: \"I'm the CEO's assistant, he's in a meeting right now. He's asked you to immediately buy $2,000 in gift cards and text me the codes — it's a gift for a client.\""},
        {t:"note",uz:"**Tahlil:** klassik BEC naqshi (7-bob) — avtoritet (\"bosh direktor nomidan\"), tekshirish imkoniyati yo'q (\"u band\"), va g'ayrioddiy to'lov usuli (hadya kartasi — bu real biznes amaliyotida deyarli hech qachon ishlatilmaydi, aynan kuzatib bo'lmasligi uchun hujumchilar tanlaydi).",
              en:"**Breakdown:** a classic BEC pattern (chapter 7) — authority (\"on behalf of the CEO\"), no way to verify (\"he's busy\"), and an unusual payment method (gift cards — almost never used in real business, chosen by attackers precisely because they're untraceable)."},
        {t:"h",n:"§",uz:"3-stsenariy",en:"Scenario 3"},
        {t:"p",uz:"Kuryer kiyimidagi notanish odam binoga kirmoqchi, qo'lida katta quti, \"qo'llarim band, eshikni ushlab turing\" deydi.",
              en:"A stranger in courier clothing wants to enter the building, hands full with a large box, saying \"my hands are full, hold the door.\""},
        {t:"note",uz:"**Tahlil:** tailgating (8-bob) — jismoniy ijtimoiy injeneriyaning eng oddiy shakli. To'g'ri javob: xushmuomalalik bilan \"Albatta, lekin avval badge'ingizni o'qitib qo'ying, keyin ushlab beraman\" deyish — 8-bobdagi himoya qoidasi.",
              en:"**Breakdown:** tailgating (chapter 8) — the simplest form of physical social engineering. The correct response: politely say \"Of course, but please badge in first, then I'll hold it\" — the defence rule from chapter 8."},
      ]},
    { uz:"Qo'shimcha resurslar va sertifikatlar", en:"Further resources & certifications", subUz:"Bu kitobdan keyingi rasmiy o'rganish yo'llari", subEn:"Formal learning paths after this book",
      blocks:[
        {t:"p",uz:"19-bobda ixtisoslashuv yo'nalishlarini ko'rgan edik. Bu bob — o'sha yo'nalishlarga rasmiy kirish nuqtalarini ko'rsatadi.",
              en:"Chapter 19 covered specialization tracks. This chapter points to formal entry points into those tracks."},
        {t:"h",n:"§",uz:"Tanilgan resurslar",en:"Well-known resources"},
        {t:"ul",uz:[
          "**SANS Institute** — xavfsizlik xabardorligi (SANS SEC- seriyasi) va ijtimoiy muhandislik testi bo'yicha sanoat standarti treninglar va sertifikatlar taqdim etadi.",
          "**Social-Engineer, LLC / DEF CON Social Engineering Village** — SE hamjamiyatining yillik konferensiyasi, jonli \"vishing musobaqasi\" (faqat kelishilgan, qonuniy sharoitda) orqali real ko'nikmalarni ko'rsatish imkoniyati.",
          "**OWASP** — veb-ilova xavfsizligi kontekstida ijtimoiy muhandislik va autentifikatsiya zaifliklari bo'yicha bepul, ochiq materiallar."],
          en:[
          "**SANS Institute** — offers industry-standard training and certifications in security awareness (the SANS SEC- series) and social engineering testing.",
          "**Social-Engineer, LLC / DEF CON Social Engineering Village** — the SE community's annual conference, with a live \"vishing competition\" (only in an agreed, lawful setting) showcasing real skills.",
          "**OWASP** — free, open materials on social engineering and authentication weaknesses in the context of web application security."]},
        {t:"p",uz:"Amaliy ko'nikmalarni sinash uchun — kutubxonadagi **OSINT kitobi** (to'liq davomi) va **Kali modulidagi L28** darsi (SET vositasi). Rivojlanishning eng tez yo'li — nazariyani (bu kitob) amaliyot (ruxsat etilgan CTF yoki laboratoriya mashqlari) bilan doimiy birlashtirish.",
